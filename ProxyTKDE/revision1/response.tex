...
Graph $G_1$ currently serves as a running example for describing how proxies are identified, how computeDRAs algorithm works, and how routing is performed.  Thanks for the suggestion!
...
As we stated in Section 7, except for (bidirectional) Dijikstra, most if not all existing approaches require a preprocessing stage to answer a shortest path or distance query to achieve high efficiency. The query answering stage is highly dependent on the preprocessing stage, and the techniques for preprocessing and for query answering are highly coupled with each other. Hence, it is hard to distinguish  techniques for precomputing from techniques for computing queries. This is also explained in the related work section.
