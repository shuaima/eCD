...
\svs
Thanks. CH is designed for the road network, which is a sparse graph and the average degree is quite low. It is very inefficient for dense graphs. The DBLP dataset is a dense co-authorship network. In our experiment, it takes more than 1 week for the preprocessing of TNR. Thus, we remove all nodes whose degrees are higher than 14, and picked the largest connected component from the remaining graph to form a new dataset. In the experiment, we explained the reason in Section 6.1, Page 6, first paragraph.
...
\svs
Thanks. We presented the space usage in Table 2 in our original version. In the amended version, we have reorganized Section 6 and emphasized this in Section 6.2.
