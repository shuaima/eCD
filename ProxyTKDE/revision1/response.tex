...
We have reorganized our paper, by moving detailed proofs to the appendix and adding a separate section for query answering with routing proxies (Section 5). We have further added an example (Example 5 in Section 5) for query answering with routing proxies. 

That is, graph G1 serves as a running example for describing how proxies are identified, how computeDRAs algorithm works, and how routing is performed.  Thanks for the suggestion!
...
Thanks. In our revised version, we have added a separate section for related work (Section 7), in which three new references are further added and discussed [10, 14, 38].

As we stated in Section 7, except for (bidirectional) Dijikstra, all existing approaches require a preprocessing stage to answer a shortest path or distance query to achieve high efficiency. The query answering stage is highly dependent on the preprocessing stage, and the techniques for preprocessing and for query answering are highly coupled with each other. Hence, it is hard to distinguish  techniques for precomputing from techniques for computing queries. This is also explained in the related work section.
...
To explain our idea more clearly, we further added \ah as one of our baselines in our revised version. Different from the CH-based \tnr, \ah utilizes coordinates information to construct their hierarchical structure. According to the experiments, we can see that though \ah has a small search space and is already very efficient, it still benefits from proxies in terms of efficiency (about 1\% faster). Please also refer to our response to [R1C1].
...
Indeed, we have already done experiment on the dataset suggested by the reviewer in the original version.
...
In Section 6.3, we compare existing approaches with their counterparts with proxies. We can observe that existing approaches benefit from proxies in terms of both query efficiency and space cost.  All these have been highlighted in the summary in the experimental study (Section 6). 
