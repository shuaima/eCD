...
We next present an experimental study to show how proxies can be combined with existing algorithms designed for shortest path and distance queries. By using real-life road networks and social graphs, we conduct experiments to evaluate (1) the properties of proxies, (2) the improvement of existing algorithms with proxies.
...
\hline\hline
DBLP14    &  (14, 090, 9.8)         & (102,085, 72.2)  & 1.56  &  1.5 \\ \hline
DBLP    & (31,475 , 9.9)             & (105,671 , 33.3) & 1.61   & 5.7 \\ \hline
CO      &  (56,277, 12.9)         & (156,329, 35.9)  & 2.38  &  3.47  \\ \hline
FL      &  (140,382, 13.1)        & (378,804, 35.4)  & 5.78  &  9.9 \\ \hline
CA      &  (273,191, 14.4)        & (623,811, 33.0)  & 9.52  &  21.1 \\ \hline
E-US    &  (546,481, 15.2)        & (1,228,876, 34.1)& 18.75  &  52.5  \\ \hline
W-US    &  (869,907, 13.9)        & (2,116,382, 33.8)& 32.29  &  111.9 \\ \hline
C-US    &  (2,034,358, 14.4)      & (4,583,413, 32.5)& 69.94 &  435.8 \\ \hline
US      &  (3,452,222, 14.4)      & (7,927,453, 33.1)& 120.96  &  1,925.4 \\ \hline
\end{tabular}
\end{scriptsize}
\end{center}
\end{table}
...
The second type of datasets is {\em road networks}. We chose seven standard road network datasets of various sizes from the Ninth DIMACS
Implementation Challenge~\cite{dimacs-datasets}\footnote{\url{http://www.dis.uniroma1.it/challenge9/download.shtml}}. Each road network is released as an undirected graph that represents a part of the road network in the United States, where each edge weight is the distance (integers) required to travel between the two endpoints of the edge.
...
the road network to compose $Q_i (i \in \{1,3,5,7\})$, such that the grid
distance of all node pairs in $Q_i$ is in $[2^{i-9}\cdot\ell, 2^{i-8}\cdot\ell)$.
For each query set $Q_i$ $(i\in \{1,3,5,7\})$, we report the average running time of 10,000 queries in the set.
...
\etitle{Algorithms.} We implemented algorithms bidirectional-Dijkstra, \arcflag, \tnr and \ah.
...
We obtain the C++ implementation of \ah from \cite{zhu2013shortest}.
...
\subsection{Performance of Proxies}
Firstly, we conduct experiments to evaluate the effectiveness of proxies, i.e. how many nodes can be represented by proxies, how much extra space we need to store the routing information for the proxies, and how much time we need to find all proxies. Specifically, we evaluate (1) the number of non-trivial proxies, (2) the number and percentage of the nodes represented by the proxies (excluding the proxies themselves from \dras), (3) the extra space cost of using proxies, and (4) the efficiency of our algorithm $\compDRAs$ for computing proxies and their \dras. In this set of experiments, we tested the datasets in Table~1. We fixed the constant $c = 2$ for computing proxies on all graphs. The experimental results are reported in Table~\ref{tab-exp1-proxies-dras}.
...
\stitle{Efficiency}
Algorithm $\compDRAs$ also scales well, and it can be done in less than half an hour for the largest graph US with $2.4$ $\times$ $10^7$ nodes and $5.7$ $\times$ $10^7$ edges. 

\stitle{Space Cost.}To support shortest distance queries, for each node in a \dra, we store its proxy $u$, its distance to $u$ and the component of $A^{+}_u$ to which it belongs, and, moreover, to support shortest path queries, we further keep the shortest paths from proxy $u$ to all nodes in the \dra. Assume that distances are stored as a 4-byte integer. Each node is represented as a 4-byte integer. Then the extra space that we need for shortest path and distance queries is $16\cdot |V_{dra}|$, where $V_{dra}$ is the set of nodes in all \dras. We can see that space cost of proxies is small. Only about 100 MB extra space is taken for the largest graph US.


\stitle{Summary} From the observations we can see that proxy is a light-weight optimization technique, which benefits existing shortest  path and distance algorithms, such as (bidirectional) Dijkstra~\cite{LubyR89}, \arcflag \cite{MohringSSWW05}, \tnr \cite{arz2013transit} and \ah\cite{zhu2013shortest} (to be seen immediately).

\subsection{Benefit of proxies}
...
In this set of experiments, we tested the datasets in Table~1. Due to the space limitation, we only reports the results on \dblpone, CA, E-US, and C-US. A more detailed results can be found in the supplementary document. We fixed the constant $c = 2$ for computing proxies on all graphs.
...
\end{center}
\vspace{1ex}
\caption{Varying graph queries (\ah and Proxy+\ah for shortest distances)}
\hrule
...
\end{center}
\vspace{1ex}
\caption{Varying graph queries (\ah and Proxy+\ah for shortest paths)}
...
Since \dblpone is a single dataset, and hence we only compare the efficiency of a group of shortest path and distance queries $\{Q_1, Q_3, Q_5, Q_7\}$  on road graphs using the same settings.
...
\end{center}
\vspace{1ex}
\caption{Varying graph sizes (\ah and Proxy+\ah for shortest distances)}
...
\end{center}
\vspace{1ex}
\caption{Varying graph sizes (\ah and Proxy+\ah for shortest paths)}
