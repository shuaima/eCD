...
We are now ready to present algorithm \compDRAs shown in Fig.~\ref{alg-compute-proxies}, which takes as input graph $G$ and constant $c$, and outputs the \dras of $G$ and their maximal proxies.
...
\icc \>\> \Let the \dra $A^+_{v'}$ of proxy $v'$ := $X'$;\\
