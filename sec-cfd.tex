...
\vspace{-1ex}
\begin{example}
The dependencies \kw{cfd1}--\kw{cfd3} and \kw{pfd1}--\kw{pfd4}
that
we have seen in Example~\ref{exa-motivation} can all be expressed as \pCFDs.
Figure~\ref{fig-pcfd} shows some of these \pCFDs:
$\varphi_1$ (for \FD\ \kw{cfd_2}),
$\varphi_2$ (for \CFD\ \kw{cfd_3}), $\varphi_3$ (for \kw{pfd_2}),
and $\varphi_4$ (for \kw{pfd_4}).
\eop
\end{example}
\vspace{-1ex}
...
Intuitively, each pattern tuple $t_{pi}$ specifies a condition via
$t_{pi}[X]$,
