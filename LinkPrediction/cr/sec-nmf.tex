...
\]
\end{small}
We assume \kwlog that $\epsilon = 1$, then $\sqrt{ \epsilon / r} = 0.58$ and $\epsilon / r = 0.33$.
...
$S_{22} \cdot S_{32}$) and ($S_{13} \cdot S_{23}$, $S_{13} \cdot S_{33}$, $S_{23} \cdot S_{33}$)), together with 6 multiplications for inner loop checking, instead of 75 times for calculating $FF^T$ directly, which reduces
...
$(\epsilon/r)\times r= \epsilon$. %This completes the proof.
\end{IEEEproof}
