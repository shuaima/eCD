...
\marked{\begin{definition}[top-$(\epsilon, \theta, k)$ predictions]
Given a threshold $\theta$, a set $L_1$ of predicted links is a top-$(\epsilon, \theta, k)$ prediction, if 
$L_1 = \{(i, j) | (i, j) \in L_2, \overline{F_i} \cdot \overline{F_j} > \theta \} $, 
where $L_2$ is a top-$(\epsilon, k)$ prediction.
\end{definition}
\begin{equation}
...
\>\> $L[i] \leftarrow \|\overline{F_i} \|$; \\
\> {\bf end for} \\
\> {\bf for} \= each $i=1$ to $f_p$ {\bf do}\\
\>\>{\bf for} \= each $j=i+1$ to $f'_p$ {\bf do}\\
\>\>\>{\bf if} $L[R_{ip}] \cdot L[R_{jp}] \leq \theta$ {\bf continue}; \\
\>\>\>{\bf if}\ \= $S_{ip} \cdot S_{jp} < \epsilon/r$ {\bf then}\\
\>\>\>\>break inner loop; \\
\>\>\> {\bf else} increase the score of node-pair $(R_{ip}, R_{jp})$
by \\
