...
%%%%%%%%%%%%%%%%%%%%%%   2016-06-20 duan begin
\marked{\stitle{Speeding up top-$(\epsilon, k)$ predictions.} In real-life applications,
a common scenario is to find a set of top-$(\epsilon, k)$ predictions with prediction
values are greater than a given threshold $\theta$ \cite{ballard2015, lemp}. In this case,
the top-$(\epsilon, k)$ prediction could be sped up by pruning a portion of the search space
which consists of node pairs whose prediction values are less and equal than $\theta$.
The simple strategy here is that
the prediction value of a node pair $(i, j)$ is
\begin{equation}
...
\end{equation}, where $\|\overline{F_i} \|$ is the length of the row vector $\overline{F_i}$.
From Equation \ref{equation-length}, we obtain
\begin{equation}
...
\end{equation}
The prediction value is less and equal than $\theta$ if the product of length is less and equal than $\theta$.
Since our goal is to find the node pairs whose prediction values are greater than $\theta$, we can ignore
the node pairs whose length product are less and equal than $\theta$ in the search process. \\
After obtained the matrix $S$ and $R$, an additional step is required to calculate the length of each row
vector $\overline{F_i}$ and store them in an array $M$. Then insert the following if-clause: 
\vspace{-1ex}
...
\vspace{-1ex}
 into the line 3
in the nested loop to prune the node pairs whose prediction values are less and equal to $\theta$. 
...
\end{prop}
\begin{IEEEproof}
According to the Proposition 1, the nested loop finds a valid set of
top-$(\epsilon, k)$ predictions. The difference between the nested loop 
and the augmented nested loop (i.e., the top-$(\epsilon, k)$ prediction with speeding up technique) 
is that the later does not consider the node pairs $(i, j)$ 
when $\|\overline{F_i} \| \cdot \| \overline{F_j} \| \leq \theta$.
Note that the prediction values of those node pairs 
are also less and equal than $\theta$. At the end of
the execution, the augmented nested loop removes the node 
pairs with prediction values are less and equal than $\theta$.
Therefore, the augmented nested loop finds a valid set of top-$(\epsilon, k)$ predictions with
prediction values are greater than $\theta$.
\\
This completes the proof.
\end{IEEEproof} }
%%%%%%%%%%%%%%%%%%%%%%   2016-06-20 duan end
