...
\sstab (1) \Adamic \cite{adamic}: Algorithm \Aa is a popular neighborhood based method that
  produces a score for each link $(u, v)$, defined as follows:
...
\RA score for a link $(u, v)$ is defined as follows:
...
compared with bagging methods. Given one of top-$k$ link prediction methods, denoted by $x$, its performance of link prediction is evaluated with the following measure:
\begin{equation}
...
Indeed, \Biased improves the accuracy by $(2.5\%, 41.4\%, 113.4\%, 34.1\%)$, $(7.5\%, 45.2\%,$ $ 38.2\%, 46.2\%)$
