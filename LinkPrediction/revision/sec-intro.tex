...
which has obvious \marked{ {\em robustness} } advantages. By incorporating with the
...
\marked{ Note that the bagging methods combine the outputs of several predictors
...
and show that our ensemble approach for link prediction is both \marked{robust} and efficient.
Indeed, (1) on \Friendster with $15$ million nodes and $1$ billion edges, our approach could finish in
two hours, while existing methods direct \NMF, \Aa \cite{adamic}, \marked{\RA \cite{zhou2009}} 
...
more \marked{robust} and efficient results. Section~4 presents and discusses the
