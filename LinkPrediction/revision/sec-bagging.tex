...
(the previous mentioned latent factor model based method in Section~\ref{sec-NMF} requires $O(nr^2)$ time),
...
\begin{enumerate}
%\hspace{-4ex} 
\item[]At the second step of the three bagging methods, construct a reduced adjacency matrix $W_s$
...
already performs better than \NMF, 
...
\ie the minimum value is assigned to the parameter $\theta$ in Section 2.2.
...
\stitle{Remarks}. It is worth mentioning that our ensemble-enabled approach is a general framework, 
not limited to \NMF, but also applied to other link prediction methods such as \Aa and \RA, to be seen in the supplementary material. 
