...
In particular, (1) we have added an example in Section 2.1 for illustrating
the procedure of the top-$(\epsilon, k)$ prediction more clearly, (2) we have
added the Resource Allocation (RA) algorithm as a comparison method in the experimental
study in Section 4, (3) we have added an accuracy comparison with varied hop distances
of our methods and BIGCLAM in Section 4.2.4, (4) we have compared our methods
with the state-of-the-art methods when $k$ is fixed to the number of links in
the ground truth data and reported the results together with the accuracy
95\% confidence intervals in the Appendix A of the supplementary
material for the lack of space, (5) we have implemented and evaluated the
ensemble-enabled approach with Adamic/Adar (AA) and RA in the Appendix B of
the supplementary material, and (6) we have also taken this opportunity to
rewrite several parts of the paper to improve the presentation.
...
\textbf{[R2C1]} \emph{Due to the fact that the decomposition is based
...
Due to space limitations, we choose to report these experimental results in the Appendix A
of the supplementary material, in which the results justify he effectiveness of our methods compared with the state-of-the-art methods.
Thanks for the suggestion!


\textbf{[R2C2]} \emph{Authors are using top-k precision to evaluate
...
of the supplementary material, in which the results justify he effectiveness and efficiency of our methods compared with the state-of-the-art methods.
Thanks for the suggestion!
...
\textbf{[R2C3]} \emph{AA can only predict two hops links, it has no
...
\textbf{[R2C4]} \emph{Authors proposed a framework to decompose the
...
problem into smaller subproblems, which is not limited to NMF, but also applied to other link prediction methods such as AA and RA.
...
in the Appendix B of the supplementary material. The results indeed show that the
ensemble-enabled AA and RA are more effective and faster than their counterparts
AA and RA on most datasets.  Thanks for this great comment!
...
\textbf{=== LATENT FACTOR MODEL FOR LINK PREDICTION ===}
...
Yes, we have clarified these in the very beginning of Section 2.
...
Yes, there are limitations. However, as pointed out in [46], the factorized matrix without the non-negativity constraints could contain
negative entries and be dense [46], which makes it more difficult to be searched.
Another benefit of NMF is that it requires $O(nr^2)$ time to be trained, and generates non-negative and sparse factorized matrices [21], which could
be searched efficiently by our top-$(\epsilon, k)$ prediction searching method (Sections 2.1\&2.2). Thanks!
...
We do agree that the prediction accuracy can be improved if noisy ones could be correctly fixed in the input matrix.
However, social networks are very sparse, and it is hard to fix these noisy ones.
Since this is to change the input, it must be very cautious as it brings error propagation.
 Hence, we choose to fit the value 0 for absent links,
which simplifies the framework of latent factor models, and provides an
intuitive way for predicting links.
...
We have added a citation [21] to show that the factorized matrices of NMF
are usually sparse in Section 2.1. Thanks!
...
We have added an example (Example 1 in Section 2.1) to explain the procedure of the top-$(\epsilon, k)$ prediction. Thanks!
...
We have added this in the Proof of Proposition 1.
Thanks for the suggestion!


\textbf{=== STRUCTURAL BAGGING METHODS ===}
...
We have added the complexity of training of latent factor models in the very beginning of
Section 3. Thanks!
...
Please refer to our response to \textbf{[R2C4]}. We really appreciate this great comment!
...
Yes, the Flickr dataset in used to illustrate the limitation that
...
is lower. We have pointed this out in the first paragraph of Section 4.2.
This result is useful for instructing users to apply
...
We have merged Section 4.2.4 into Section 4.2.1. Thanks!
