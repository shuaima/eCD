...
the top-$(\epsilon, k)$ prediction in Section 2.1, (2) we have added a new 
comparison algorithm, \ie Resource Allocation (RA), in the experimental study, 
...
a set of test for evaluating the ensemble-enabled approach with other link 
...
We have added the recommended survey in the related work. Thanks!

[33] L. L邦, M. Medo, C. H. Yeung, Y. Zhang, and Z. Zhang. Recommender
...
Yes. We have discussed the complexities of AA and RA in Section 4.1. 
...
The edge bagging method is different with the snowball sampling because the 
edge bagging only selects a random adjacent node when the sampled node set is 
grown. Moreover, the snowball sampling has been introduced in the graph sampling 
survey [4] in our references.
...
Thanks for recommending the above works for link prediction. We have added 
the link predictability [34] in the related work. For the lack of space, 
we have cited the surveys [20][35] that introduce the other works.
...
We have added a table for illustrating the accuracy together with its 
95\% confidence intervals of our methods in Section 4.2.1. 
...
Since we focus on the top-$k$ ranking problem for link prediction, we adopt 
the top-$k$ precision for evaluation.
...
We have added an accuracy comparison in each hop distance in XXX. 
Thanks for the suggestion! 
...
We adopt NMF because of it requires only $O(nr^2)$ time rather than .
...
We have added a test for evaluating the ensemble-enabled methods with 
AA and RA in xxx. Thanks for the suggestion! 
...
The Flickr dataset in used to illustrate the limitation of the bagging 
methods that bagging methods may reduce the prediction accuracy when the pairwise 
