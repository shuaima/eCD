...
to the number of links in the ground truth data \cite{yang2015}. We fixed $r = 30$ on \Digg and \YouTube, 
$r = 20$ on \Wikipedia and fixed other parameters to their default values. 
...
\Biasedp is $(1.1, 1.1, 1.1)$ times faster than
...
In this set of tests, we evaluated the robustness and efficiency of our
methods compared with \Aa, \RA and \BIGCLAM. We fixed $k$ to the number of links 
...
The accuracy results tell us that (a) \Biased and \Biased outperform other methods on
most of the datasets, (b) the accuracy of \Biasedp is very close to that of \Biased, (c) both
\Biased and \Biasedp have a higher accuracy than \Aa, \RA and \BIGCLAM, except for \Aa and \RA on \Wikipedia,
(d) \NMF is more accurate than \Aa, \RA and \BIGCLAM, except for \Aa and \RA on \Wikipedia,
and (e) the accuracy of all methods decreases with the increase of $k$.
Indeed, \Biased improves the accuracy by $(2.1\%, 24.3\%, 70.8\%, 11.9\%)$ (resp. $(0.8\%, 34.0\%, 34.0\%, 22.9\%)$
and $(10.1\%, -16.0\%, -51.3\%, 12.9\%)$) over \NMF, \Aa, \RA and \BIGCLAM on \Digg, \YouTube and \Wikipedia,
respectively. Moreover, \Biased and \Biased perform consistently well on all networks (\ie more robust), unlike \RA which works well
on \Wikipedia but poorly on other datasets. This verifies the robustness of our bagging methods.


The running time results tell us that (a) \Biasedp is the fastest method among 
\Biased, \NMF and \BIGCLAM, (b) the two bagging methods are much faster than \NMF and \BIGCLAM,
(c) the running time of \Aa and \RA increases rapidly since their complexities are $O(nd^2\log(k))$.
Indeed, the bagging+ and bagging methods finished the prediction in 430 seconds on the three datasets.
Furthermore, \Biasedp is $(7.1, 0.2, 0.1, 1.2)$ (resp. $(9.2, 0.1, 0.1, 1.5)$ and $(13.3, 7.2, 6.0, 16.0)$)
...
\NMF & 2.86 $\pm$ (0.082)	& 1.33 $\pm$ (0.087) 	& 1.19 $\pm$ (0.047) \\
\Biased & \textbf{2.92 $\pm$ (0.020)}	& 1.34 $\pm$ (0.059)	& 1.31 $\pm$ (0.018)\\
\Biasedp & 2.88 $\pm$ (0.054)	& \textbf{1.37 $\pm$ (0.062)}	& 1.32 $\pm$ (0.022) \\
\hline \hline
\end{tabular}
\vspace{-2ex}
\end{table}
...
\Biasedp & 58.91	& 399.84	& \textbf{353.12} \\
\hline \hline
\end{tabular}
\vspace{-2ex}
\end{table}
...
\subsection{Ensemble-Enabled Approach with AA and RA}
