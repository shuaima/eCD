...
comparison \cite{yang2015}. Therefore, we conducted two sets of experiments when
$k$ is varied from hundreds to the number of links in the ground truth data to evaluate:
(1) the robustness and efficiency of our bagging+ methods vs. their counterparts
bagging methods, and (2) the robustness and efficiency of our methods
vs. \Aa, \RA and \BIGCLAM. To provide a more fair and detailed comparison,  
we report the accuracy comparison with confidence intervals of the tests 
when $k$ is the number of the ground truth links in Exp-4 and Exp-5, 
and the tests when $k$ is fixed to the default value 
in Exp-1.2, Exp-2.1 and Exp-2.2.




\subsection{Bagging+ vs. Bagging}
In the first set of tests, we evaluated the robustness and efficiency of our
bagging+ methods compared with bagging methods.

\stitle{Exp-4: Impacts of $k$}. We varied $k$ from hundreds
to the number of links in the ground truth data \cite{yang2015}.
We fixed $r = 30$ on \Digg and \YouTube, $r = 20$ on \Wikipedia
...
It is means that the bagging+ methods maintain the high accuracy while some of the
...
\begin{figure*}[tb!]
  ...
  %\vspace{-2ex}
  % Requires \usepackage{graphicx}
  \subfigure[Digg]{\label{fig_exp_1_2_k_acc_digg}\includegraphics[width= 1.8in, height=1.2in]{eps-script/1-2-a.eps} }
  \quad\quad
  \subfigure[YouTube]{\label{fig_exp_1_2_k_acc_youtube}\includegraphics[width= 1.8in, height=1.2in]{eps-script/1-2-b.eps} }
  \quad\quad
  \subfigure[Wikipedia]{\label{fig_exp_1_2_k_acc_wikipedia}\includegraphics[width= 1.8in, height=1.2in]{eps-script/1-2-c.eps} }
  \subfigure[Digg]{\label{fig_exp_1_2_k_time_digg}\includegraphics[width= 1.8in, height=1.2in]{eps-script/1-2-e.eps} }
  \quad\quad
  \subfigure[YouTube]{\label{fig_exp_1_2_k_time_youtube}\includegraphics[width= 1.8in, height=1.2in]{eps-script/1-2-f.eps} }
  \quad\quad
  \subfigure[Wikipedia]{\label{fig_exp_1_2_k_time_wikipedia}\includegraphics[width= 1.8in, height=1.2in]{eps-script/1-2-g.eps}}
  \vspace{-1ex}
  \caption{Accuracy and efficiency comparison: with respect to the number $k$ of predicted links.}\label{fig_exp_1_2}
  \vspace{-2ex}
\end{figure*}
...
In the second set of tests, we evaluated the robustness and efficiency of
...
\stitle{Exp-5: Impacts of $k$}. We varied $k$ from hundreds
to the number of links in the ground truth data \cite{yang2015},
fixed $r = 30$ on \Digg and \YouTube, $r = 20$ on \Wikipedia
and other parameters to their default values.
The results of accuracy and running time are reported
in Figures \ref{fig_exp_1_2_k_acc_digg}--\ref{fig_exp_1_2_k_acc_wikipedia} and
Figures \ref{fig_exp_1_2_k_time_digg}--\ref{fig_exp_1_2_k_time_wikipedia}, respectively.
...
This verifies the robustness of our methods.
...
prediction in 421 seconds on the three datasets. Furthermore, \Biasedp is
$(7.5, 1.2)$ (\resp $(8.8, 1.5)$ and $(11.4, 16.0)$) times faster than \NMF and 
\BIGCLAM on \Digg, \YouTube and \Wikipedia, respectively.
...
\begin{table*}
\caption{Accuracy (\%) comparison with 95\% confidence intervals in different network sizes.}
...
\hline
\multirow{5}{*}{\Digg} & $3 \times 10^4 $ & 3.39 & 2.84 & 3.87 $\pm$ (0.240) & 4.40 $\pm$ (0.095) & 4.07 $\pm$ (0.095) & 4.00 $\pm$ (0.069) \\
...
\hline
\multirow{5}{*}{\YouTube} & $3 \times 10^5 $ & 1.53 & 1.22 & 1.43 $\pm$ (0.052) & 1.87 $\pm$ (0.402) & 2.01 $\pm$ (0.058) & 1.99 $\pm$ (0.091)  \\
...
\hline
\multirow{5}{*}{\Wikipedia} & $3 \times 10^5 $ & 1.71 & 2.48 & 1.01 $\pm$ (0.029) & 2.13 $\pm$ (0.065) & 2.12 $\pm$ (0.024) & 2.10 $\pm$ (0.014)  \\
...
To provide a more fair and detailed comparison of accuracy, we 
...
The interval gives an indication of how much uncertainty there is in 
the estimate of the accuracy. The narrower the interval, the more precise is the estimate. 
The accuracy comparison when $k$ is the number of links in the ground truth data in Exp-4 and Exp-5
and when $k$ is fixed to its default value in Exp-1.2 and Exp-2.1 are shown in Table \ref{tab_accuracy_1}. 
In addition, we also report the accuracy comparison among our methods, 
\Aa, \RA and \BIGCLAM in different network sizes when $k$ is fixed to the default value 
in Exp-2.2, shown in Table  \ref{tab_accuracy_2}.


The results tell us that (a) \Biased and \Biasedp are more accurate than other methods 
on most of the datasets, except \Aa and \RA on \Wikipedia, 
(b) bagging+ and bagging methods perform better than other methods on most of the datasets, 
and (c) the accuracy of each method when $k$ is fixed to the default value 
is higher than that when $k$ is the number of links in the ground truth data. 
...
verify the robustness of our methods. Note that the confidence intervals of all methods 
are narrow, which means that the accuracy estimate is reasonable. 
...
\stitle{Exp-6: Impacts of $k$}. We varied $k$ from hundreds to the number of links in the ground truth data
...
is small. This is verifies the robustness of the ensemble-enabled approach.
...
The speedup of \AABiased and \AABiasedp (\resp \RABiased and \RABiasedp) are (5, 4) and (14, 5)
(\resp (7, 5) and (20, 6)) on the large networks (\Twitter and \Friendster), which verifies the efficiency
