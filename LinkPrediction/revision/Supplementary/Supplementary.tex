...
comparison \cite{yang2015}. Therefore, we have conducted three sets of experiments when
...
\Aa, \RA and \BIGCLAM. This is useful for choosing $k$ appropriately.
...
We used networks with different sizes that used in Exp-1.3,
...
in Section 4.2.3, we keep the default values for these parameters and 
report the impacts of the parameter $r$ on
...
\sstab (1) The bagging+ and bagging methods are more accurate than other 
...
methods have significant improvements when $k$ is small, and also accurate than 
others when $k$ is the number of links in the ground truth data.
...
number of links in the ground truth data. In this case, however, it also provide an efficiency 
advantage. Similar trends have been found for \Biasedp.

\sstab (3) The accuracy of all methods decrease with the increase of $k$, and 
...
Although our methods are more accurate than other methods under this condition, we still 
keep the default value for $k$ since the higher accuracy would be more useful in 
practice.
...
We report the average of accuracy for comparison in previous tests, 
but it is would be more fair and detailed with providing the accuracy confidence 
intervals in comparison. Therefore, we
report the 95\% confidence intervals of accuracy of our
methods and \BIGCLAM in this section. 
...
\sstab (1) \Biased and \Biasedp are more accurate than other methods
on most of the datasets. Even when $k$ is the number of the ground truth links, \Biased improves
the accuracy by $(2.8\%, 24.3\%, 70.8\%, 11.9\%)$ (\resp $(1.5\%, 34.0\%, 34.0\%, 22.9\%)$)
over \NMF, \Aa, \RA and \BIGCLAM on \Digg and \YouTube, respectively. 
Moreover, Bagging+ and bagging methods perform better than other methods on most of the datasets. 
...
\sstab (1) The ensemble-enabled \Aa and \RA improve the accuracy 
compared with their counterparts \Aa and \RA on some networks, but 
they are not robust on all datasets. This means that it is difficult 
to directly apply our ensemble-enabled framework to any link prediction 
methods. 

\sstab (2) The ensemble-enabled \Aa and \RA would provide an efficiency 
advantage when the average degree of each ensemble component is small, 
\eg on \YouTube and \Wikipedia. However, our sampling techniques tend to 
sampling more dense subgraphs and reduce the efficiency of ensemble-enabled 
approach with \Aa and \RA. To provide a general framework, the graph sampling techniques used
in our methods should be augmented.
