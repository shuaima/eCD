...
(1) the effectiveness and efficiency of bagging methods with the top-$(\epsilon, k)$ speeding up,
node selection and edge filtration techniques (denoted as bagging+ methods)
vs. their counterparts bagging methods with only node selection technique \cite{liang2016},
...
Note that we set $k = 1\times 10^5$ for \Digg and \YouTube and $k =1\times 10^6$ for
other datasets.}
\label{tab_parameter}
\vspace{-2ex}
\centering
...
\marked{ \Node with top-$(\epsilon, k)$ speeding up and edge filtration (\Nodep),
\Edge with top-$(\epsilon, k)$ speeding up and edge filtration (\Edgep) and
\Biased with top-$(\epsilon, k)$ speeding up and edge filtration (\Biasedp)}
