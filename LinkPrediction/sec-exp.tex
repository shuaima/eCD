...
(1) the effectiveness and efficiency of the bagging methods with the top-$(\epsilon, k)$ speeding up, 
node sampling and edge sampling techniques (denoted as bagging+ methods)
vs. their counterparts bagging methods with only the node sampling technique \cite{liang2016},
(2) the effectiveness and efficiency of our approach vs. conventional methods \Aa \cite{adamic} and \BIGCLAM \cite{yang-wsdm2013},
(3) the impacts of various factors and (4) the improvements of the bagging+ methods.}
...
(1) \Digg, \YouTube, \Wikipedia and \Flickr contain timestamps of edge arrivals.
...
\begin{figure*}[tb!]
  ...
  \quad\quad
  \subfigure[Wikipedia]{\label{fig_exp_1_1_k_time_wikipedia}\includegraphics[width= 1.8in]{eps-script/1-1-g.eps}}
  \vspace{-2ex}
...
\begin{figure*}[tb!]
  ...
  \quad\quad
  \subfigure[Wikipedia]{\label{fig_exp_1_2_size_time_wikipedia}\includegraphics[width= 1.8in]{eps-script/1-2-g.eps}}
  \subfigure[Twitter]{\label{fig_exp_1_2_size_time_twitter}\includegraphics[width= 1.8in]{eps-script/1-2-i.eps}}
  \quad\quad
  \subfigure[Friendster]{\label{fig_exp_1_2_size_time_friendster}\includegraphics[width= 1.8in]{eps-script/1-2-j.eps}}
  \vspace{-2ex}
...
\marked{ \Node with the top-$(\epsilon, k)$ speeding up, node and edge sampling techniques (\Nodep),
\Edge with the top-$(\epsilon, k)$ speeding up, node and edge sampling techniques (\Edgep) and
\Biased with the top-$(\epsilon, k)$ speeding up, node and edge sampling techniques (\Biasedp)}
...
(See Exp-3.4 for more details about the setting of $r$). For the limitation of space, 
we only report the results of the bagging+ methods with the edge 
sampling incorporating with PA characteristic.
...
\subsubsection{Bagging plus vs. Bagging}
...
\stitle{Exp-1.1: Impacts of $k$}. To evaluate the impacts of the number $k$ of
...
(b) the accuracy of the bagging+ methods are close to the
accuracy of their counterparts bagging methods, and (c) the accuracy of all
...
\Biasedp is $(1.17, 1.09, 1.16)$ times faster than
...
(resp. $r = 30$ for \BIGCLAM and $r = 15$ for the bagging methods). The results
...
and Figure \ref{fig_exp_1_2_size_time_digg}, \ref{fig_exp_1_2_size_time_youtube}, 
\ref{fig_exp_1_2_size_time_wikipedia}, \ref{fig_exp_1_2_size_time_twitter} and
...
$1.31$ (resp. $1.35$) times on \Twitter (resp. \Friendster) and is thus essential for
...
\begin{figure*}[tb!]
  ...
  \quad\quad
  \subfigure[Wikipedia]{\label{fig_exp_2_1_k_time_wikipedia}\includegraphics[width= 1.8in]{eps-script/2-1-g.eps}}
  \vspace{-2ex}
...
\begin{figure*}[tb!]
  ...
  \quad\quad
  \subfigure[Wikipedia]{\label{fig_exp_2_2_size_time_wikipedia}\includegraphics[width= 1.8in]{eps-script/2-2-g.eps}}
  \subfigure[Twitter]{\label{fig_exp_2_2_size_time_twitter}\includegraphics[width= 1.8in]{eps-script/2-2-i.eps}}
  \quad\quad
  \subfigure[Friendster]{\label{fig_exp_2_2_size_time_friendster}\includegraphics[width= 1.8in]{eps-script/2-2-j.eps}}
  \vspace{-2ex}
...
\stitle{Exp-2.1: Impacts of $k$}. Using the same setting as Exp-1.1, we evaluated
...
In addition, \NMF is slower than \Aa on four datasets because $r$ is fixed to at least $50$,
...
\stitle{Exp-2.2: Impacts of network sizes}. Using the same setting as Exp-1.2, we
...
and Figure \ref{fig_exp_2_2_size_time_digg}, \ref{fig_exp_2_2_size_time_youtube},
\ref{fig_exp_2_2_size_time_wikipedia}, \ref{fig_exp_2_2_size_time_twitter} and
...
The running time results tell us that (a) our bagging methods are much faster than other methods,
...
that \NMF is slower than \BIGCLAM on \Digg, \YouTube and \Friendster,
...
the accuracy and running time of bagging methods, bagging+ methods, \NMF 
...
The results tell us that (a) bagging and bagging+ methods more accurate than
...
\begin{figure*}[tb!]
  ...
  \quad\quad
  \subfigure[Wikipedia]{\label{fig_exp_3_mu_time_wikipedia}\includegraphics[width=1.8in]{eps-script/3-1-g.eps}}
  \vspace{-2ex}
...
than \NMF when $f$, (b) the accuracy of bagging methods decreases
with the increase of $f$ when $f$ is greater than 0.1, and (c) the running time of bagging methods increases
...
\begin{figure*}[tb!]
  ...
  \quad\quad
  \subfigure[Wikipedia]{\label{fig_exp_3_f_time_wikipedia}\includegraphics[width= 1.8in]{eps-script/3-2-g.eps}}
  \vspace{-2ex}
...
\stitle{Exp-3.3: Impacts of $s$}. To evaluate the impacts of $s$ for
...
\begin{figure*}[tb!]
  ...
  \quad\quad
  \subfigure[Wikipedia]{\label{fig_exp_3_s_time_wikipedia}\includegraphics[width= 1.8in]{eps-script/3-3-g.eps}}
  \vspace{-2ex}
...
The results tell us that (a) \Biased and \Biasedp obtain the best accuracy 
...
\begin{figure*}[tb!]
  ...
  \quad\quad
  \subfigure[Wikipedia]{\label{fig_exp_3_r_time_wikipedia}\includegraphics[width= 1.8in]{eps-script/3-4-g.eps}}
  \vspace{-2ex}
...
\begin{figure*}[tb!]
  ...
  \quad\quad
  \subfigure[Wikipedia]{\label{fig_exp_3_e_time_wikipedia}\includegraphics[width= 1.8in]{eps-script/3-5-g.eps}}
  \vspace{-2ex}
...
In the fourth set of tests, we evaluate the improvements of bagging+ methods.
Compared with their counterparts bagging methods, the bagging+ methods have two modifications. One
is the top-$(\epsilon, k)$ speeding up technique (denoted as top-$(\epsilon, k)$+) 
and the other is the edge sampling technique.
...
(b) the edge sampling technique also speeds up the bagging methods, 
...
Note that \Biased is slightly slower than \Nodep on \YouTube due to 
the top-$(\epsilon, k)$+ performs better with \Nodep. 
Furthermore, the improvement of Top-$k$+ on \Wikipedia is not
distinct since the $\epsilon = 1$ is large enough on this dataset.
...
and Biased Edge Bagging) with edge sampling. Top-$(\epsilon, k)$+ is 
the top-$(\epsilon, k)$ speeding up technique.}
...
Biased+ & Top-$(\epsilon, k)$+  & \textbf{55.32}    & \textbf{38.26} & \textbf{117.25}  \\
\hline \hline
\end{tabular}
\end{table}
...
than \NMF in some situations. We first report the results on \Flickr which show that 
our bagging methods obtain lower accuracy than \NMF. Furthermore, 
we explain the reasons which could be useful
...
be achieved by using different ensemble components or different parameters for the ensemble base
algorithms. Since the base algorithm and its parameters were fixed in our experiments, we checked
the diversity of ensembles generated by our bagging methods. We use the pairwise 
overlapping of predicted links and nodes (i.e., $N_s$ in each bagging methods) 
...
methods with the top-$(\epsilon, k)$ speeding up edge sampling techniques, 
...
for around $(1.31, 12, 135, 53)$ (resp. $(1.35, 15, 8, 65)$) times on \Twitter (resp. \Friendster).
