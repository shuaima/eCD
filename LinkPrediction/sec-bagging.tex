...
\marked{\stitle{Edge sampling}. Inspired by graph sparsification \cite{chen2015, satuluri2011} which has been applied on clustering without sacrificing quality, we consider two edge sampling strategies for selecting edges from the subgraph
induced on $N_s$ of $G$. \\
(1) dx + dy. in order to reduce the loss of common neighbors. \\
(2) min(dx, dy). PA }
...
For each time, an ensemble component $N_s$ is sampled by one of the above node, edge and biased edge bagging methods,
 the symmetric \NMF method in Section \ref{sec-NMF} is used
to the reduced matrix $W_s$, and the aforementioned  pruning search process in Section \ref{sec-NMF-topk} is used to determine the predictions of all pairs of nodes. If a node pair appears in multiple ensemble components and has multiple predicted values, the maximum predicted value is considered.
