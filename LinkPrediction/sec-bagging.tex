...
our bagging methods should be designed in particular for link prediction.
We develop a special graph sampling method consisting of two steps:
node selection and edge filtration, which takes positive and negative strategies
into consideration. The positive strategy here is to select the nodes that are more
likely to form new links as much as possible and the negative strategy is to
filter out some edges which do not affect the prediction accuracy. }

\stitle{Node selection}. Motivated by the observation that most of all new links in social networks span within very short distances, typically closing triangles, which has been justified in \cite{leskovec-2008}, we develop three structural bagging approaches such that {\em a node is always sampled together
with all its neighbors, which guarantees the possibility of forming triangles}. To achieve this, we revise the previous bagging methods as follows.
\vspace{-1ex}
\begin{enumerate}
%\vspace{-0.5ex}
...
\marked{\stitle{Edge filtration}. Inspired by previous research on graph sparsification \cite{chen2015, satuluri2011}
which has been successfully applied on clustering without sacrificing quality,
we adopt an edge filtration strategy for
selecting a portion of edges rather than all of edges from the subgraph induced on $N_s$ of $G$. With less edges,
each ensemble can be processed faster, which is significant for improving the efficiency of the bagging methods.
The challenge here is how to remove edges as much as possible while maintaining high prediction accuracy.
To overcome this challenge, we filter out edges from the induced subgraph incorporating with the following link
prediction characteristics.
\\
Preferential attachment (PA) \cite{albert1999,leskovec-2008} is one of the well-known
link prediction characteristics, which postulates the probability
that a new link is connected to the node $i$ is proportional to the degree of $i$.
It means that the smaller degree nodes would have fewer links in the future.
Therefore, we would like to remove the edges containing at least one smaller degree node.
...
descending order and only select the top $ms$ edges, where $m$ is the number of edges in the input graph.
\\
...
their common neighbors. \cite{chen2015} provided a method for filtering out edges in order to minimize the
loss of common neighbors. This method could be adopted in our bagging methods by sorting edges by $d_i + d_j$
in a descending order and then select the top $ms$ edges.}
...
prediction values, the maximum prediction value is considered, \marked{which means that we can use the
