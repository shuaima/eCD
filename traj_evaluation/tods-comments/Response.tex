...
Second, we have extended dataset UCar from 200 to 1000 trajectories such that it has more than {100M} points in total (a data point has tens of bytes, including object id, latitude, longitude, height, time and other information). Besides, we have already used the full datasets of Geolife and Mopsi, which are widely used in many recent trajectory compression studies, \eg Geolife is used in the recent algorithm algorithm CISED [Lin:CISED] and experimental study [Zhang:Evaluation], and Mopsi is used in [Lin:CISED].  %Thus, an even larger datasets is quite not friendly to the tests.
...
\emph{ [2] COMPRESS: A Comprehensive Framework of Trajectory Compression in Road Networks. Yunheng Han, Weiwei Sun, Baihua Zheng. TODS 42, 2, 11:1¨C11:49, 2017.}
