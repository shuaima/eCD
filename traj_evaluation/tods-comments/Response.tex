...
The paper has been substantially revised according to the comments of you, Dr. Seeger and referees. In particular, we have added{
	...
	(e) an introduction to map-matching and semantic-based trajectory compression methods in appendix.}
Note that map-matching is still not considered as a preprocessing step, the reasons of that please refer to {[R1C3]}.
...
We have added a brief introduction to map-matching-based trajectory compression techniques and the relationship between them and line simplification in the appendix. However, the map-matching-based techniques are not experimentally tested in this evaluation, that is, we focus on line simplification and do not consider map-matching as the pre-process, for some serious considerations:

(1) First of all, \emph{any original trajectory is naturally a sequence of piece-wise line segments as well as a sequence of data points}, no matter they are along the road network or not. And from the point of views of data management and data analysis, saving a trajectory as a sequence of line segments or data points (either original or simplified) is a lighter, more convenient and more efficient way than saving the map-matched data. Indeed, \emph{map-matching is not necessary the pre-process of trajectory simplification.} 
...
(3) In urban having road network, of cause, the data points could be mapped to road networks. However, this is not necessary a work must be done. For example, in some server-side applications like moving object monitoring, it just shows the piece-wise line trajectory on a map without map-matching in the consideration of efficiency. Indeed, \emph{map-matching is more likely a task parallel to trajectory simplification.}
...
(4) Let map-matching be the pre-step of trajectory simplification will require extra information (road network) and extra heavy computation (map-matching), and introduce extra error and running time, which make it hard to compare trajectory simplification and/or compression algorithms in a fair circumstance.
...
In the revised version, we have extended the scope of the paper and allowed insert new data points. Weak versions of algorithms SIPED, OPERB and CISED have been tested in Section 6, and sentences in Page 6 (line 19) have also been revised to meet the new scope. Thank you!
...
\textcolor{blue}{It is worth pointing out that, due to the high time complexity of batch algorithms, \ie $O(n^2)$ of both Douglas-Peucker and Theo-Pavlidis, they need \textcolor{red}{days} to run the tests listed in Section 6. }
...
Yes, we have clarified this in the last paragraph of Section 6.1.
...
Indeed, this statement refers to the compression ratios of algorithms using DAD. We have clarified this in the text.
...
Yes, it seems that data size does not have apparent and regular impacts on compression ratios.
We have clarified this in Section 6.3.1 (the 2{nd} item).
...
We have clarified this in the penultimate paragraph of Section 6.3.1. Thanks!
...
We have fixed this bug. Thanks for pointing out this!
...
Yes, we have added tests \wrt~\emph{when\_at} query in Section 6.3.5. Thanks for your advice!
...
Yes, an algorithm using PED/DAD is not able to guarantee the error bounds of spatio-temporal queries, which makes it inappropriate in scenarios that spatio-temporal queries are required. We have highlighted this in the penultimate paragraph of Section 6.3.6. Thanks!
...
{We have introduced the road network based methods in the Appendix: Semantic-based Trajectory Compression Algorithms. Thanks!} 
...
As we know, \emph{error bounds} and \emph{trajectories} are the basic two inputs of trajectory simplification algorithms solving the ``min-\#" problem, thus, a comprehensive evaluation should test the impacts of them on the effectiveness and efficiency of those algorithms.  
%
...
Yes, we have moved some comparison with [41] to the motivation part. Thanks for your advice!
...
%We re-implement the methods with the same language, Java, for fair comparison. Because no new findings are revealed yet, 
...
Also note that, in this revision, we tend to treat LISSED as a special SED-based error measure or a methodology of quickly computing errors, rather than a new kind of distance metric.
...
We have summarized the new, and also conflicting, findings compared with [41] at the end of Section 6.3.6.
