...
However, if PED is used, then the answer to a spatio-temporal query is not error bounded, \ie PED is not query friendly. Though DAD is not discussed in [r4] (DAD is developed after the publish of [r4]), it's obviouly not spatio-temporal query friendly too. 

(2) Yes, it important to consider applications on the simplified trajectories. Thus, we have evaluated line simplification algorithms and distance metrics with respect to spatio-temporal queries \emph{where\_at} and \emph{when\_at}, two fundamental block of spatio-temporal queries. Note [r4] have proved that ``the errors of the \emph{intersect} and \emph{nearest\_neighbor} query types are bounded if and only if the error of \emph{where\_at} is bounded", which indicates that the test of \emph{where\_at} query is a key to test spatio-temporal queries. And \emph{when\_at} is another important functional block.

(3) \todo{criteria of trajectory compression.}
    -- from the views of simplification
    -- applications
    --

[r4] Cao, W., and Li, Y. Dots: An online and near-optimal trajectory simplification algorithm. Journal of Systems and
Software 126(Supplement C) (2017), 34–44.
