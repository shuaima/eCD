...
(1) \emph{The error bounded line simplification algorithm using every distance metric no doubt has its specific usages}. For SED, authors in [r4] have proved that the SED-simplified trajectory is answer-error-bound for spatio-temporal queries (\emph{query friendly} in short) of \emph{where\_at, intersect} and \emph{nearest\_neighbor}, \ie if the simplification is SED error bounded by $\epsilon$, then the answer to such query is also error bounded by $\epsilon$.
Besides, the above example, ``in order to find out the witness of a fatal car accident, police might want to locate all the drivers who pass by a specific location within a given time window by querying the trajectories", is indeed a case of spatio-tempporal \emph{intersect} query (also called a spatio-temporal \emph{range} query) [r4], defined in $intersect$ ($T$, \emph{Poly}, $t_1$, $t_2$), that returns \emph{true} if the trajectory $T$ intersects the polygon \emph{Poly} between the times $t_1$ and $t_2$. Obviously, it's supported by SED.

For PED and DAD, authors in [r4] have also proved that PED is not spatio-temporal query friendly, and PED and SED neither is friendly to the ``when\_at" query. Though DAD is not discussed in [r4] (DAD is developed after the publish of [r4]), it's obviously not spatio-temporal query friendly too. Our tests in Section 6.3.5 also support these statements. Nevertheless, \emph{the error bounded line simplification using PED or DAD is often the pre-processing step of trajectory clustering that are the base of trajectory applications like traffic pattern recognition and urban plan, where rather the shape of a trajectory than the exact positions of the individual are concerned}. 
