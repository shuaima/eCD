...
To clarify this, we have revised the descriptions of distance metrics PED, SED and DAD, and also highlighted their limitations, in the part of ``quality criteria and distance metrics", in Section 1. Thanks!
...
(3) {Yes, it is important to explain the criteria of trajectory compression to offer the audience a complete image.} Thus, we have added ``Quality criteria" in Section 1. Indeed, when we talk about the queliaty criteria of trajectory simplification, there are two aspects, (a) the first one is from the point of view of trajectory simplification itself, including compression ratio, efficiency and errors of simplification. The recent works in this area all test their works by a part or all of these criteria, 
and (b) the second one is from the points of views of trajectory applications, such as spatio-temporal queries, trajectory clustering, and so on. Indeed, every trajectoty application is appliable to evaluate the simplified trajectories from its point of view, such as spatio-temporal queries concern the answer-errors and usabilities, a trajectory clustering method concerns the similarity between the clusters of original and simplified trajectories, and so on.
%
In this paper, we follow the mainstream works in this area, and test the compression ratios, efficiency and errors of simplification algorithms. For trajectory application, we choose spatio-temporal queries as the presentative as they are well-known and commonly used. Besides, as mentioned in Section 5, data aging is a potential challenge of trajectory management, thus, the safety and friendliness of line simplification algorithms and distance metrics are tested.
%
Also note that compression ratio is not the only requirement as mentioned above, and we argue that some applications may not such care about error bounds as applications are diverse.
