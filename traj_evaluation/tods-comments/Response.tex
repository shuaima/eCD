...
(b) the second one is from trajectory applications, \eg spatio-temporal queries, trajectory clustering, and so on. Indeed, every trajectory application is applicable to evaluate the simplified trajectories from its point of view, such as spatio-temporal queries concern the answer-errors and a trajectory clustering method concerns the similarity between the clusters of original and simplified trajectories.
%
In this paper, we follow the mainstream works in this area, and test the compression ratios, efficiency and errors of simplification algorithms. For trajectory application, we choose spatio-temporal queries as the representatives as they are well-known and commonly used in the management of trajectories. Besides, as mentioned in Section 5, the data aging is a potential challenge to the management of trajectory, thus, the aging safety and aging friendliness of line simplification algorithms and distance metrics are also tested.
%
We believe these are also the key consideration when we compress trajectories. Besides, compression ratio is not the only requirement as mentioned above, and it is sure necessary to support certain applications with error bounds, \eg spatio-temporal queries of \emph{where\_at}, \emph{intersect} and \emph{nearest\_neighbor}.
