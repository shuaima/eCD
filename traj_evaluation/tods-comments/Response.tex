...
As shown below, if we connect $P_0$ and $P_5$, then the \kw{PED} from $P_4$ to line segment $\overline{P_0P_5}$, \ie $|P_4P'_4|$, is a bit larger than the error bound $\epsilon$ (exact two frames in the figure). Thus,  $P_0$ and $P_5$ are not reachable, and there should be no link between them. Thanks for your seriousness and meticulousness!
...
It is worth pointing out that, due to the high time complexity of batch algorithms, \ie $O(n^2)$ of both Douglas-Peucker and Theo-Pavlidis, they need \textcolor{red}{weeks} to run the tests listed in Section 6. 
