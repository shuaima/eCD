...
(3) {Yes, it is important to explain the criteria of trajectory compression to offer the audience a complete image.} Thus, we have added a part of content named ``Quality criteria" in Section 1. Indeed, when we talk about the quality criteria of trajectory simplification, there are two aspects, (a) the first one is from the point of view of trajectory simplification itself, including compression ratio, efficiency and errors of simplification. The recent works in this area all test their works by a part or all of these criteria, 
and (b) the second one is from the points of views of trajectory applications, such as spatio-temporal queries, trajectory clustering, and so on. Indeed, every trajectory application is applicable to evaluate the simplified trajectories from its point of view, such as spatio-temporal queries concern the answer-errors, a trajectory clustering method concerns the similarity between the clusters of original and simplified trajectories, and so on.
%
In this paper, we follow the mainstream works in this area, and test the compression ratios, efficiency and errors of simplification algorithms. For trajectory application, we choose spatio-temporal queries as the representative as they are well-known and commonly used. Besides, as mentioned in Section 5, data aging is a potential challenge of trajectory management, thus, the safety and friendliness of line simplification algorithms and distance metrics are tested.
%
Also note that compression ratio is not the only requirement as mentioned above, and it is sure necessary to support certain applications with error bounds, \eg spatio-temporal queries like \emph{where\_at}, \emph{intersect} and \emph{nearest\_neighbor}.
