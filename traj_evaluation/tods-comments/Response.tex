...
All advices from you and referees have been fully adopted except that map-matching is still not considered as a pre-processing step of trajectory simplification, the reasons please refer to {[R1C3]}.
...
We have added two ``weak simplification" algorithms (OPERB-A and CISED-W) that allow insert new data points into the output trajectories. Please refer to the second paragraph of Section 1, the last paragraph of Section 2 and Section 6 for more details.
Thanks!
...
We observe that there is a kind of trajectory compression called road-aware trajectory compression [r1, r2] that either first maps a trajectory onto roads then simplifies those mapped data points by \emph{numerical streams} method [r3], such as PRESS [r1], or first simplifies the trajectory by some line simplification method then maps the result onto roads, such as the dilution-matching-encoding method [r2]. In the first case, only numerical streams method is needed, and in the second case, line simplification is the pre-step of, and hence independence of, map-matching. 

(5) Finally, let map-matching be the pre-step of trajectory simplification will require extra information (road network) and extra heavy computation (map-matching), and introduce extra error (mismatching) and running time, which make it hard to compare trajectory compression algorithms in a fair condition.
...
We have extended the scope of the paper and allowed insert new data points (the related algorithms are called weak simplification), and weak simplification algorithms OPERB-A and CISED-W have been tested in Section 6. Besides, sentences in Section 1 (second paragraph, related to comment C1), Section 2 (last paragraph) and Section 6.1 (penultimate paragraph) have also been revised to meet the new scope. Thank you!
...
Yes, each distance metric has its advantages and limitations. To clarify this, we have revisited the descriptions of them and explained their characters in the part of ``quality criteria and distance metrics", in Section 1.
...
Following the mainstream works in this area, the quality criteria here are \emph{compression ratio, efficiency} and \emph{errors (including simplification errors and query errors)}, where query errors are calculated from the applications of \emph{where\_at} and \emph{when\_at} queries.
...
We have replaced UCar with a larger dataset which has 1000 trajectories and more than {100M} points. Besides, we had already used the full datasets of Geolife and Mopsi, which have 182 and 51 trajectories, respectively.
{It is also worth pointing out that, due to the relatively high time complexities of optimal, batch and online algorithms, \eg $O(n^2)$ of both Douglas-Peucker and Theo-Pavlidis, we already need {days} to run the whole tests listed in Section 6.}
...
Actually, we specifically refer to the compression ratios of algorithms \emph{using DAD} (as shown in these figures, algorithms using DAD has completely different phenomenons compared with PED and SED). We have fixed this misleading in the second paragraph of Section 6.3.1. Thanks for pointing out these!
...
It seems that data size does not have noteworthy impacts on compression ratios.
