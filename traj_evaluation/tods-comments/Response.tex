...
The paper has been substantially revised according to the comments of you, Dr. Seeger and referees. In particular, we have added{
	...
	(c) experimental studies on query friendliness from the view of ``when\_at" query, 
	(d) a larger data set of UCar in the experimental studies, and
	(e) a brief introduction to map-matching-based trajectory compression methods and its relation to line simplification in appendix.}
Note that map-matching is still not considered as a pre-processing step of trajectory simplification, the reasons of that please refer to {[R1C3]}.
...
We have explained this in Section 1 (the second paragraph). 
Namely, the \emph{$min-\#$ problem} of trajectory simplification is, given a curve (\ie an original trajectory) and an $\epsilon$, to construct an approximate curve with error within $\epsilon$ and, at the same time, having the minimum number of line segments.
...
We have added ``weak simplification" algorithms (\eg CISED-W) that allow insert new data points into the output trajectories. Please refer to the second paragraph of Section 1, the last paragraph of Section 2 and Section 6 for more details.
Thanks!
...
We have added a brief introduction to map-matching-based trajectory compression techniques and the relationship between them and line simplification in the appendix. However, the map-matching-based techniques are still not experimentally tested in this evaluation, that is, we focus on line simplification and do not consider map-matching as the pre-process. This decision has some serious considerations:

(1) First of all, \emph{any original trajectory is naturally a sequence of piece-wise line segments as well as a sequence of data points}, no matter they are along the road network or not. And saving a trajectory as a sequence of line segments or data points (either original or simplified) is a lighter, more convenient and efficient way than saving the map-matched data. Indeed, \emph{map-matching is not necessary the pre-process of trajectory simplification.} 
...
(2) Moving objects often move in areas not on roads or without fine road network, \eg human being, wild animals, bikes, air plane may move on ground, at gardens, in countryside and in the atmosphere. In these cases, \emph{map-matching should not be the pre-step of trajectory simplification.}

(3) In urban having road network, of cause, the data points could be mapped to road networks, however, \emph{this is not necessary a work must be done}. For example, in some server-side applications like moving object monitoring, it just needs to show trajectories on a map without map-matching in the consideration of efficiency. 

(4) \emph{Map-matching is more likely a task parallel to trajectory simplification,} and neither is necessary the pre-step of the other.
%Besides, map-matching is most likely an application-level task called by application such as navigation. As we known, trajectory simplification as a fundamental functionality should (better) not depend on these high-level applications. 
We also know that there is a kind of trajectory compression called road-aware trajectory compression [r1, r2] that either first maps a trajectory to road network then simplifies those mapped data points by \emph{numerical streams} method[r3], such as PRESS [r1], or first simplifies the trajectory by some line simplification method then maps the result to roads, such as the dilution-matching-encoding method [r2]. In the first case, there is no line simplification, and in the second case, line simplification is the pre-step of, and hence independence of, map-matching. 

(5) Let map-matching be the pre-step of trajectory simplification will require extra information (road network) and extra heavy computation (map-matching), and introduce extra error (mismatching) and running time, which make it hard to compare trajectory compression algorithms in a fair condition.
...
We have extended the scope of the paper and allowed insert new data points. Weak versions of algorithms OPERB and CISED have been tested in Section 6. Besides, sentences in Section 1 (second paragraph), Section 2 (last paragraph) and Section 6.1 (penultimate paragraph) have also been revised to meet the new scope. Thank you!
...
Yes, each distance metric has its advantages and limitations. To clarify this, we have revisited the descriptions of them and explained their limitations in the part of ``quality criteria and distance metrics", in Section 1.
...
We have explained the distance metrics and quality criteria of trajectory compression in the part of ``quality criteria and distance metrics" in Section 1. 
The quality criteria here are \emph{compression ratio, efficiency} and \emph{errors (including simplification errors and query errors)}, where query errors are calculated from the applications of \emph{where\_at} and \emph{when\_at} queries.
Thanks for pointing out this.

\textbf{[R1C7]} \emph{ The definition of Error Bounded Algorithms is not precise as the error bounds are not presented. }

We have revised the definition in the penultimate paragraph of Section 2. Thanks!
...
Anyway, thanks for your seriousness and meticulousness!
...
We have replaced UCar with a larger dataset which has 1000 trajectories and \textcolor{red}{1000M} points (please see Table 4). Besides, we had already used the full datasets of Geolife and Mopsi, which have 182 and 51 trajectories, respectively.
\textcolor{blue}{It is worth pointing out that, due to the high time complexity of batch algorithms, \ie $O(n^2)$ of both Douglas-Peucker and Theo-Pavlidis, they need \textcolor{red}{days} to run the whole tests listed in Section 6. }
...
Indeed, when calculate the average errors, we do consider ALL the points in the original trajectories. We have revised the description of ``average errors" in Section 6.2 to avoid the misleading. Thanks for points out this!
...
It is a sub-trajectory of 10 points (say p1, p2, ..., p10). We have clarified this in the last paragraph of Section 6.1.
...
This statement is misleading. Actually, we specifically refer to the compression ratios of algorithms \emph{using DAD} (as shown in these figures, algorithms using DAD has completely different phenomenons compared with PED and SED). We have fixed this bug in the second paragraph of Section 6.3.1. Thanks for pointing out these!
...
Yes, it seems that data size does not have noteworthy impacts on compression ratios.
We have clarified this in the third paragraph of Section 6.3.1 (the 2{nd} item).
...
We have clarified this in the penultimate paragraph of Section 6.3.1 (also refer to the discusses in ``Quality criteria and distance metrics" of Section 1). Thanks!
...
{Yes, in comparing with line simplification, we have added a brief introduction to the semantic-based methods in the Appendix: Semantic-based Trajectory Compression Algorithms. Thanks!} 
...
We re-implement the methods with the same language, Java, for a fair comparison. However, no new finding is revealed or discussed in this point of view.
...
(1) \underline{``If a user considers data aging is important, then he/she has no choice but to use DP"}: \emph{Data aging} is rather an important requirement derived from the management of trajectory data, and it is not identical to \emph{aging friendly}. Indeed, when we talk about data aging, it is not necessary to choose the \emph{aging friendly} algorithm (DP) to compress trajectories --- though given the same error bounds, it may bring better compression ratios. That is to say, any algorithm could be used in data aging.
...
(3) reported our findings about DOTS and LISSED in Section 6.
Also note that, in this revision, we tend to treat LISSED as a special SED-based error measure or a methodology that efficiently calculate errors, rather than a new kind of distance metric.
...
We have summarized the new as well as conflicting findings compared with [41] at the end of Section 6.3.6.
