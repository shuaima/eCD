...
%
However, we have notice that (1) [41] only has a limited evaluation on running time. It does show a table that lists \emph{compression time per trajectory point} of each algorithm, but it is not sufficient or reasonable, as the impacts of both the sizes of trajectories and the error bounds is ignored by [41], and (2) [41] only has a limited evaluation on compression ratios. It does report the relationship between compression ratios and errors (not error bounds),  but the most important aspect in practice, the impacts of the error bounds on the compression ratios, is ignored. Thus, it does not fully test the compression ratios of algorithm and it is hard to guide users to set reasonable error bounds.
%
...
To clarify this, we have revised \todo.
