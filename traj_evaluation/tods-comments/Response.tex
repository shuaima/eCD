...
Besides, an explaination on ``why map-matching is not considered as a pre-processing step of trajectory simplification" is provided in {[R1C3]}.
...
(1) We have correct the statements in the begning of the paper (the second paragraph of Section 1), where ``(whose corresponding data points are a subset of the original one)" is replaced by ``(if all data points of the coase curve are a subset of the original curve, then it's a strong simplification; otherwise, it's a weak simplificatoin)".

(2) Now, this paper has covered both strong and weak simplifications, more specifically, we have tested and analyzed two ``weak simplification" algorithms (OPERB-A and CISED-W) in Section 6. 
...
We strong believe that \emph{map-matching is a task parallel to line simplification}, and should not be the pre-processing step of the later. The reasons are as follows:

(1) Trajectory simplification is better performed as ealy as possible, \ie it's better to run trajectory simplification on mobile devides that collect GPS points, such that not only storages of data servers but also network bandwith between mobile devices and data servers is saved. 
...
(2) Line representation is a simple and natural way to repreent a trajectory, and we can freely match an original or simplified trajectory to road network at anytime, which means map-matching is not necessary the pre-processing step of trajectory simplification too. We have also observed that there is a kind of trajectory compression called road-aware trajectory compression [r1, r2] that is used to compress trajectories that capture the moving objects' movement along the road network. However, it has at least two kind of works, the first one maps a trajectory onto roads then simplifies those mapped data points by \emph{numerical streams} method [r3], such as PRESS [r1] does, and the second one first simplifies the trajectory by some line simplification method, then maps the result onto roads, such as the dilution-matching-encoding method [r2] does. In the first case, map-matching is the pre-step of compression, but in the second case, line simplification is the pre-step of map-matching. 

(3) Moving objects often move from city to wild, or move in areas outside urban where no fine or up-to-date road network is available. In these cases, if we let map-matching be the pre-step of trajectory simplification, it will introduce errors caused by mis-map-matching. However, if we save the line representations of these trajectories (simplified), then once the road network is available or updated (this can happen many times), they can also be matched onto roads when we need. Here, map-matching is better not the pre-step of trajectory simplification.
