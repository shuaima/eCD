...
For PED and DAD, authors in [r3] have also proved that PED is not spatio-temporal query friendly, and PED and SED neither is friendly to the ``when\_at" query. Though DAD is not discussed in [r3] (DAD is developed after the publish of [r3]), it's obviously not spatio-temporal query friendly too. Our tests in Section 6.3.5 also support these statements. Nevertheless, \emph{the error bounded line simplification using PED or DAD has it usages}, \eg it's often the pre-processing step of trajectory clustering that is the base of applications like urban plan and traffic pattern recognition [r4], where rather the shape of a trajectory than the exact positions of the individual are concerned. 
