...
Cao et al. proved in \cite{Cao:Spatio} that ``the top-down algorithm \dpa is aging friendly \wrt~\ped and \sed" on the premise that the second run of algorithm \dpa takes as input the whole simplified trajectory produced by the first run. 
...
\todo{why dp using ped and sed is aging friendly? dp using dad has different performance}

\begin{theorem}
	\label{theo-aging-dp-dad}
	The top-down algorithm \dpa is not aging friendly \wrt~\dad, and given error bounds $\epsilon_1>0$ and $\epsilon_2>0$, for distance metric $\mathcal{D}$ of \dad, the error bound between original trajectory $\dddot{\mathcal{T}}$ and simplified trajectory $\overline{\mathcal{T}}=DP(DP(\dddot{\mathcal{T}}, \epsilon_1, \mathcal{D}), \epsilon_2, \mathcal{D})$ is $\epsilon_1 + \epsilon_2$.
\end{theorem}
