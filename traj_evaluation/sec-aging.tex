...
%\todo{From the above we can find that only algorithms \dpa using \ped and \sed are aging friendly, others are not. Indeed, {aging friendly} is the result of the {specific nature}, \ie batch and top-down, of algorithm \dpa.}
% pretty and rare feature of

\subsection{Error of Data Aging}
We then discuss the error between the simplified trajectory $\overline{\mathcal{T}}_2$ and the original trajectory $\dddot{\mathcal{T}_0}$.
...
\begin{proof} 	
	...
	If $\mathcal{A}$ is a top-down algorithm using \dad or a bottom-up algorithm using any distance metric, then from Figures~\ref{fig:aging-dp-dad}-(2) and Figure~\ref{fig:aging-pavlidis}-(2) we can find that the error bound between original trajectory $\dddot{\mathcal{T}}$ and simplified trajectory $\overline{\mathcal{T}}=\mathcal{A}(\mathcal{A}(\dddot{\mathcal{T}}, \epsilon_1, \mathcal{D}), \epsilon_2, \mathcal{D})$ is not less than $\epsilon_1+ \epsilon_2$.
