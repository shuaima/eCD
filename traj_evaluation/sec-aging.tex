...
%
This section is to answer these questions from the views of friendliness \cite{Cao:Spatio} and compression error.
...
Cao et al. also proved that ``the top-down algorithm \dpa is aging friendly \wrt~\ped and \sed\cite{Cao:Spatio}," on the premise that the second run of algorithm \dpa takes as input the whole simplified trajectory produced by the first run.%, including the same start and end points of trajectories. 

%\cite{Cao:Spatio} did not discuss \dad, 

\begin{theorem}
	\label{theo-aging-tp-dad}
...
\begin{theorem}
	...
	Given error bounds $\epsilon_1>0$ and $\epsilon_2>0$, for distance metric $\mathcal{D}$ of \dad, the error bound between original trajectory $\dddot{\mathcal{T}}$ and simplified trajectory $\overline{\mathcal{T}}=DP(DP(\dddot{\mathcal{T}}, \epsilon_1, \mathcal{D}), \epsilon_2, \mathcal{D})$ is $\epsilon_1 + \epsilon_2$.
\end{theorem}
