...
We first discuss the relationship between $\overline{\mathcal{T}}_1$ and $\overline{\mathcal{T}}_2$ from the view of friendliness, which was defined in \cite{Cao:Spatio}, but was seldomly discussed later.
	
\stitle{Aging friendly \cite{Cao:Spatio}}. {A \lsa algorithm $\mathcal{A}$ is aging friendly with respect to a distance metric $\mathcal{M}$ if for every $\epsilon_1$ and every $\epsilon_2$ such that $\epsilon_1 < \epsilon_2$, and for every trajectory $\dddot{\mathcal{T}}$, $\mathcal{A}(\dddot{\mathcal{T}}, \epsilon_2, \mathcal{M})= \mathcal{A}(\mathcal{A}(\dddot{\mathcal{T}}, \epsilon_1, \mathcal{M}), \epsilon_2, \mathcal{M})$.}
...
\begin{table}
	...
	\begin{tabular}{|l|c|c|c|c|c|c|}
		\hline
		{\bf{Algorithms}} &\bf{Distance Metrics} &\multicolumn{2}{|c|}{\bf{Aging Friendliness and Proofs}} &\multicolumn{2}{|c|}{\bf{Error Bounds and Proofs}}  \\		
		\hline

		{\multirow{2}*{{\opt} algorithms}} &\ped or \sed	& \hspace{2ex} $\times$ \hspace{2ex} &\cite{Cao:Spatio} & $\epsilon_1 + \epsilon_2$   & Proposition \ref{theo-aging-distance}	\\
		\cline{2-6}
		&\dad	& $\times$  	& Proposition \ref{theo-aging-opt-dad}	& $\epsilon_1 + \epsilon_2$    &Proposition \ref{theo-aging-distance}\\
		\hline
		
		{\multirow{2}*{Batch algorithm \dpa}} &\ped or \sed	&\checkmark  & \cite{Cao:Spatio} & $\max\{\epsilon_1, \epsilon_2\}$   &Proposition \ref{theo-aging-error-dp}\\
		\cline{2-6}
		&\dad	& $\times$  	& Proposition \ref{theo-aging-dp-dad} & $\epsilon_1 + \epsilon_2$   & Proposition \ref{theo-aging-distance}\\
		\hline
		
		{Batch algorithm \tpa}	& \ped, \sed or \dad & $\times$  & Proposition \ref{theo-aging-tp}& $\epsilon_1 + \epsilon_2$  & Proposition \ref{theo-aging-distance}  \\
		\hline
		
		{Online algorithms}	& \ped, \sed or \dad & $\times$  & Proposition \ref{theo-aging-online}& $\epsilon_1 + \epsilon_2$  & Proposition \ref{theo-aging-distance}  \\
		\hline

		{One-pass algorithms}	& \ped, \sed or \dad & $\times$  & Proposition \ref{theo-aging-online}& $\epsilon_1 + \epsilon_2$  & Proposition \ref{theo-aging-distance}  \\
...
\bi	
	...
	\item Assume that it is true for every trajectory \trajec{T} having $n$, $(n \ge 3)$, points.
