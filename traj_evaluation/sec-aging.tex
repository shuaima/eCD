...
\begin{figure}[tb!]
	\centering
	\includegraphics[scale=0.60]{Figures/Fig-aging-pavlidis.png}

	\caption{\small Counter example of the aging friendliness of algorithm \tpa, where (1) the original trajectory is compressed using $\epsilon_2=5$ to three points $\{P_0, P_2, P_3\}$; (2) the original trajectory is first compressed using $\epsilon_1=3$ to three points $\{P_0, P_2, P_3\}$, then compressed using $\epsilon_2=5$ to two points $\{P_0, P_3\}$. }
	\vspace{-2ex}
...
Let error bounds $\epsilon_1 =3$ and $\epsilon_2=5$. 
...
Only algorithm \dpa is aging friendly, others are not. Indeed, it is the result of the specific algorithm nature of \dpa.
