...
In the rest of the section, we will fully discuss this problem, start from algorithm \opt coupling with \dad. The major results are summarized in \mytable{tab:summary-data-aging}.
...
\begin{proof}
	...
	For $\epsilon_1 > \epsilon_2$, we will first prove $DP(DP(\dddot{\mathcal{T}}, \epsilon_1, \mathcal{M}), \epsilon_2, \mathcal{M}) = DP(\dddot{\mathcal{T}}, \epsilon_1, \mathcal{M})$ by total induction on the number of points of \trajec{T}.
