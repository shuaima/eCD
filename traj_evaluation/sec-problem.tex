...
%The optimal \lsa algorithms have relative high time/space complexities.
...
\todo{There are also some trajectory simplification algorithms that do not directly enforce threshold on a distance, instead, they accumulate the distances of points to a line segment and enforce a threshold on the accumulated value. For example, algorithms MRPA and DOTS use ISSD which is an accumulated value of \ped. We do not evaluate these ISSD based algorithms because the comparison between ISSD and the above distances seems meaningless.}
...
\begin{table}
	...
	\begin{tabular}{|l|c|c|c|c|c|c|c|}
		\hline
		\kw{Name}  & \kw{Type}      &\kw{\ped} &\kw{\sed}  &\kw{\dad} &  \kw{Time} & \kw{Space} \\		\hline
		\opt~\cite{Imai:Optimal}	&optimal		&Y & Y & Y & $O(n^3)$	& {$O(n^2)$} \\		\hline
		\optp\cite{Chan:Optimal}	&optimal		&Y & N & N & $O(n^2)$	& {$O(n)$} \\		\hline
		\dpa\cite{Douglas:Peucker, Meratnia:Spatiotemporal}	&batch  &Y &Y & Y   & $O(n^2)$ & O(n)  \\		\hline
		\tpa\cite{Pavlidis:Segment}	&batch       &Y &Y & Y  & $O(n^2/K)$ & O(n)  \\		\hline
		\bqsa\cite{Liu:BQS}	&online	   &Y   & N & N & $O(n^2)$  & $O(|Q|)$   \\		\hline
		\squishe\cite{Muckell:Compression}\hspace{-1ex}	&{online}	  & N &Y  & N  & \hspace{-1ex}$O(n\log|Q|)$\hspace{-1ex} & $O(|Q|)$ \\		\hline
		\operb\cite{Lin:Operb}	& one-pass	  &Y & N & N & $O(n)$ & O(1)  \\		\hline
		\siped\cite{Dunham:Cone, Zhao:Sleeve}\hspace{-1ex}	&one-pass	  &Y & N & N & $O(n)$ & O(1) \\		\hline
		\cised\cite{Lin:Cised}	&one-pass	 	&N & Y & N & $O(n)$ & O(1) \\		\hline
		\interval\cite{Ke:Interval}	&one-pass	 	&N & N & Y & $O(n)$ & O(1) \\		\hline
	\end{tabular}
