...
\textcolor{blue}{Instead of directly using \sed, \cite{Chen:Fast} introduces Local Integral Square \sed (\lissed) satisfying $lissed(P_s,P_{s+k}) = \sum_{i=s}^{s+k-1} sed^2(P_i, \vv{P_sP_{s+k}})$, which can be computed in an incremental way, \ie given information about $lissed(P_s,P_{s+k})$, $lissed(P_s,P_{s+k+1})$ can be computed in $O(1)$ time, such that algorithms using it achieve lower time complexities. Also note that (1) if the \lissed error bound of such an algorithm (\eg~\bumr~\cite{Chen:Fast} or \dagots~\cite{Cao:Dots}) is set to $\epsilon^2$, then the maximal \sed error of it is not more than $\epsilon$, and (2) \lissed brings very different simplified trajectories compared with \sed.}
%, and (3) the most recent algorithm using \sed, \ie~\cised, is also efficient, having a time complexity of $O(n)$.
