...
Now we can define the Min-$\#$ problem of trajectory simplification.
...
\textcolor{blue}{The simplified trajectories are stored in data stores. As time goes by, the less precision of them may be necessary\cite{Cao:Spatio}. Thus, they need to be simplified again to get more coarse trajectories as time progresses. Suppose we first compressed a trajectory $\overline{\mathcal{T}_0}$ using an \lsa algorithm $\mathcal{A}$ with error bound $\epsilon_1$ to $\overline{\mathcal{T}}_1$, then we need a coarser trajectory produced from $\overline{\mathcal{T}}_1$  having error bound $\epsilon_1 + \epsilon_2$ corresponding to $\overline{\mathcal{T}}_0$, can we get that by compressing $\overline{\mathcal{T}}_1$ with error bound $\epsilon_2$?  \emph{Aging feasible} and \emph{aging friendly \cite{Cao:Spatio}} are concepts to answer this question. Note that \emph{aging friendly} was defined in \cite{Cao:Spatio} which also discussed the aging friendliness of algorithms \opt and \dpa with respect to distance metrics \ped and \sed, while \emph{aging feasible} is a new concept.}

\stitle{Aging feasible}. \textcolor{blue}{A \lsa algorithm $\mathcal{A}$ is aging feasible with respect to a distance metric $D$ if for every $\epsilon_1$ and every $\epsilon_2$, and for every trajectory $\dddot{\mathcal{T}}$, the max error between $\mathcal{A}(\mathcal{A}(\dddot{\mathcal{T}}, \epsilon_1, D), \epsilon_2, D)$ and $\dddot{\mathcal{T}}$ is not more than $\epsilon_1+ \epsilon_2$.}
...
Obviously, if a \lsa algorithm $\mathcal{A}$ is aging friendly, then it must be aging feasible.
