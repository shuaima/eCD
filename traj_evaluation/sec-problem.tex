...
Note that here we \textcolor{blue}{do not} require that for all $i\in[0,m]$, points $P'_{s_i}$ and $P'_{e_i}$ of $\overline{\mathcal{T}}$ belong to the original trajectory \trajec{T}. That is to say, \sout{all points in the simplified trajectory belong to the original trajectory.} \textcolor{blue}{we allow data interpolation.}
