...
\textcolor{blue}{Instead of directly using \sed, authors of \cite{Chen:Fast} introduce Local Integral Square \sed (\lissed) satisfying $lissed(P_s,P_{s+k}) = \sum_{i=s}^{s+k-1} sed^2(P_i, \vv{P_sP_{s+k}})$, which can be computed in an incremental way, \ie given information about $lissed(P_s,P_{s+k})$, $lissed(P_s,P_{s+k+1})$ can be computed in $O(1)$ time, such that algorithms using it (\eg~\bumr~\cite{Chen:Fast} and \dagots~\cite{Cao:Dots}) achieve lower time complexities compared with directly using \sed. It is worth pointing out that (1) if the \lissed error bound of such an algorithm  is set to $\epsilon^2$, then the maximal \sed error of it is always not more than $\epsilon$, and (2) the most recent algorithm using \sed, \ie~\cised, is also efficient, having a time complexity of $O(n)$.}
