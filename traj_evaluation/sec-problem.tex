...
Notations used are summarized in Table \mytable{tab:notations}.
...
Note that (1) each directed line segment in $\overline{\mathcal{T}}$ essentially represents a continuous sequence of data points in trajectory $\dddot{\mathcal{T}}$, \textcolor{blue}{and (2) a point ($x$, $y$) on a spherical surface is projected on a flat plane in the piece-wise line representation.}
