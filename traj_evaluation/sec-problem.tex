\section{Trajectory Simplification Problem}
In this section, we introduce basic concepts and statement the problem of piece-wise line based trajectory simplification.
...
This paper evaluates the optimal and sub-optimal methods using \ped and \sed.
...
\begin{table*}
	\vspace{1ex}
	\caption{\small Summary of line simplification algorithms. }
	\label{tab:summary-lsa}
	\centering
	\small
	\begin{tabular}{|l|c|c|c|c|c|c|c|}
		\hline
		\kw{Short}  & \kw{Key~Ideas}    & \kw{Type}  &\multicolumn{2}{|c|}{\kw{Error~Metrics}} &\multicolumn{2}{|c|}{ \kw{Time~Complexity}} & \kw{Space} \\		\cline{4-7}
		\kw{Name}   & \kw{}    			&            &\kw{\ped} &\kw{\sed} 		& \kw{Best}& \kw{Worst}		& \kw{Complexity}\\		\hline\hline
		DP~\cite{Douglas:Peucker, Meratnia:Spatiotemporal}	&Top-down   &batch  &Y &Y   & $\Omega(n)$ & $O(n^2)$ & O(n)  \\		\hline
		TP~\cite{Pavlidis:Segment}	&Bottom-up   &batch    &Y &Y   & $\Omega(n\log n)$ & $O(n^2/K)$ & O(n)  \\		\hline
		BQS~\cite{Liu:BQS}	&Top-down and Convex hull &online   &Y   & N &$\Omega(n)$ & $O(n^2)$  & $O(|Q|)$   \\		\hline
		\squishe~\cite{Muckell:SQUISH}	& Bottom-up and Priority queue   &{online}    & N &Y   & $\Omega(n\log|Q|)$ & $O(n\log|Q|)$ & $O(|Q|)$ \\		\hline
		...
		\operb~\cite{Lin:Operb}	& Fitting function  & one-pass  &Y & N &   $\Omega(n)$ & $O(n)$ & O(1)  \\		\hline
		\sia~\cite{Williams:Longest,Sklansky:Cone,Dunham:Cone, Zhao:Sleeve}	& Sector      &one-pass   &Y & N &   $\Omega(n)$ & $O(n)$ & O(1) \\		\hline
		\cia~\cite{Lin:Cised}	& Spatio-temporal Cone      &one-pass   &N & Y &   $\Omega(n)$ & $O(n)$ & O(1) \\		\hline
		OptimalPED~\cite{Chan:Optimal}	& Graph and Sector  &optimal   &Y & N &   $\Omega(n)$ & $O(n^2)$ & O(n) \\		\hline
		OptimalSED~	& Graph and Spatio-temporal Cone      &optimal   &N & Y &   $\Omega(n\log n)$ & $O(n^2 \log n)$ & O(n) \\		\hline
	\end{tabular}
