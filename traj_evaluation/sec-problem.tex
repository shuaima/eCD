...
\textcolor{blue}{The simplified trajectories are stored in data stores. As time goes by, the less precision of them may be necessary\cite{Cao:Spatio}. Thus, they need to be simplified again to get more coarse trajectories as time progresses. Suppose we first compressed a trajectory $\overline{\mathcal{T}_0}$ using an \lsa algorithm $\mathcal{A}$ with error bound $\epsilon_1$ to $\overline{\mathcal{T}}_1$, then we need a coarser trajectory produced from $\overline{\mathcal{T}}_1$  having error bound $\epsilon_2$ corresponding to $\overline{\mathcal{T}}_0$, can we get that by compressing $\overline{\mathcal{T}}_1$ with error bound $\epsilon_2$? Authors in \cite{Cao:Spatio} introduced a concept of \emph{aging friendly} to answer this question, and discussed the aging friendliness of algorithms \opt and \dpa with respect to \ped and \sed.}

\stitle{Aging friendly \cite{Cao:Spatio}}. \textcolor{blue}{A simplification algorithm $\mathcal{A}$ is aging friendly with respect to a distance metric $D$ if for every $\epsilon_1$ and every $\epsilon_2$ such that $\epsilon_1 < \epsilon_2$, and for every trajectory $\dddot{\mathcal{T}}$, $\mathcal{A}(\dddot{\mathcal{T}}, \epsilon_2, D)= \mathcal{A}(\mathcal{A}(\dddot{\mathcal{T}}, \epsilon_1, D), \epsilon_2, D)$}.
