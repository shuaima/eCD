...
%
These algorithms, some are \ped specific (\eg ~\optp, \bqsa and \operb), some are \sed specific (\eg~\squishe and \cised) and some are \dad specific (\eg~\interval), depending on the optimization techniques they applied.

\todo{There are also some trajectory simplification algorithms that do not directly enforce threshold on a distance, instead, they accumulate the distances of points to a line segment and enforce a threshold on the accumulated value. For example, algorithms MRPA and DOTS use ISSD which is an accumulated value of \ped. We do not evaluate these ISSD based algorithms because the comparison of compression ratios between ISSD and the above distances seems meaningless.}
