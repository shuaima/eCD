...
A large number of lossy trajectory compression techniques have been developed, typically the piece-wise line simplification (or approximation) \cite{Douglas:Peucker, Hershberger:Speeding, Keogh:online,Liu:BQS, Muckell:Compression, Chen:Trajectory, Chen:Fast, Cao:Spatio, Shi:Survey}.
...
This work focus on the \lsa methods of trajectory compression. 
More specifically, in trajectory compression, the \lsa algorithms commonly use two distance metrics, \ie the \emph{perpendicular Euclidean distances} (\ped) and the \emph{synchronous Euclidean distances} (\sed).
...
The \textit{optimal} methods are to find the minimal number of points or segments to represent the original polygonal lines \wrt an error bound $\epsilon$.
