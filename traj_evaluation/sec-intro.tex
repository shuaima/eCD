...
\textcolor{blue}{Besides, the simplified trajectories are stored in data stores. As time goes by, the less precision of them may be necessary \cite{Cao:Spatio}. Thus, they need to be simplified again to get more coarse trajectories as time progresses. Yet, can we compress simplified trajectories and get trajectories having limited error \wrt the original trajectories by certain \lsa algorithm? This is the \emph{data aging} problem, initially introduced, and partially answered, in \cite{Cao:Spatio}. However, it remains an open question for \lsa algorithms other than \opt and \dpa.}
...
\stab (2) \textcolor{blue}{We comprehensively evaluate the data aging problem of \lsa algorithms and distance metrics from the views of aging friendliness and aging error. We proved that only algorithms running in both batch and top-down manners are aging friendly, others are not; and for aging error, the batch and top-down algorithms also have unique performance compared with other algorithms.}
