...
Hence, \textit{sub-optimal} \lsa algorithms have been developed and/or introduced for trajectory compression, and they achieve better efficiencies at an expense of outputting a little more data points. By the applied distance checking policies, sub-optimal algorithms can further be classified into
batch, online and one-pass algorithms.

\sstab (1) {\em Batch algorithms} such as Douglas-Peucker\cite{Douglas:Peucker,Meratnia:Spatiotemporal} and \pavlidis~\cite{Pavlidis:Segment} apply global distance checking policies such that all trajectory points need to be loaded before starting compression, and a point may be checked multiple times to compute its distance to the corresponding line segments.
...
\begin{table*}
	...
	\begin{tabular}{|l|c|c|c|c|c|c|c|c}
		\hline
		\kw{Name}  & \kw{Type}      &\kw{\ped} &\kw{\sed}  &\kw{\dad} &  \kw{Time} & \kw{Space} & \kw{Key~Ideas} \\		\hline
		\opt~\cite{Imai:Optimal}				&optimal	&Y & Y & Y & $O(n^3)$	& {$O(n^2)$}  & Reachability Graph \\		\hline
        \optp\cite{Chan:Optimal}             	&optimal	&Y & N & N & $O(n^2)$	& {$O(n^2)$}  & Reachability Graph and Sector intersection  \\		\hline
%		\optp\cite{Chan:Optimal}				&optimal	&Y & N & N & $O(n^2)$	& {$O(n)$}  &Graph and Sector intersection  \\		\hline
        \kw{SP} \cite{Long:Direction}           &optimal	&N & N & Y & $O(n^2)$	& {$O(n^2)$}  & {Reachability Graph and Direction range intersection}  \\		\hline
        {\kw{Ramer} \cite{Ramer:Split}}			&batch  	&Y &Y & Y   & $O(n^2)$ & $O(n)$   & Top-down \\		\hline
		...
		\kw{SWAB} \cite{Keogh:online} 	        &online  	&Y &Y  &Y   	& $O(n*|Q|)$	& $O(|Q|)$  &Bottom-up and Sliding window	\\		\hline
        ...
        \rwa \cite{Reumann:Strip}               & one-pass	&Y & N & N 		& $O(n)$ 	& $O(1)$   & Strip  \\		\hline
...
\begin{table*}
	...
	\begin{tabular}{|c|c|l|c|c|c|c|c|c|c|c}
		\hline
		\multicolumn{2}{|c|}{\kw{Category}} &\kw{Algorithms} &\kw{\ped} &\kw{\sed}  &\kw{\dad} &  \kw{Time} & \kw{Space} &\kw{Rep} & \kw{Key~Ideas} \\		
        \hline
        \multicolumn{2}{|c|}{\multirow{3}*{\kw{optimal}}} &\opt~\cite{Imai:Optimal}					&\checkmark  & \checkmark & \checkmark & $O(n^3)$	& {$O(n^2)$} & \checkmark & Reachability Graph \\		
        \cline{3-10}
        ...
        \multirow{14}*{\rotatebox{90}{\kw{sub}-\kw{optimal}}}
        &\multirow{3}*{\rotatebox{90}{\kw{batch}}}  &{\kw{Ramer} \cite{Ramer:Split}}		&\checkmark &\checkmark & \checkmark   & $O(n^2)$ & $O(n)$  & & Top-down \\		
        \cline{3-10}
        ...
        &\multirow{4}*{\rotatebox{90}{\kw{online}}}	&\opwa \cite{Meratnia:Spatiotemporal} 	&\checkmark &\checkmark  &\checkmark   	& $O(n^2)$	& $O(n)$  &\checkmark &Top-down and Opening window	\\		
        \cline{3-10}
		& &\bqsa\cite{Liu:BQS}					&\checkmark &$\times$ &$\times$ 		& $O(n^2)$  & $O(|Q|)$   &\checkmark &{Top-down, Opening window and Convex hull} \\		
        \cline{3-10}
		& &\kw{SWAB} \cite{Keogh:online} 	        &\checkmark &\checkmark  &\checkmark   	& $O(n*|Q|)$	& $O(|Q|)$  & &Bottom-up and Sliding window	\\		

        \cline{3-10}
	    & &\squishe\cite{Muckell:Compression}		&$\times$ &\checkmark  &$\times$  	& $O(n\log|Q|)$ & $O(|Q|)$  &\checkmark &Bottom-up and Priority queue \\		
        \cline{2-10}

        &\multirow{4}*{\rotatebox{90}{\kw{one}-\kw{pass}~~~ }}&\rwa \cite{Reumann:Strip}              &\checkmark &$\times$ &$\times$ 		& $O(n)$ 	& $O(1)$  & & Strip  \\		
        \cline{3-10}
...
%
\mytable{tab:summary-lsa} summarizes the algorithms, which consist of optimal and suboptimal algorithms, and the later is further classified into batch, online and one-pass algorithms.
