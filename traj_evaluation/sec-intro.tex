...
\sstab (1) {\em Batch algorithms} such as Douglas-Peucker (\dpa) \cite{Douglas:Peucker,Meratnia:Spatiotemporal} and \pavlidis (\tpa)~\cite{Pavlidis:Segment} apply global distance checking policies such that all trajectory points need to be loaded before starting compression, and a point may be checked multiple times to compute its distance to the corresponding line segments.
...
\begin{table*}
	...
	\begin{tabular}{|c|c|l|c|c|c|c|c|c|c|c}
        ...
        \hline
	\end{tabular}
	{\\  \vspace{0.5ex}Here ``Rep" means ``Representative", $K$ is the number of the final segments of a trajectory and $|Q|$ is the size of a buffer/window.}
	\vspace{-2ex}
\end{table*}
...
\textcolor{blue}{Besides, the simplified trajectories are stored in data stores. As time goes by, the less precision of them may be necessary \cite{Cao:Spatio}. Thus, they need to be simplified again to get more coarse trajectories as time progresses. Yet, can we compress simplified trajectories and get trajectories having limited error \wrt the original trajectories by certain \lsa algorithm? This is the so called \emph{data aging} problem, initially introduced, and partially answered, in \cite{Cao:Spatio}. However, it remains an open question for \lsa algorithms other than \opt and \dpa.}
...
\stab (2) \textcolor{red}{data aging}
