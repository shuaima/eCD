...
\stitle{\textcolor{blue}{Quality criteria and} distance metrics}. 
...
\textcolor{blue}{Obviously, given the same error bound, algorithms using \sed will output more data points than \ped as they also save temporal information.}
%
\textcolor{blue}{Also note that, in order to speed up the simplification, a few algorithms \cite{Chen:Fast, Wu:Graph,Cao:Dots} alternatively use an accumulation of square \sed, named the integral square synchronous Euclidean distance (\kw{ISSED}) \cite{Chen:Fast}, as the error bound. \cite{Wu:Graph} follows the ideas of \cite{Chen:Fast} and Dots \cite{Cao:Dots} is an online algorithm that uses \kw{ISSED}. These \kw{ISSED}-equipped algorithms ensure that any \emph{accumulation of square \sed(s)} from data points to their representing line segment is limited by a preset \kw{ISSED} error bound, which has quite different effectiveness compared with \sed.}
...
\begin{table*}
	...
	\begin{tabular}{|c|c|l|c|c|c|c|c|c|c|c}
        ...
        \cline{3-10}
        \multicolumn{2}{|c|}{}& \textcolor{blue}{\optss\cite{Chen:Fast} } 		&$\times$ & \sout\checkmark & $\times$ & $O(n^2)$	& {$O(n^2)$} & & {reachability graph \& \lissed} \\		
        \cline{3-10}
	    ...
	    \cline{3-10}
	    & &\textcolor{blue}{\bumr\cite{Chen:Fast}}		&$\times$ &\sout{\checkmark} &$\times$  	& $O(n)$ & { $O(n)$}  & & \lissed \& bottom-up\\	
		\cline{2-10}
        ...
        \cline{3-10}
		& &\textcolor{blue}{\dagots\cite{Cao:Dots}}		&$\times$ &\sout{\checkmark} &$\times$  	& $O(n^2/K)$ & { $O(|Q|^2)$}  &\checkmark &\lissed \& incremental DAG\\	
		
		\cline{3-10}
		& &\textcolor{blue}{\olts\cite{Wu:Graph}}		&$\times$ &\sout{\checkmark} &$\times$  	& $O(n^2/K)$ & { $O(|Q|^2)$}  & &\lissed \& incremental DAG\\		
		\cline{2-10}
        ...
        \hline
	\end{tabular}
	{\\ \vspace{2ex} Here, (1) `Rep" means ``Representative" \textcolor{blue}{(in each category, we comprehensively consider the effectiveness and the supporting of distance metrics of algorithms, as well as the novelty of their key ideas)},  $K$ is the number of the final segments of a trajectory and $|Q|$ is the size of a buffer/window, \textcolor{blue}{(2) \optss, \bumr, \dagots and \olts actually use Local Integral Square \sed (\lissed) as error measure instead of using \sed directly}, and (3) batch and top-down algorithms \dpa\cite{Douglas:Peucker, Meratnia:Spatiotemporal} and \kw{Ramer} \cite{Ramer:Split} are aging friendly, while other algorithms are not.   }
	\vspace{-2ex}
\end{table*}
...
Essentially, this study is a necessary complement to existing studies by providing a systematic evaluation and analyses of error bounded trajectory simplification algorithms. \sout{More specifically,
...
\textcolor{blue}{In comparison with \cite{Zhang:Evaluation},
(1) compression ratio analyses are systematicly considered,
(2) variations of distance metrics are studied, 
(3) optimal algorithms using \ped and \sed and one-pass algorithms \siped and \cised are investigated, and
(4) data aging of \lsa algorithms is studied.}
\sout{Indeed, algorithm \siped is {\em completely overlooked} by existing trajectory compression studies as it is originally developed in fields of computational geometry and pattern recognition, and we recently find that it can be easily adopted for trajectory compression with good performance.} %\cite{Lin:Cised}.
