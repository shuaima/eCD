...
\begin{table*}
	...
	\begin{tabular}{|c|c|l|c|c|c|c|c|c|c|c}
        ...
        \cline{3-10}
        \multicolumn{2}{|c|}{}& \textcolor{blue}{\optss\cite{Chen:Fast} } 		&$\times$ & \sout\checkmark & $\times$ & $O(n^2)$	& {$O(n^2)$} & & {reachability graph \& integral square SED} \\		
        \cline{3-10}
        ...
        \cline{3-10}
		& &\textcolor{blue}{\dagots\cite{Cao:Dots}}		&$\times$ &\sout{\checkmark} &$\times$  	& $O(n^2/K)$ & { $O(n)$}  &\checkmark &integral square SED \& incremental DAG\\	
		
		\cline{3-10}
		& &\textcolor{blue}{\olts\cite{Wu:Graph}}		&$\times$ &\sout{\checkmark} &$\times$  	& $O(n^2/K)$ & { $O(n)$}  & &integral square SED \& incremental DAG\\		
		\cline{2-10}
