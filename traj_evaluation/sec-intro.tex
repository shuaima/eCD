...
%
{Apart from the widely using of GPS sensors, the sampling rate is also increasing, \eg from 1 point per minute to currently 1 point per second, in order to get more accurate position information of a moving object, which leads to an increasing size of trajectory data stored in and transmit by mobile devices.}
%
...
%
This work focus on the \lsa methods of trajectory compression, solving the \emph{min-$\#$} problem \cite{Chan:Optimal, Imai:Optimal,Pavlidis:Segment}. For this problem, two distance metrics are commonly used, namely, the \emph{perpendicular Euclidean distances} (\ped) and the \emph{synchronous Euclidean distances} (\sed).
...
Moreover, the \lsa algorithms for this problem fall into two categories, \ie the \textit{optimal} and \textit{sub-optimal} algorithms. 
