...
Various mobile devices, such as smart-phones, on-board diagnostics, personal navigation devices, and wearable smart devices, have been widely using their sensors to collect massive trajectory data of moving objects at a certain sampling rate, and transmit it to cloud servers for location based services, trajectory mining and many other applications.
\textcolor{blue}{The sampling rate is continuously increasing, \eg from 1 point per minute to currently 1 point per second, to get more accurate position information of a moving object. Thus an increasing size of trajectory data is collected, transmitted and stored.}
%
...
A large number of lossy trajectory compression techniques, \eg the piece-wise line {simplification (or approximation)} \cite{Douglas:Peucker, Hershberger:Speeding, Keogh:online,Liu:BQS, Muckell:Compression, Chen:Trajectory, Chen:Fast, Cao:Spatio, Shi:Survey}, have been developed.
The idea of piece-wise line simplification (\lsa) comes from computational geometry, whose target is to approximate a given finer piece-wise linear curve by another coarser piece-wise linear curve ({normally} a sub set of the former), such that the maximum distance of the former from the later is bounded by a user specified constant. 
...
This work focus on the \lsa methods for trajectory compression. 
...
Moreover, the \lsa algorithms fall into two categories, \ie the optimal and sub-optimal algorithms. %targeting the ``min-\#" problem\cite{Imai:Optimal,Chan:Optimal},
...
\ni (4) The recent important progresses in the trajectory simplification, \ie \squishe~\cite{Muckell:Compression}, \bqsa~\cite{Liu:BQS}, \operb~\cite{Lin:Operb} and \cised~\cite{Lin:Cised}, are not systematically evaluated for trajectory data. 
...
This paper presents an experimental comparison of the state-of-the-art \lsa algorithms for trajectory compression, including both the optimal and the sub-optimal methods using \ped and/or \sed. 
The algorithms evaluated in our experiments are summarized in Table~\ref{tab:summary-lsa}, namely,
...
(ii) \bqsa\cite{Liu:BQS} and \squishe~\cite{Muckell:SQUISH}, famous online \lsa algorithms,
(iii) \operb\cite{Lin:Operb}, sector intersection \cite{Williams:Longest,Sklansky:Cone,Dunham:Cone, Zhao:Sleeve} and \cised \cite{Lin:Cised}, one-pass \lsa algorithms, and
(iv) \oped\cite{Chan:Optimal} and \osed, two improved optimal \lsa algorithms, respectively.
%
%
