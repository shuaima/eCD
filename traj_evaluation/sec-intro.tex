...
\textcolor{blue}{The second aspect of quality criteria comes from the points of views of trajectory applications, such as a ``where\_at" query that concerns the errors of  its answer, a trajectory clustering method concerns the similarity between the clusters of original and simplified trajectories, and so on. Indeed, every trajectory application is applicable to evaluate the simplified trajectories from its point of view.}	
%
...
%
\textcolor{blue}{They use errors, \ie~\emph{query errors}, to denote the qualities, where query errors are the max and average value of temporal differs (\eg ``when\_at") or spatial distances (\eg ``where\_at") between the original trajectory and the answer to the queries on the simplified trajectories.}
