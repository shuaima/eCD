...
\sstab (3) {\em One-pass algorithms} such as \operb\cite{Lin:Operb}, \siped \cite{Williams:Longest,Sklansky:Cone,Dunham:Cone, Zhao:Sleeve}, \cised \cite{Lin:Cised}, \intersec\cite{Long:Direction} and \interval \cite{Ke:Interval} apply better local distance checking policies, which even do not need a window or \textcolor{blue}{a buffer to keep} the previous read points, and process each point in a trajectory once and only once.
