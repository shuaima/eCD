...
However, we argue that an optimal and a near optimal \lsa algorithms using \sed (\opts) can achieve $O(n^2 \log n)$ time and $O(n^2)$ time, respectively, by applying the spatio-temporal cone intersection mechanism (See Section~\ref{sec-cised}) developed in our preview work \cised~\cite{Lin:Cised}. 



In the rest of this section, we first introduce the naive optimal (\opt) algorithm \cite{Imai:Optimal} that runs in $O(n^3)$ time. We then review the \ped specific optimal algorithm \optp \cite{Chan:Optimal} that achieves $O(n^2)$ time. Finally, we provide the \sed enabled near optimal algorithm \nopts that achieves $O(n^2)$ time and $O(n)$ space. 
...
Authors of \cite{Chan:Optimal} provide an optimal algorithm using \ped (\optp) that constructs the reachability graph $G$ in $O(n^2)$ time, by using the \textit{sector intersection} mechanism (See Section~\ref{sec-siped}). 
