...
In the rest of this section, we first introduce the naive optimal \lsa algorithm \cite{Imai:Optimal} that runs in $O(n^3)$ time. We then review the \ped specific optimal algorithm \oped \cite{Chan:Optimal} that achieves $O(n^2)$ time. Finally, we provide the \sed enabled optimal algorithm \osed that achieves $O(n^2 \log n)$ time. 
...
Finding a shortest path takes no more than $O(n^2)$ time. Hence, the straightforward algorithm takes $O(n^3)$ time in total.
...
Given a point $P_s$, $0 \le s < n$, \oped builds a sector taking $P_s$ as the start point and checks for all points $P_{s+k}$, $k>0 ~and~ s+k \le n$, whether $(P_s, P_k)$ can be included in graph $G$. It can be added to the graph if and only if $(\bigsqcap_{i=1}^{k-1}\mathcal{S}(P_s, P_{s+i}, \epsilon)) \bigsqcap \vv{P_sP_{s+k}} \ne \{P_s\}$, \ie line segment $\vv{P_sP_{s+k}}$ ~passing through the common intersection area of sectors $\mathcal{S}(P_s, P_{s+i}, \epsilon))$, $0<i<k$. 
