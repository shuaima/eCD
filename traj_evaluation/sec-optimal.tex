...
%
The authors of \cite{Chan:Optimal} presented \oped, an optimal algorithm using \ped that achieves $O(n^2)$ time by using the \textit{sector intersection} mechanism (See Section~\ref{sec-siped}).
...
In the rest of this section, we first introduce the naive optimal \lsa algorithm \cite{Imai:Optimal} that runs in $O(n^3)$ time, then review the \ped specific algorithm \oped \cite{Chan:Optimal} that achieves $O(n^2)$ time. Finally, we provide an \sed enabled optimal \lsa method, named \osed, that achieves $O(n^2 \log n)$ time by applying the spatio-temporal cone intersection mechanism (See Section~\ref{sec-cised}) developed in \cised~ \cite{Lin:Cised}. 
Note \siped and \cised are evaluated in our experiments.
