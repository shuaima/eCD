...
%
The original optimal algorithm supports \ped, however, it is easy to revise it to support \sed or \dad. 
%
...
The authors of \cite{Chan:Optimal} further presented an optimal algorithm using \ped (\optp) that achieves $O(n^2)$ time by using the \textit{sector intersection} mechanism.
% 
Algorithm SP\cite{Long:Direction} is essential an optimization of the original optimal algorithm using \dad that achieves $O(n^2)$ time.
%
...
In the rest of this section, we introduce the naive optimal (\opt) algorithm \cite{Imai:Optimal} that runs in $O(n^3)$ time. 
