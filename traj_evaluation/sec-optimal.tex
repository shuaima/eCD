...
However, we argue that an optimal and a near optimal \lsa algorithms using \sed (\nopts) can achieve $O(n^2 \log n)$ time and $O(n^2)$ time, respectively, by applying the spatio-temporal cone intersection mechanism (See Section~\ref{sec-cised}) developed in our preview work \cised~\cite{Lin:Cised}. 
