...
\todo{All tests are run on an x64-based  PC with 4 Intel(R) Core(TM) i7-6700 CPU @3.40GHz  and 8GB of memory, and {the max heap size of Java VM is 4GB.}}
...
\textcolor{blue}{one of the points} in trajectory $\dddot{\mathcal{T}}_j$ \textcolor{blue}{represented by} a line segment $\mathcal{L}_{l,i}\in\overline{\mathcal{T}_l}$ ($l\in[1,M]$),
...
($125.05\%$, $131.01\%$, $138.01\%$), ($130.03\%$, $140.56\%$, $139.00\%$) and \myred{($?\%$, $?\%$, $?\%$)} of \opt
on datasets \dSets, respectively.
...
(\opt, \tpa, \dpa, \bqsa, \siped(${\epsilon}$), \siped($\frac{\epsilon}{2}$), \operb, \myblue{\operb-A} ) are ($1.6\%$, $2.2\%$, $2.2\%$, $1.9\%$, $2.0\%$, $2.4\%$, $2.4\%$, \myred{$?\%$}) when $\epsilon$ = $40m$.
%
...
%
{Algorithms \myblue{\cised-W}, \cised(${\epsilon}$), \cised($\frac{\epsilon}{2}$), \squishe and {\dagots} are on average \myred{(?, ?, ?)}, ($109.27\%$, $110.13\%$, $115.90\%$,), ($134.35\%$, $159.30\%$, $136.06\%$), ($165.94\%$, $225.68\%$, $206.90\%$) and \myred{$(?\%, ?\%, ?\%)$}
 of \opt on \dSets, respectively.}
%
For example, in \mopsi, the compression ratios of algorithms
(\tpa, \dpa, \squishe, {\dagots}, \cised(${\epsilon}$), \cised($\frac{\epsilon}{2}$),  \myblue{\cised-W},)
are ($3.45\%$, $3.41\%$, $5.75\%$, \myred{$?\%$}, $3.02\%$, $3.86\%$,  \myred{$?\%$}), respectively, when $\epsilon$ = $40m$.
%
...
\textcolor{blue}{Besides, weak algorithms \operb-A and \cised-W usually show a bit advantages in terms of compression ratios compared with corresponding strong algorithms \operb and \cised, especially in cases that smaller error bounds are set.}
...
%
Algorithms \myblue{\operb-A}, \siped(${\epsilon}$) and \bqsa are on average \myred{$(?\%, ?\%, ?\%)$}, ($100.05\%$, $101.01\%$, $102.69\%$) and ($104.67\%$, $108.91\%$, $106.92\%$) of \opt on datasets \dSets, respectively.
For example, the average errors of algorithms
(\opt, \tpa, \dpa, \bqsa, \siped(${\epsilon}$), \siped($\frac{\epsilon}{2}$), \operb, \myblue{\operb-A} ) in the full \mopsi are ($16.08$, $9.19$, $9.68$, $17.4$, $12.96$, $16.83$, $12.77$, \myred{$?\%$}) metres when $\epsilon$ = $40m$.
...
Algorithms \myblue{\cised-W}, \cised(${\epsilon}$), \cised($\frac{\epsilon}{2}$), \squishe and {\dagots} are on average {\myred{$(?\%, ?\%, ?\%)$}, ($97.32\%$, $106.74\%$, $108.16\%$), ($75.29\%$, $76.03\%$, $81.44\%$), ($40.61\%$, $38.15\%$, $34.22\%$) and \myred{$(?\%, ?\%, ?\%)$} of \opt on datasets \dSets, respectively.
%
For example, the average errors of algorithms
(\opt, \tpa, \dpa, \squishe, {\dagots}, \cised(${\epsilon}$), \cised($\frac{\epsilon}{2}$),  \myblue{\cised-W}) in full \mopsi are ($19.39$, $12.17$, $12.20$,  $6.76$, \myred{$?\%$}, $20.68$, $14.71$,  \myred{$?\%$}) metres, respectively, when $\epsilon$ = $40m$.
%
...
(\tpa, \dpa, \bqsa, \siped(${\epsilon}$), \siped($\frac{\epsilon}{2}$), \operb, \myblue{\operb-A} ) is ($232.9$, $124.2$, $469.4$, $6.89$, $7.6$, $8.6$, \myred{$?\%$}) seconds when $\epsilon$ = $40m$.
...
%
For example, in \mopsi, the running time of algorithms
(\tpa, \dpa, {\dagots}, \squishe, \cised($\epsilon$), \cised($\frac{\epsilon}{2}$), \myblue{\cised-W}) is ($156.6$, $104.8$, \myred{$?\%$}, $27.2$, $11.6$, $9.7$, \myred{$?\%$}) seconds when $\epsilon$ = $40m$.
...
\ni \textcolor{blue}{(3) \sed algorithms have smaller average \emph{when\_at} query errors than \ped algorithms, majorly because \sed has smaller \emph{where\_at} query errors and poorer compression ratios compared with \ped. \dad algorithms also have smaller average \emph{when\_at} query errors than \ped algorithms, partially because its poorer compression ratios compared with \ped. }
...
\myblue{and algorithm \dagots~has poor compression ratios, the best average errors and varied efficiency. When \dagots~is processing long trajectories, it needs to frequently copy memory, this will make it even slower than batch algorithms if it is implemented in Java.}
%
...
%
\myblue{Weak simplifications show a bit better compression ratios compared with strong simplifications, especially when the \sed/\ped error bound is set to a small value, \eg less than 40 meters in this tests.}
...
%
\emph{(5)} {All the tested algorithms have bounded errors in data aging, where the error bounds of \dpa using \ped and \sed are $max\{\epsilon_1, \epsilon_2\}$ and the others are $\epsilon_1 + \epsilon_2$, \myblue{hence, they are all \emph{aging safe}}. Moreover, algorithms \dpa using \ped and \sed are \emph{aging friendly}, while others are not. }
