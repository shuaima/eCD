...
In this set of tests, we compare the error and compression ratios of algorithms in data aging. We set $\epsilon_1=40m$ (or $\epsilon_1=30^o$) in the first run and $\epsilon_2=60m$ (or $\epsilon_2=50^o$) in the second run, the max errors and compression ratios are reported in Table \ref{tab:aging-me} and Table \ref{tab:aging-cr}, respectively.

\ni (1) Algorithms \dpa using \ped and \sed both are aging friendly, and have max errors less than $\epsilon_2 = 60m$, while algorithm \dpa using \dad and other algorithms are not aging friendly, and their max error are not more than $\epsilon_1 + \epsilon_2 = 100m$ or $\epsilon_1 + \epsilon_2 = 80^o$.

\ni (2)


\todo{cr:  first $\epsilon_1$ then $\epsilon_2$ vs $\epsilon_1 + \epsilon_2$}


\begin{table}
	%\renewcommand{\arraystretch}{1.2}
	\vspace{-1ex}
	\caption{\small The max errors of data aging that set $\epsilon_1=40m$ (or $\epsilon_1=30^o$) in the first run and $\epsilon_2=60m$ (or $\epsilon_2=50^o$) in the second run.}
	\centering
	\scriptsize
	\begin{tabular}{|l|c|c|c|l|c|c|c|l|c|c|c|}
		...
		\hline
		{\tpa} &	$ $ & $  $ &	$ - $	&\tpa 	& $97.55$& $96.70$ &$90.53$ & \tpa	& $79.94$	& $79.93$	& $79.64$ \\
		\hline
...
\begin{table}
	%\renewcommand{\arraystretch}{1.2}
	\vspace{-1ex}
	\caption{\small The compression ratios of data aging that set $\epsilon_1=40m$ (or $\epsilon_1=30^o$) in the first run and $\epsilon_2=60m$ (or $\epsilon_2=50^o$) in the second run.}
	\centering
	\scriptsize
	\begin{tabular}{|l|c|c|c|l|c|c|c|l|c|c|c|}
		\hline
		\bf{Alg. (PED)}  &\ucar &\geolife &\mopsi & \bf{Alg. (SED)}  &\ucar &\geolife &\mopsi &\bf{Alg. (DAD)}  &\ucar &\geolife &\mopsi \\
		\hline
		{\dpa} &	$5.10 $ & $2.16 $ &	$1.97 $	&\dpa &$10.03$ &$3.83$ & $2.78 $ & \dpa	& $12.75$	& $22.20$	& $20.34 $ \\
		\hline
		{\tpa} &	$ $ & $  $ &	$ - $	&\tpa 	& $10.09$& $3.78$ &$2.76$ & \tpa	& $3.80$	& $13.28$	& $11.30$ \\
		\hline
		{\bqsa} &	$ $ & $ $ &	$  $	&\squishe &$11.38$ &$4.55$ & $3.47$ & \opwa	& $4.09$	& $13.88$	& $11.92$ \\
		\hline
		{\siped($\epsilon$)} &	$  $ & $  $ &	$  $	&\cised($\epsilon$) & $9.12$ &$3.28$ &$2.45 $ & \interval	& $3.81$	& $13.37$	& $11.47 $ \\
		\hline
		{\siped($\frac{\epsilon}{2}$)} &	$  $ & $  $ &	$  $ &\cised($\frac{\epsilon}{2}$) &$10.66 $ & $3.94$ & $2.99 $& \intersec	& $5.37$	& $17.11$	& $15.05 $ \\
		\hline
		{\operb} &	$ $ & $ $ &	$ $	& & & & & & & & \\
		\hline
	\end{tabular}
...
%\todo{ae:  first $\epsilon_1$ then $\epsilon_2$ vs $\epsilon_1 + \epsilon_2$}
