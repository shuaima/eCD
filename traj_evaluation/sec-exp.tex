...
For one-pass algorithms \siped and \cised, we implement two versions of them, \ie the half and full $\epsilon$.
...
to the worst are the \opt algorithm, online algorithm \bqsa, one-pass algorithm \siped using a full $\epsilon$ sector (\siped($\epsilon$) in short), batch algorithms \tpa and
...
to the largest are are one-pass algorithm \intersec, batch algorithms \dpa and \tpa, one-pass algorithm \interval and online algorithms \opwa, and the \opt algorithm.
\eat{%%%%%%%%%%%%%
...
\stitle{Analyses of distance metrics.}
The computation using \dad is faster than using \ped and \sed, and the computation time using \ped and \sed are 2.3 and 1.7 times of using \dad, respectively.
{It is also worth pointing out that algorithms \dpa using \ped, \sed and \dad have a similar running time in all datasets, though the computing of \ped is much heavier than \sed and \dad. The reason is that \dpa using \ped has the best compression ratios which instead leads to the least splitting processes in the top-down manner. Combining these two factors, \ie the computing of distance/direction deviation and the processing of trajectory splitting, finally, \dpa using \ped has a similar running time as \dpa using \dad or \sed.}
...
%
Using \dad is faster than using \ped and \sed, and the computation time using \ped and \sed are 2.3 and 1.7 times of using \dad, respectively.
