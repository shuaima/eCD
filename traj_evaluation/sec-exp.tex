...
We use four real-life datasets \sercar, \geolife\cite{Web:Geolife}, \mopsi\cite{Web:Mopsi} and \pricar shown in Table~\ref{tab:datasets} to evaluate those \lsa algorithms.
...
\begin{table}
	...
	\begin{tabular}{|l|c|c|c|r|}
		...
		%\sercar	&11,000	    &3-5	    &$\sim119.1$   &1.31G\\		\hline
		%
...
\sstab (1) When increasing $\epsilon$, the compression ratios decrease. For example, in \sercar,
...
For example, when $\epsilon = 40m$, the compression ratios are ($20.9\%$, $20.4\%$, $11.1\%$, $3.1\%$) of \fbqsa, ($20.7\%$, $18.7\%$, $8.9\%$, $2.6\%$) of \dpa and ($22.3\%$, $20.3\%$, $10.0\%$, $2.6\%$) of \operb on (\taxi, \truck, \sercar, \geolife), respectively.
For all $\epsilon$, the compression ratios of \operb are on average ($107.2\%$, $98.3\%$, $92.9\%$, {$85.1\%$}) of \fbqsa and ($107.7\%$, $106.6\%$, $113.5\%$, $99.6\%$) of \dpa on (\taxi, \truck, \sercar, \geolife), respectively.
...
To evaluate the impacts of $\epsilon$ on errors of these algorithms, we varied $\epsilon$ from $5m$ to $100m$ on the entire \taxi, \truck, \sercar and \geolife, respectively.
The results are reported in Figure~\ref{fig:ae-ped} and Figure~\ref{fig:ae-sed}.

\sstab (1) When increasing $\epsilon$, the average errors increase. For example, in \sercar,
the compression ratios are greater than $28.7\%$ when $\epsilon$ = $5m$, but are less than $6.6\%$ when $\epsilon$ = $100m$.
...
To evaluate the impacts of $\epsilon$ on efficiency of these algorithms, we varied $\epsilon$ from $5m$ to $100m$ on the entire \taxi, \truck, \sercar and \geolife, respectively.
The results are reported in Figure~\ref{fig:ae-ped} and Figure~\ref{fig:ae-sed}.
...
\emph{\sstab{(1) Compression ratios}}. (a) \operb is comparable {with \fbqsa and \dpa}. Its compression ratios are on average $(107.2\%, ~98.3\%, ~92.9\%, ~85.1\%)$ of \fbqsa and ($107.7\%$, $106.6\%$, $113.5\%$, $99.6\%$) of \dpa on (\taxi, \truck, \sercar, \geolife), respectively, and \operb has a better performance on trajectories with higher sampling rates.
...
\emph{\sstab{(2) Average errors}}. {Algorithm \operb has similar average errors with \operba. They have lower average errors than the other algorithms on \taxi while higher on \sercar.}
