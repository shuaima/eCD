...
\textcolor{blue}{Besides, algorithms \operb\cite{Lin:Operb} and \cised\cite{Lin:Cised} both have weak versions, named \operb-A and \cised-W, that allow data interpolations. We also implement them.}
...
\sout{a point} \textcolor{blue}{one of the points} in trajectory $\dddot{\mathcal{T}}_j$ \sout{contained in} \textcolor{blue}{represented by} a line segment $\mathcal{L}_{l,i}\in\overline{\mathcal{T}_l}$ ($l\in[1,M]$),
...
(\opt, \tpa, \dpa, \bqsa, \siped(${\epsilon}$), \siped($\frac{\epsilon}{2}$), \operb, \todo{\operb-A} ) are ($1.6\%$, $2.2\%$, $2.2\%$, $1.9\%$, $2.0\%$, $2.4\%$, $2.4\%$, \todo{$?\%$}) when $\epsilon$ = $40m$.
%
...
to the worst are the \opt algorithm, \todo{\cised-W}, one-pass algorithm \cised($\epsilon$), batch algorithms \tpa and
\dpa, one-pass algorithm \cised($\frac{\epsilon}{2}$), and online algorithm \todo{\dagots} and \squishe.
%
...
%
{Algorithms \todo{\cised-W}, \cised(${\epsilon}$), \cised($\frac{\epsilon}{2}$), \squishe and \todo{\dagots} are on average \todo{(?, ?, ?)}, ($109.27\%$, $110.13\%$, $115.90\%$,), ($134.35\%$, $159.30\%$, $136.06\%$), ($165.94\%$, $225.68\%$, $206.90\%$) and \todo{(?, ?, ?)}
 of \opt on \dSets, respectively.}
%
For example, in \mopsi, the compression ratios of algorithms
(\tpa, \dpa, \squishe, \todo{\dagots}, \cised(${\epsilon}$), \cised($\frac{\epsilon}{2}$),  \todo{\cised-W},)
are ($3.45\%$, $3.41\%$, $5.75\%$, \todo{$?$}, $3.02\%$, $3.86\%$,  \todo{$?$}), respectively, when $\epsilon$ = $40m$.
%
...
\begin{figure*}[tb!]
	...
	\includegraphics[scale=0.50]{Figures/Exp-DAD-CR-size-service.jpg} 	\hspace{0.5ex}
	\includegraphics[scale=0.50]{Figures/Exp-DAD-CR-size-geolife.jpg}	\hspace{0.5ex}
	\includegraphics[scale=0.50]{Figures/Exp-DAD-CR-size-mopsi.jpg}		
	\vspace{-2ex}
...
algorithms \siped($\frac{\epsilon}{2}$), \operb, the optimal algorithm \opt and one-pass algorithm \siped(${\epsilon}$),\todo{\operb-A}, and online algorithm \bqsa.
%
...
%
Algorithms \todo{\operb-A}, \siped(${\epsilon}$) and \bqsa are on average \todo{(?, ?, ?)}, ($100.05\%$, $101.01\%$, $102.69\%$) and ($104.67\%$, $108.91\%$, $106.92\%$) of \opt on datasets \dSets, respectively.
For example, the average errors of algorithms
(\opt, \tpa, \dpa, \bqsa, \siped(${\epsilon}$), \siped($\frac{\epsilon}{2}$), \operb, \todo{\operb-A} ) in the full \mopsi are ($16.08$, $9.19$, $9.68$, $17.4$, $12.96$, $16.83$, $12.77$, \todo{\operb-A}) metres when $\epsilon$ = $40m$.
...
to the largest are online algorithm \squishe,\todo{\dagots}, batch algorithms \tpa and \dpa,
one-pass algorithm \cised($\frac{\epsilon}{2}$), \todo{\cised-W}, and one-pass algorithm \cised(${\epsilon}$) and the \opt algorithm.
%
Algorithms \tpa and \dpa are comparable, and they are on average
{($60.36\%$, $66.11\%$, $62.43\%$) and ($62.54\%$, $67.04\%$, $68.64\%$)} of \opt on datasets \dSets, respectively.
Algorithms \todo{\cised-W}, \cised(${\epsilon}$), \cised($\frac{\epsilon}{2}$), \squishe and \todo{\dagots} are on average {\todo{(?, ?, ?, ?)}, ($97.32\%$, $106.74\%$, $108.16\%$), ($75.29\%$, $76.03\%$, $81.44\%$), ($40.61\%$, $38.15\%$, $34.22\%$) and \todo{(?, ?, ?, ?)} of \opt on datasets \dSets, respectively.
%
For example, the average errors of algorithms
(\opt, \tpa, \dpa, \squishe,\todo{\dagots}, \cised(${\epsilon}$), \cised($\frac{\epsilon}{2}$),  \todo{\cised-W}) in full \mopsi are ($19.39$, $12.17$, $12.20$,  $6.76$, $\todo{\dagots}$, $20.68$, $14.71$,  $\todo{\cised-W}$) metres, respectively, when $\epsilon$ = $40m$.
%
...
\begin{figure*}[tb!]
	...
	\includegraphics[scale=0.50]{Figures/Exp-DAD-error-size-service.jpg}	\hspace{0.5ex}
	\includegraphics[scale=0.50]{Figures/Exp-DAD-error-size-geolife.jpg}	\hspace{0.5ex}
	\includegraphics[scale=0.50]{Figures/Exp-DAD-error-size-mopsi.jpg}	
	\vspace{-2ex}
...
\begin{figure*}[tb!]
	...
	\includegraphics[scale=0.50]{Figures/Exp-DAD-time-size-service.jpg} 	\hspace{0.5ex}
	\includegraphics[scale=0.50]{Figures/Exp-DAD-time-size-geolife.jpg}	\hspace{0.5ex}
	\includegraphics[scale=0.50]{Figures/Exp-DAD-time-size-mopsi.jpg}		
	\vspace{-2ex}
...
\sstab (2) When using \ped, in most cases, the running time from the smallest to the largest is one-pass algorithms \siped and \operb,\todo{\operb-A} batch algorithms \tpa and \dpa, and online algorithm \bqsa.
Algorithms \todo{\operb-A}, \siped($\frac{\epsilon}{2}$) and \operb are comparable, algorithm \siped(${\epsilon}$) is $(0.92, 0.92, 0.91)$ times of \siped($\frac{\epsilon}{2}$), and algorithms \tpa, \dpa and \bqsa are on average
...
(\tpa, \dpa, \bqsa, \siped(${\epsilon}$), \siped($\frac{\epsilon}{2}$), \operb, \todo{\operb-A} ) is ($232.9$, $124.2$, $469.4$, $6.89$, $7.6$, $8.6$, \todo{\operb-A}) seconds when $\epsilon$ = $40m$.

\sstab (3) When using \sed, the running time from the smallest to the largest is one-pass algorithm \cised, \todo{\cised-W}, online algorithm \squishe, batch algorithms \tpa and \dpa, and online algorithm \todo{\dagots}. Algorithm \cised(${\epsilon}$) is $(1.17, 1.17, 0.91)$ times of \cised($\frac{\epsilon}{2}$), and algorithms \todo{\dagots}, \tpa, \dpa and \squishe are on average
\todo{(?, ?, ?)}, ($13.33$, $15.81$, $13.09$), ($12.93$, $10.64$, $8.79$) and
($2.75$, $2.78$, $2.57$) times slower than \cised($\frac{\epsilon}{2}$) on datasets \dSets, respectively.
%
For example, in \mopsi, the running time of algorithms
(\tpa, \dpa, \todo{\dagots}, \squishe, \cised($\epsilon$), \cised($\frac{\epsilon}{2}$), \todo{\cised-W}) is ($156.6$, $104.8$,\todo{\dagots}, $27.2$, $11.6$, $9.7$, \todo{\cised-W}) seconds when $\epsilon$ = $40m$.

\sstab {(4) When using \dad,} \textcolor{blue}{one-pass algorithms \intersec and \interval run prominently fast than batch algorithms \tpa and \dpa and online algorithm \opwa.}
%
...
Algorithms \siped, \operb and \interval have similar running time, and algorithm \cised runs a bit slower than them, partially because finding the common intersection of spatial-temporal cones is a heavier work than sectors or ranges. \todo{weak vs strong}
...
\ni \textcolor{blue}{(1) When using \ped, the max query errors of all algorithms are more than $10^6$ meters in all datasets, significantly larger than the error bound (40 meters in \mytable{tab:query-when-me}). The large max errors also lead to larger average query errors, \ie they are greater than error bounds in all datasets.}
...
\ni \textcolor{blue}{(3) When using \dad, the max query errors of all algorithms are more than $10^6$ meters in dataset \ucar, and more than $10^5$ meters in datasets \geolife and \mopsi, respectively, also significantly larger than the error bound (40 meters in \mytable{tab:query-when-me}). Moreover, compared with using \ped, it has more points having query errors larger than error bounds, thus, it has the largest average query errors in all algorithms and all datasets.}
