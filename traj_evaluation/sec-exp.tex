...
($122.69\%$, $129.08\%$, $131.97\%$, $129.08\%$),($121.36\%$, $129.27\%$, $130.11\%$, $129.27\%$)
...
($42.88\%$, $47.49\%$, $63.15\%$, $47.49\%$),($45.17\%$, $50.88\%$, $64.50\%$,
...
(\tpa,\dpa,\bqsa,\siped,\operb ) are ($232960.00$, $124310.00$,\textcolor{red}{0.0}, $7553.00$, $8798.00$) when $\epsilon$ = $40m$.
...
For example, in \mopsi, the running time of algorithms
(\tpa,\dpa,\squishe,\cised) are  ($156540.00$, $104829.00$, $27612.00$, $9808.00$) when $\epsilon$ = $40m$.

\sstab (4) Batch algorithms \dpa and \tpa using \sed run a bit faster than using
\ped, while the one-pass algorithm \cised run $1.30$ times slower than \siped and \operb.
...
\dpa, \bqsa, \siped, \operb)  are on average ($131.25\%$, $136.50\%$,
\textcolor{red}{  0.0}, $146.60\%$, $147.85\%$)
...
(c) Batch algorithms \dpa and \tpa using \sed run a bit faster than using \ped, while the one-pass algorithm \cised run \textcolor{red}{$1.30$} times slower than \siped and \operb.
