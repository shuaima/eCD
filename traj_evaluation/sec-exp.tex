...
($100.81\%$, $102.91\%$, $102.27\%$) and ($102.38\%$, $101.98\%$, $103.52\%$)
 of the optimal algorithm \opt on datasets \dSets, respectively.}
%
{Algorithm \dpa is on average ($133.19\%$, $283.93\%$, $143.79\%$)
 of the optimal algorithm \opt on datasets \dSets, respectively.}
%
%
...
\sstab  \todo{(5) When using \dad,} the average errors from the smallest
to the largest are online algorithm \squishe, batch algorithms \dpa and \tpa,
and one-pass algorithm \interval. 
For example, in \mopsi, the average errors of algorithms
(\tpa, \dpa, \interval) are ($11.0$, $9.6$, $11.5$) degrees, respectively, when $\epsilon$ = $45$.
...
\sstab  \todo{(4) When using \dad,} the average errors from the smallest
to the largest are batch algorithms \dpa and \tpa, one-pass algorithm \interval, and the naive optimal algorithm \opt.

Algorithms \tpa and \interval are comparable, and they are on average
{($89.46\%$, $89.61\%$, $89.16\%$), ($91.41\%$, $97.52\%$, $94.05\%$)} of \opt on datasets \dSets, respectively.
Algorithms \dpa is on average ($62.97\%$, $71.42\%$, $71.85\%$) of \opt on datasets \dSets, respectively.
...
\sstab  \todo{(4) When using \dad,} the running time from the smallest to the
largest are one-pass algorithm \interval, batch algorithms \tpa and \dpa. 

Algorithms \tpa, and \dpa are on average
($6.66$, $13.55$, $12.73$, $12.73$), ($11.67\%$, $14.24\%$, $16.51\%$, $17.59\%$)
times slower than \interval on datasets \dSets, respectively.
%
For example, in \mopsi, the running time of algorithms
(\tpa, \dpa, \interval) are  ($105.57\%$, $152.53\%$, $8.57\%$) seconds when $\epsilon$ = $45d$.
