...
\sstab (6) One pass algorithms \siped (\ped), \operb (\ped) and \interval (\dad) have the similar efficiency, and algorithm \cised (\sed) runs a bit slower than them.
...
(d) Batch algorithms \dpa and \tpa using \sed run a bit faster than using \ped, while the one-pass algorithm \cised runs {$1.30$} times slower than \siped and \operb.
