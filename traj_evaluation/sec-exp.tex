...
\sstab {(5) When using \dad,} the compression ratios from the best to the worst are \tpa, \interval and \dpa.
...
and varied the size \trajec{|T|} of a trajectory from $1,000$ points to $10,000$ points, while fixed error bound $\epsilon = 60$ meters for \ped and \sed ({or $\epsilon = 45$ degrees for \dad}).
The results are reported in Figure~\ref{fig:cr-ped-size}, Figure~\ref{fig:cr-sed-size} and Figure~\ref{fig:cr-dad-size}.


\sstab (1) Trajectory size has few impact on compression ratio.
...
\sstab {(4) When using \dad,} the compression ratios from the best
...
The results are reported in Figure~\ref{fig:ae-ped-epsilon}, Figure~\ref{fig:ae-sed-epsilon} and Figure~\ref{fig:ae-dad-epsilon}.
...
\sstab {(5) \textcolor{blue}{When using \dad, the average errors from the smallest
to the largest are  batch algorithms \dpa and \tpa,
and one-pass algorithm \interval.
For example, in \mopsi, the average errors of algorithms
(\tpa, \dpa, \interval) are ($65.6$, $75.4$, $68.1$) degrees, respectively, when $\epsilon$ = $45$ degrees.
}}
...
The results are reported in Figure~\ref{fig:ae-ped-size}, Figure~\ref{fig:ae-sed-size} and Figure~\ref{fig:ae-dad-size}.
...
\sstab {(4) \textcolor{blue}{When using \dad, the average errors from the smallest
to the largest are batch algorithms \dpa and \tpa, one-pass algorithm \interval, and the optimal algorithm \opt.
%
Algorithms \tpa and \interval are comparable, and they are on average
{($89.46\%$, $89.61\%$, $89.16\%$, $85.46\%$) and ($91.41\%$, $97.52\%$, $94.05\%$, $106.25\%$)} of algorithm \opt on datasets \dSets, respectively.
Algorithm \dpa is on average ($62.97\%$, $71.42\%$, $71.85\%$, $85.26\%$) of \opt on datasets \dSets, respectively.
}}
...
To evaluate the impacts of $\epsilon$ on running time of these algorithms, we varied $\epsilon$ from $10m$ to $100m$ on the entire four datasets, respectively.
%
...
\subsection{Appendix Experiments}

\sstab {(5) \textcolor{blue}{When using \dad, the average errors from the smallest
to the largest are  batch algorithms \dpa and \tpa,
and one-pass algorithm \interval.
For example, in \mopsi, the average errors of algorithms
(\tpa, \dpa, \interval) are ($65.6$, $75.4$, $68.1$) degrees, respectively, when
$\epsilon$ = $45$ meters.},
the max errors of algorithms
(\tpa, \dpa, \interval) are ($68247.9$, $66794.4$, $68247.9$) meters, respectively, when $\epsilon$ = $45$ degrees.}
...
\begin{figure*}[tb!]
	\centering
	\includegraphics[scale=0.315]{Figures/Exp-DAD-pedMaxErr-epsilon-taxi.png} \hspace{1ex}
	\includegraphics[scale=0.315]{Figures/Exp-DAD-pedMaxErr-epsilon-service.png}	\hspace{1ex}
	\includegraphics[scale=0.315]{Figures/Exp-DAD-pedMaxErr-epsilon-geolife.png}	\hspace{1ex}
	\includegraphics[scale=0.315]{Figures/Exp-DAD-pedMaxErr-epsilon-mopsi.png}	
	\vspace{-3ex}
	\caption{\small Evaluation of max errors (\ped): varying the error bound $\epsilon$.}
	\label{fig:ae-dad-ped-epsilon}
	\vspace{-2ex}
\end{figure*}
...
\begin{figure*}[tb!]
	...
	\vspace{-3ex}
	\caption{\small Evaluation of average errors (\ped): varying the size of trajectories.}
	\label{fig:ae-dad-ped-size}
	\vspace{-2ex}
\end{figure*}
\begin{figure*}[tb!]
	\centering
	\includegraphics[scale=0.315]{Figures/Exp-DAD-pedMaxErr-size-taxi.png} \hspace{1ex}
	\includegraphics[scale=0.315]{Figures/Exp-DAD-pedMaxErr-size-service.png}	\hspace{1ex}
	\includegraphics[scale=0.315]{Figures/Exp-DAD-pedMaxErr-size-geolife.png}	\hspace{1ex}
	\includegraphics[scale=0.315]{Figures/Exp-DAD-pedMaxErr-size-mopsi.png}	
	\vspace{-3ex}
	\caption{\small Evaluation of max errors (\ped): varying the size of trajectories.}
	\label{fig:ae-dad-ped-size}
	\vspace{-2ex}
\end{figure*}
