...
\label{sec-exp}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section, we present extensive and systematic experimental studies and analyses of twelve representative \lsa algorithms \myblue{(including their variants)}.
...
\begin{table}[tb!]
	...
	\begin{tabular}{|l|c|c|c|c|}
		...
		%
		\ucar	&\myblue{200}	    &3-5	&$\sim114.0$K   &\myblue{22.8M} 	\\	\hline
...
{Compression ratios and errors are the most popular metrics to evaluate the effectiveness of \lsa algorithms, and they are also the measures to evaluate the aging safety and aging friendliness of \lsa algorithms.
...
\myblue{The max (average) simplification error is the max (average) value of the distances from every point of the original trajectories to its representing line segment of the simplified trajectories.}
...
\textcolor{blue}{Besides, weak algorithms \operb-A and \cised-W normally have a bit advantages in terms of compression ratios compared with corresponding strong algorithms \operb and \cised in cases that small error bounds (\eg $\epsilon<40 meters$ in these tests) are set.}
...
%
First, \textcolor{blue}{because \sed saves temporal information while \ped does not (this will lead to larger query error compared with \sed, see Section \ref{sec-exp-query})}, given the same error bound $\epsilon$, the compression ratios of algorithms using \ped are obviously better
...
%
For example, the average errors of algorithms
(\opt, \tpa, \dpa, \squishe, {\dagots}, \cised(${\epsilon}$), \cised($\frac{\epsilon}{2}$),  \myblue{\cised-W}) in full \mopsi are ($19.39$, $12.17$, $12.20$,  $6.76$, \myblue{$4.67$}, $20.68$, $14.71$,  \myblue{$24.12$}) metres, respectively, when $\epsilon$ = $40m$.
%
...
%
\myblue{Weak simplifications show a bit better compression ratios compared with strong simplifications when the \sed/\ped error bound is set to a small value, \eg less than 40 meters in this tests.}
...
\textcolor{blue}{Comparing with the recent experimental study \cite{Zhang:Evaluation}, this work has the following new or different findings: 
	(1) compression ratios and efficiency of optimal, batch, online and one-pass algorithms are systematicly evaluated \wrt distance metrics (\ped, \sed and \dad), error bounds and data sizes, as summarized in the above, 
	(2) aging friendliness and safety are comprehensively evaluated, as reported in Section \ref{sec:exp-data-aging},
	...
	(4) one-pass algorithm \siped ($\epsilon$) is efficient as well as has good compression ratios, hence, it is a good choice in scenarios that only spacial information is concerned, and 
