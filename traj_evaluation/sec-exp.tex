...
Using 4 real-life datasets, we conducted 3 sets of experiments, each under varied error bounds $\epsilon$ and trajectory sizes, to evaluate the compression ratios, errors and efficiency of these algorithms, and the impacts of 3 distance metrics, \ie~\ped, \sed and \dad, to them.
%
...
We implement the representative algorithms of \mytable{tab:summary-lsa}. They are \opt, \dpa, \tpa, \opwa, \bqsa, \squishe, \operb, \siped, \cised, \intersec and \interval.
...
%
This result shows ~\sed saves temporal information at a cost of saving twice more points.
...
to the largest are batch algorithms \dpa and \tpa, one-pass algorithm \interval and online algorithms \opwa, and the \opt algorithm.
\eat{%%%%%%%%%%%%%
...
(1) For batch algorithms, both bottom-up algorithm (\tpa) and top-down algorithm (\dpa) have the similar average errors, and they are pretty good compared with other algorithms.
%
...
Note that even on the small datasets, \emph{the running time of the \opt algorithm is thousands of times slower than one-pass algorithms}. Indeed, it is not easy to show all these algorithms in a figure, thus, only the results of sub-optimal algorithms are shown in these figures.
...
For online algorithms, \squishe is faster than \bqsa and \opwa at a cost of poorer compression ratios, and it is still times slower than one pass algorithms. \bqsa and \opwa both have poor efficiencies as they finally need the batch approach. Even this batch approach is run in a buffer, it is still time consuming.
...
%
The running time of batch algorithms \dpa and \tpa decreases and increases with the increase of error bound $\epsilon$, respectively. When using \ped or \sed, top-down algorithm \dpa usually runs faster than bottom-up algorithm \tpa when the error bound $\epsilon$~is large enough (\eg in \geolife, $\epsilon >10$ metres when using \ped and $\epsilon >30$ metres when using \sed). When using \dad, the top-down algorithm is normally a bit slower than the bottom-up algorithm.
Top-down algorithm also runs faster than bottom-up algorithm in high sampling datasets when using \ped or \sed.
%
...
%
One-pass algorithms show a linear running time and they are not very sensitive to error bound $\epsilon$, and also scale well with the increase of trajectory size.
...
The output sizes of algorithms using \sed are approximately twice of \ped, and in practical (\eg $\epsilon <100$ meters and $\epsilon < 60$ degrees), \ped and \sed usually bring obvious better compression ratios than \dad, especially in high sampling data sets.
%
...
%
The computing of a \dad is faster than \ped and \sed, and the running time of computing \ped and \sed are 2.3 and 1.7 times of \dad, respectively.
