...
\stitle{{Exp-2.2}: Impacts of the optimization techniques}.
%To evaluate the impacts of the optimization techniques (see Section~\ref{subsec-optimizations}),
We compared algorithms \operb and \operba with \kw{Raw}-\operb and \kw{Raw}-\operba, respectively.
...
\operb is  on average {($87.9\%$, ${71.8\%}$, $61.8\%$, $58.0\%$)} of \kw{Raw}-\operb, and \operba is  on average {($93.1\%$, ${88.5\%}$, $77.1\%$, $78.5\%$)} of \kw{Raw}-\operba
...
For example, \operb is {($92.1\%$, ${87.3\%}$, $82.6\%$)} of \kw{Raw}-\operb on \taxi, and {($78.5\%$, $70.9\%$, $63.7\%$)} of \kw{Raw}-\operb on \truck
when $\zeta$ = ($10$, $40$, $100$), respectively. It is similar for algorithm \operba.


\stitle{{Exp-2.3}: Distribution of line segments}.
...
%\sstab(1) \taxi and \truck has many light line segments (a line segment including few points) while \sercar has many heavy line segments (a line segment including many points). The distributions of lines of \sercar are likely subjected to $log-normal$ distributions. Most line segments have 3-6 points in \sercar. These distributions have relative long tails. The longest line of \sercar includes more than $200$ points when {$\zeta = 40$}.
...
\stitle{Exp-1.3: Impacts of optimization techniques}.
...
\sstab(1) The running time of all algorithms slightly decreases with the increase of $\zeta$, consistent with {Exp-1.2}.

\sstab(2) The running time of \kw{Raw}-\operb is {$(85.0\%$, $79.6\%$, $90.4\%$, $100.4\%$)} of \operb on average on ($\taxi$, $\truck$, $\sercar$, $\geolife$), respectively,
and the running time of \kw{Raw}-\operba is {$(91.3\%$, $90.1\%$, $91.6\%$, $101.5\%$) }of \operba on average on ($\taxi$, $\truck$, $\sercar$, $\geolife$), respectively.
