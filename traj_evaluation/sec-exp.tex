...
In this set of tests, we compare the error and compression ratios of algorithms in data aging. We set $\epsilon_1=40m$ (or $\epsilon_1=30^o$) in the first run, $\epsilon_2=60m$ (or $\epsilon_2=50^o$) in the second run and $\epsilon_3=100m$ (or $\epsilon_3=80^o$) in the third run, the max errors are reported in Table \ref{tab:aging-me}, and the compression ratios are reported in Tables \ref{tab:aging-cr} and \ref{tab:cr}, respectively.
...
\ni (3) Tables \ref{tab:aging-cr} and \ref{tab:cr} tell that data aging, using $\epsilon_1$ and $\epsilon_2$ in turn, leads to a bit poorer compression ratios compared with simplification directly using $\epsilon_3=\epsilon_1 + \epsilon_2$. 
...
\begin{table}
	%\renewcommand{\arraystretch}{1.2}
	\caption{\small The compression ratios of algorithms in data aging that set $\epsilon_1=40m$ (or $\epsilon_1=30^o$ when using \dad) in the first run and $\epsilon_2=60m$ (or $\epsilon_2=50^o$ when using \dad) in the second run.}
	\centering
	\scriptsize
	\vspace{-1ex}
	\begin{tabular}{|l|c|c|c|l|c|c|c|l|c|c|c|}
...
\begin{table}
	%\renewcommand{\arraystretch}{1.2}
	\caption{\small The compression ratios of algorithms that set $\epsilon_3=100m$ (or $\epsilon_3=80^o$ when using \dad).}
	\centering
	\scriptsize
	\vspace{-1ex}
	\begin{tabular}{|l|c|c|c|l|c|c|c|l|c|c|c|}
...
\stitle{Analyses of distance metrics.}
\sed is spatio-temporal queries friendly while the others are not.
...
%
(5) \textcolor{blue}{Algorithms \dpa using \ped and/or \sed are aging friendly, others are not. All the algorithms have bounded error in data aging, where the aging bounds of \dpa using \ped and \sed are $max\{\epsilon_1, \epsilon_2\}$ while others are $\epsilon_1 + \epsilon_2$.}
...
%
\textcolor{blue}{(5) \sed is spatio-temporal queries friendly while the others are not. Notice that algorithms using \ped and/or \dad may lead to enormously large query errors.}
