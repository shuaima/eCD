...
\cite{Chen:Fast} defined \eat{the local integral square synchronous Euclidean distance (LISSED) and} the integral square synchronous Euclidean distance (ISSED), and used it to fast the construction of reachability graphs, \cite{Wu:Graph} followed the ideas of \cite{Chen:Fast} and Dots\cite{Cao:Dots} is an online and near-optimal trajectory simplification algorithm that uses ISSED.
