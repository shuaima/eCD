...
We next review algorithms \operb, \siped and \cised evaluated in out experiments.
...
\label{sec-interval}
The \interval\cite{Ke:Interval} approach is develop for \dad. It is somehow very like the \emph{sector intersection} method. It defines \emph{direction range} for each line segment and checks the intersection of direction ranges as a way similar with the sector intersection.
...
It is proved that if the \emph{common subinterval} $\bigsqcap_{i=1}^{k}Range(\vv{P_{s+i-1} P_{s+i}}.\theta, \epsilon)\ne \phi$ and $\vv{P_sP_{s+k}}$ falls in the \emph{common subinterval}, then the maximum angular difference between each $\vv{P_{s+i-1}P_{s+i}}$, $i\in [1, k]$, and $\vv{P_sP_{s+k}}$ is not larger than $\epsilon$.
