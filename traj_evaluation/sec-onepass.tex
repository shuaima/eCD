...
\textcolor{blue}{It is worth pointing out that algorithms \siped, \ldr, \operb, \cised and \intersec share a common idea, \ie using a half-$\epsilon$ to implement the strong simplification to ensure the max error of $\epsilon$, and in the rest of the paper, we will discuss that the half-$\epsilon$ of \siped and \cised could be extended to the full-$\epsilon$ with some small modifications, similar to the way that \interval extends \intersec. }
