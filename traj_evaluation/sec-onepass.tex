...
Direction range intersection \cite{Long:Direction,Ke:Interval} approach is somehow like the \emph{sector intersection} methods except that they are designed for \dad.
%(\eg\ \intersec \cite{Long:Direction} and \interval \cite{Ke:Interval})
%{\em As \interval is an extension of \intersec, we choose \interval as the representative of  one-pass algorithms using \dad}.
...
\stitle{Algorithm \ridad Using \dad\cite{Long:Direction, Ke:Interval}.}
...
Algorithm \ridad\cite{Long:Direction}, \textcolor{blue}{originally called \kw{Intersec}}, uses a half range, and shows that if the \emph{common subinterval} $\bigsqcap_{i=1}^{k}range(\vv{P_{s+i-1} P_{s+i}}.\theta, \epsilon/2)\ne \phi$, then the angular difference between $\vv{P_{s+i-1}P_{s+i}}$ and $\vv{P_sP_{s+k}}$ for all $i\in [1, k]$ is not larger than $\epsilon$.
%
Recently, \textcolor{blue}{authors of \cite{Ke:Interval} extend} \ridad from half to full ranges, \textcolor{blue}{and name it \interval,} by showing that if the \emph{common subinterval} $\bigsqcap_{i=1}^{k}range(\vv{P_{s+i-1} P_{s+i}}.\theta, \epsilon)\ne \phi$ and $\vv{P_sP_{s+k}}.\theta$ falls in the \emph{common subinterval}, then the angular difference between $\vv{P_{s+i-1}P_{s+i}}$ and $\vv{P_sP_{s+k}}$ for all $i\in [1, k]$ is not larger than $\epsilon$.
%
