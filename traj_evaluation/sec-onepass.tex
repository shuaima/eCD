...
{Direction range intersection} \cite{Long:Direction} approaches (\eg\ \intersec \cite{Long:Direction} and \interval \cite{Ke:Interval}) are somehow like the \emph{sector intersection} methods except that they are designed for \dad,
\textcolor{blue}{we choose both of them as the representative of one-pass algorithms using \dad.}
...
\stitle{Algorithm \intersec\cite{Long:Direction} and \interval\cite{Ke:Interval} Using \dad.}
