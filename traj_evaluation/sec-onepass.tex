...
One-pass algorithms adopts local checking policies, and are essentially designed for a specific distance metric. 
...
%
There is also algorithm \siped (sector intersection), {\em completely overlooked} by existing trajectory compression studies,  that can be easily adopted for trajectory compression, as it is originally developed in fields of computational geometry and pattern recognition~\cite{Williams:Longest,Sklansky:Cone,Dunham:Cone, Zhao:Sleeve}.
These algorithms are designed for \ped. Algorithms \operb and \siped have good compression ratios, and, hence, we choose them as the as the representatives of  one-pass algorithms using \ped.
...
These one-pass algorithms are designed for \sed. As algorithm \cised has a compression ratio close to algorithm \dpa using \sed, we choose them as the as the representative of  one-pass algorithms using \sed.



In addition, the \emph{direction range intersection} \cite{Long:Direction} approach (algorithms \intersec \cite{Long:Direction} and \interval \cite{Ke:Interval}) is somehow like the \emph{sector intersection} method except that it is specifically designed for \dad. As algorithm \interval is an extension of \intersec, we choose \interval as the representative of  one-pass algorithms using \dad.
...
\stitle{Algorithm \siped Using \ped}.
