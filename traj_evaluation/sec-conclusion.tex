...
%
In case average errors are concerned, then {online algorithm \dagots}~and batch algorithms are good candidates as one-pass algorithms and online algorithms \opwa and \bqsa all have relatively large average errors. % and algorithm \squishe has really limited compression ratios
%
...
\stitle{(2) Choice of distance metrics}. Users essentially choose a distance metric of \ped, \sed and \dad based on the needs of applications, \eg~\sed is the only distance metric that is query friendly {\wrt~the \emph{where\_at}, \emph{range} and \emph{nearest\_neighbor} queries. Hence, it is the best choice for such applications, and all of them are not friendly \wrt the \emph{when\_at} query.}
%
