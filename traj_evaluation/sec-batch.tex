...
Global checking policies enforce batch algorithms to have an entire raw trajectory first \cite{Meratnia:Spatiotemporal}.
...
\stitle{Douglas-Peucker Algorithm (\dpa) \cite{Douglas:Peucker} }. It is invented for reducing the number of points required to represent a digitized line or its caricature in the context of computer graphics and image processing.
...
The basic \dpa uses \ped, however, it also supports \sed \cite{Meratnia:Spatiotemporal} that runs in the same routine as using \ped. %, except that $ped$ in Figure~\ref{alg:dp} is replace by $sed$.
...
\stitle{Theo-Pavlidis Algorithm (\tpa) ~\cite{Pavlidis:Segment}}. It initially employs the global checking policy to output disjoint line segments, and we slightly modify it to have continuous line segments.
