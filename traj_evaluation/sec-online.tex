...
{Online algorithms enforces the constrained global checking policy}.
The \emph{constrained global checking} algorithms restrict the checking within a window, sliding or opening, hence, they are online algorithms.
...
Several line simplification algorithms have been developed \eg by combining \dpa or \tpa with sliding or opening windows for online processing\cite{Meratnia:Spatiotemporal, Keogh:online}.
...
%
\bqsa \cite{Liu:BQS} and \squishe\cite{Muckell:Compression} further optimize these online algorithms.
\bqsa \cite{Liu:BQS} fasts the process by using a convex hull while \squishe\cite{Muckell:Compression} uses a data structure named priority queue.
%uses a new distance checking method by picking out at most eight special points from an open window based on
...
\textcolor[rgb]{1.00,0.00,0.00}{\subsubsection{Sliding Window and Bottom-up}}
