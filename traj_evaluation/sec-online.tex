...
The \bqsa algorithm \cite{Liu:BQS} is essentially an opening window and top-down algorithm\cite{Meratnia:Spatiotemporal}. It reduces the running time by introducing \textit{convex hulls} to pick out a certain number of points. This strategy makes \bqsa be \ped specific.
...
It is worth pointing out that its simply version, \fbqsa, essentially avoids case (3) to fast the process. \fbqsa has already been evaluated in our preview work \cite{Lin:Operb}.
