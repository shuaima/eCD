...
\textcolor{blue}{It is a directed acyclic graph based online trajectory simplification method that supports \lissed. Other than optimal algorithms and other near optimal algorithms \cite{Chen:Fast,Daescu:metric} that first construct the reachability graph then find a shortest path from it, \dagots determines part of the shortest path when constructing preceding part of the whole graph \cite{Cao:Dots}.}
%, \ie it runs in a recursive manner.
%before the whole graph is completely constructed, 

It first shows that in this problem, the reachability graph of an input trajectory finally retrogrades to a tree structure. It also observes that the local approximation error $lissed(P_s,P_{s+k})$ is growing with respect to $k$, thus, it is reasonable to assume that $k$ should not be too large. 
...
%
Then, let $c>0$ and $Q_c$ is the current layer under constructing, 
...
\begin{example}
	...
	%
	(b) \textcolor{blue}{Then, points $P_1$, $P_2$ and $P_3$ are in turn added to the second layer $Q_1$. Because $P_0$ does not have any new child point, \dagots updates the status from $Q_0$ to $Q_1$, and outputs $P_0$ as it is the only \emph{alive} point in $Q_0$.}
	%
	(c) \textcolor{blue}{$P_4$ and $P_5$ are added to the next layer, $Q_2$. After that, $Q_1$ does not have new descendants, hence, status of $Q_1$ and $Q_2$ is updated. The parent of $P_4$ is adjusted to $P_3$ as path [$P_0$, $P_3$, $P_4$] has smaller total integral square \sed. Besides, $P_1$ and $P_2$ are marked as \emph{dead} as they do not have any descendant, and $P_3$ is output as it is the only \emph{alive} element in layer $Q_1$.}
	%
	(d) \textcolor{blue}{The process repeat. Finally, it outputs 4 line segments $\vv{P_0P_3},\vv{P_3P_5},\vv{P_5P_7}$ and $\vv{P_7P_{10}}$.}
\end{example}
