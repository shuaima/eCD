...
Though \ldr is originally designed to track the position of a moving object, Authors in \cite{Trajcevski:LDRH} proved that \ldr with some small variations, named linear dead reckoning with half $\epsilon$ (\ldrh) in \cite{Lange:Tracking}, is also a line simplification algorithm such that it can be used to compress trajectories, \ie~\ldrh supports both position tracking and trajectory simplification, thus, it is a trajectory tracking algorithm.
%
...
\subsection{Trajectory simplification in circular areas}
Recently, a one-pass trajectory simplification algorithm using \sed, namely \cised \cite{Lin:Cised} is developed. It uses a local synchronous distance checking approach based on a concept of spatio-temporal cone, defined in a 3D Cartesian coordinate system whose $x$-axis, $y$-axis and $t$-axis are longitude, latitude and time, respectively, that converts the \sed distance tolerance into the intersection of cones for testing the successive points such that it is possible to achieve a linear time complexity.
...
Proposition \ref{theo-half-cone} serves as the foundation of the one-pass trajectory simplification algorithm \cised~\cite{Lin:Cised}, and for efficiency consideration, \cised projects those cones on a plane,\eg plane $t=P_{s+1}.t$, so as to convert the checking of cones intersection into a much simpler way, \ie checking of circles intersection on the plane.
