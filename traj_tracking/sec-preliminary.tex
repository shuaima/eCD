...
\begin{figure}[tb!]
	...
	\vspace{-1ex}
	\caption{\small A running example of trajectory tracking by \ldrh taking the same trajectory of Figure \ref{fig:ldr} as input. Initially, point $P_0$ is set as $P_s$ and $\vec{v}$ is set to $\vec{v}_0$. Then, (1) points $P_1$ and $P_2$ have \sed distances less than $\epsilon/2$ to their expected points $P'_1$ and $P'_2$, respectively, (2) $|P_3P'_3| > \epsilon/2$, thus, a new update is triggered and $P_2$ is the new start point of the next section of trajectory tracking. Finally, this algorithm in turn sends 5 points, $P_0, P_2, P_4, P_6$ and $P_8$, and 5 velocities, $\vec{v_1}$, $\vec{v_2}$, $\vec{v_3}$, $\vec{v_4}$ and $\vec{v_5}$ (not shown), to the MOD server.}
	\vspace{-2ex}
	\label{fig:ldrh}
\end{figure}
