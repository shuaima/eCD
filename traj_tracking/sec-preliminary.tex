...
\begin{example}
	...
	%
	(2) a set of two continuous line segments $\{\vv{P_0P_4}$, $\vv{P_4P_{8}}$\} is a piece-wise line representation of trajectory $\dddot{\mathcal{T}}$,
	%
	(3) $ped\left(P_4, \vv{P_0P_{8}}\right)=|\vv{P_4P^*_4}|$, where $P^*_4$ is the perpendicular point of $P_4$ \wrt line segment $\vv{P_0P_{8}}$, and 
	%
	(4) $sed\left(P_4, \vv{P_0P_{8}}\right)= |\vv{P_4P'_4}|$,
	where $P'_4$ is the synchronized point of $P_4$ \wrt $\vv{P_0P_{8}}$ satisfies $\frac{|\vv{P_0P'_4}|}{|\vv{P_0P_{8}}|} = \frac{P_4.t - P_0.t}{P_{8}.t-P_0.t} = \frac{4-0}{8-0}= \frac{1}{2}$.
\end{example}
...
\begin{figure}[tb!]
	\centering
	\includegraphics[scale=0.9]{figures/Fig-LDRH.png}
	\vspace{-1ex}
	\caption{\small A running example of linear dead reckoning with half $\epsilon$ (LDRH) taking the same trajectory as input. Initially, point $P_0$ is set as $P_s$ and $\vec{v}$ is set to $\vec{v}_0$. Then, (1) points $P_1$ and $P_2$ have \sed distances less than $\epsilon/2$ to their expected points $P'_1$ and $P'_2$, respectively, (2) $|P_3P'_3| > \epsilon/2$, thus, a new update is triggered and $P_2$ is the new start point of the next section of trajectory tracking. Finally, this algorithm in turn sends 5 points, $P_0, P_2, P_4, P_6$ and $P_8$, and 4 velocities, $\vec{v_0}$, $\vec{v_1}$, $\vec{v_2}$ and $\vec{v_3}$, to the MOD server.}
	\vspace{-2ex}
	\label{fig:ldrh}
\end{figure}
