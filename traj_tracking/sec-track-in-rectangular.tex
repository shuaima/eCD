...
\label{sec:rectangle}
Though it is possible to do a tracking in a rectangular area, it is really hard to design such a shape without introducing a new distance metric, and at the same time develop such an efficient algorithm. Hence, we alternatively define a rectangle-like area that subtly makes use of \ped and \sed, and develop such an effective and efficient trajectory tracking algorithm. 
