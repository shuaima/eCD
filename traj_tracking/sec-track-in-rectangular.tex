...
%
(2) if $\epsilon_{ped} << \epsilon_{sed}$, then its effect is actually approximate to a \ped. Through \ped is not seemed in position and trajectory tracking, it is familiar in trajectory simplification that always ensures that the original data points live in strip-like areas built around the simplified trajectory. Otherwise, 
%
