...
\subsection{The basic tracking approaches}
By using \bed, we are able to track the position of a moving object and simplify its trajectory in a rectangular or rectangle-like area, where the position tracking algorithm is the modified \ldr (mLDR in short) and the trajectory simplification algorithm is the modified open/sliding window approach \cite{Meratnia:Spatiotemporal} (mOPW in short), in which the \sed used in these algorithms is replaced by \bed, either (\sed, \ped) or (\red, \ped). 
%
The mLDR and mOPW could also be integrated into \grts~\cite{Lange:GRTS}, \ie~\grts using \bed, such that we can track a trajectory in a rectangular or rectangle-like area in one framework.
%
Due to the effectiveness beneficial of mOPW, the \grts using \bed should also be effective in terms of compression ratio. Meanwhile, it is not efficient or space-saving enough. 

We next try to develop effective and efficient trajectory tracking algorithms. We finally find such an approach for rectangle-like areas, while much work is still needed for rectangular areas.
...
Proposition \ref{theo-full-sector} tells that the full-$\epsilon$ sector approach with constrains that (1) $\overline{P_sP_{s+k}}$ lives in the common intersection of the preview full-$\epsilon$ sectors and (2) $|P_sP_{s+k}|$ is longer than $\sqrt{l_{m}^2 -\epsilon^2}$, is applicable to simplify and track a moving object in a strip area. The full-$\epsilon$ sector and $|P_sP_{s+k}| \ge \sqrt{l_{m}^2 - \epsilon^2}$ are looser constraints than the half-$\epsilon$ sector and $|P_sP_{s+k}| \ge l_{m}$ used in \cite{Williams:Longest, Sklansky:Cone,Zhao:Sleeve}, hence, this new approach is sure of bringing a better compression ratio.
...
Next, by combining cones and sectors, we are able to efficiently track trajectory in rectangle-like areas.
...
\subsection{Algorithm of tracking in rectangle-like areas}
