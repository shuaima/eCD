...
By this way, a line segment of \citt is potential to represent a longer sub-trajectory compared with \ldrh, therefore the storage and network bandwidth are saved.
...
The correctness of \citt follows from Propositions \ref{theo-ldrh-cised} and \ref{theo-full-cone}.
It is easy to find that every data point is processed only once in \citt, and for each point, it needs $O(1)$ time as the process of getRPolygon() \cite{Lin:Cised}, the intersecting of polygons \cite{Lin:Cised} and {the other operations} all have a time complexity of $O(1)$. Hence, \citt has a time complexity of $O(n)$, where $n$ is the number of data points.
