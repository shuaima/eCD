		...
		{\small
			\begin{minipage}{3.3in}
				...
				\mat{0ex}{
					...
					\icc \>\hspace{3ex} else if $\vv{P_sP_{i}}$ ~passes through~ $\mathcal{S}^*$ ~and~ $|P_sP_{i}| \ge l_{max} - \epsilon$ \\ \hspace{9ex} ~and~ $ped(P_i, \vv{v}) \ge \epsilon $ then  \\ %~// updates velocity only 
...
\todo{algorithm}. section intersection based trajectory tracking (SITT), similar as CITT, fig.
