...
The half-$\epsilon$ cone in the above is still a little conservative. Fortunately, it can be extended to a full-$\epsilon$ cone with some constrains to get a better effectiveness.
...
Theorem \ref{theo-full-cone} tells that a full-$\epsilon$ cone can also be used in trajectory simplification or tracking. 

\begin{theorem}
	\label{theo-cone-vs}
	Given a sub-trajectory $[P_s,...,P_{s+k}]$ and an error bound $\epsilon$, if $\bigsqcap_{i=1}^{k}$\cone{(P_s, P_{s+i}, \epsilon/2)} $\ne \{P_s\}$, then $P_{s+i} \in \bigsqcap_{i=1}^{k-1}$\cone{(P_s, P_{s+i}, \epsilon)}; and the opposite is not always true.
\end{theorem}
...
Theorem \ref{theo-full-cone} tells that the full-$\epsilon$ cone always has a better effectiveness than the half-$\epsilon$ cone.
