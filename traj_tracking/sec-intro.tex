...
Secondly, the current works ensure that the moving object at time $t$ is located in a circular area taking the expected position of the object at that time as its center and an error bound $\epsilon$ as its radius, \ie they track a moving object in a floating \emph{circular area} as shown in Figure \ref{fig:areas}-(1). 
%
However, in practical, there is a need of tracking a moving object in other areas, such as a \emph{rectangular} or \emph{rectangle-like area}, \eg suppose a school boy (or a old man) is going home along a straight road. For safety consideration, he is suggested to walk freely as long as he does not deviate too far to the road, \ie he is expected to have a larger radial deviation and a smaller perpendicular deviation, meaning he is moving in a floating \emph{rectangular area} (Figure \ref{fig:areas}-(2)) or a \emph{rectangle-like area} (Figure \ref{fig:areas}-(3)).
...
%\eg a school boy is expected going home on some straight roads that his perpendicular deviation to a road is strict restricted, while his horizontal deviation on the road is not restricted (meaning tracking him in a \emph{strip area} as shown in Figure \ref{fig:areas}-(2). The term \emph{strip} and its slight variation are also called ``infinite beam'' \cite{Chen:Space,Daescu:metric} and  ``tolerance-zone'' \cite{Imai:Optimal} in the field of computational geometry) or is restricted (meaning tracking him in a floating rectangular or rectangle-like area as shown in Figure \ref{fig:areas}-(3)). 
...
\begin{figure}[tb!]
	...
	\vspace{-1ex}
	\caption{\small  Trajectory tracking in (1) a circular area, (2) a rectangular area, and (3) a rectangle-like area. Where $P_s$ is the start position of the sub-trajectory and $P'$ is the expected position of the moving object whose actually position at that time is $P$.}
	\vspace{-1ex}
	\label{fig:areas}
\end{figure}

\stitle{{Contributions}.}
To the end, we explore ways to track a moving object in circular, rectangular and rectangle-like areas, and design two novel one-pass trajectory tracking algorithms that effectively and efficiently run for circular and rectangle-like areas, respectively. 

\ni (1) We develop a one-pass trajectory tracking algorithm \citt and theoretically prove that it has better effectiveness than \ldrh. Thus, we are able to effectively and efficiently track a moving object in a circular area (Section \ref{sec:circle}).
...
\ni (2) We provide a convenient way to design new tracking shapes, \eg a rectangle-like area, and develop a one-pass trajectory tracking algorithm, \ie~\bitt based on the intersections of sectors and spatio-temporal cones, such that we are able to effectively and efficiently track a moving object in a rectangle-like area (Section \ref{sec:rectangle}). 
...
\ni (3) Using three real-life trajectory datasets (ServiceCar, GeoLife, Mopsi), we finally conduct an extensive experimental study that compares our methods \citt and \bitt with representative trajectory tracking algorithms \ldrh (the first and the most efficient trajectory tracking algorithm) and \grts (the most effective trajectory tracking algorithm). The experimental results show that our approaches are both efficient and effective that outperform \ldrh and \grts, and are feasible to track a moving object in circular and rectangle-like areas (Section \ref{sec-exp}).
