...
The one-pass trajectory simplification algorithm using \sed, namely \cised \cite{Lin:Cised}, uses a local synchronous distance checking approach based on a concept of spatio-temporal cone, defined in a 3D Cartesian coordinate system whose $x$-axis, $y$-axis and $t$-axis are longitude, latitude and time, respectively, that converts the \sed distance tolerance into the intersection of cones for testing the successive points to achieve a linear time.
			...
			{\small
				\begin{minipage}{3.3in}
					...
					\mat{0ex}{
						...
						\icc \>\hspace{3ex} if $\vv{P_sP_{i}}$ ~does not pass~ $\mathcal{R}^*$ then \\ % // updates velocity and location \\
						...
						\icc \>\hspace{3ex} else if $\vv{P_sP_{i}}$ ~passes~ $\mathcal{R}^*$ ~and~ $sed(P_{i},\vv{v}) \ge \epsilon $ then  \\ %~// updates velocity only 
