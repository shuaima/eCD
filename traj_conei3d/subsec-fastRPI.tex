...
Let $\vv{A} = (P_{s_A}, P_{e_A})$ and $\vv{B} = (P_{s_B}, P_{e_B})$  be two directed edges on polygons $\mathcal{G}_{s+l+1}$ and $\mathcal{G}^*_{s+l}$, respectively.
...
\begin{proof}
We first explain how the edges $\vv{A}$ is moved forward.
Indeed, $\vv{A}$ is moved from its original position to its opposite edge on $\mathcal{G}_{s+l+1}$ \wrt $\vv{B}$.
...
Here, the label of edge $\vv{A}$ is changed to $5$, the opposite of $1$ \wrt edge $\vv{B}$ labeled with $3$ on $\mathcal{G}^*_{s+l}$.
...
\begin{proof}
We first explain how the edges $\vv{B}$ is moved forward.
...
$\vv{B}$ moves to the edge labeled with 8 on $\mathcal{G}^*_{s+l}$, which is the next of the edge labeled with 7 on $\mathcal{G}^*_{s+l}$.
Note that if the edge labeled with 8 were not actually existing in intersection polygon $\mathcal{G}^*_{s+l}$, then $\vv{B}$ should repeat move on until it reaches the first ``real" edge on $\mathcal{G}^*_{s+l}$.

We then present the proof.
If ($\vv{A} \bigsqcap \vv{B} \ne \emptyset$ \And $\vv{A} \times \vv{B} > 0$ \And $P_{e_B} \not \in \mathcal{H}(\vv{A})$) or ($\vv{A} \bigsqcap \vv{B} \ne \emptyset$ \And $\vv{A} \times \vv{B} < 0$ \And $P_{e_A} \in \mathcal{H}(\vv{B})$), then it is also easy to find that, for each position of $\vv{B}$ between its original to its target positions, we have (1) $\vv{A} \bigsqcap \vv{B} = \emptyset$, and (2) either $P_{e_B} \not \in \mathcal{H}(\vv{A})$ or $P_{e_A} \in \mathcal{H}(\vv{B})$. Hence, by the advance rule (2) of algorithm \cpia in Section~\ref{subsec-cpi}, edge $\vv{B}$ is always moved forward until it reaches the target position. From this, we have the conclusion.
\end{proof}
...
Given intersection polygon $\mathcal{G}^*_{s+l}$ of the preview $l$ polygons and the next approximate polygon $\mathcal{G}_{s+l+1}$, the algorithm \rpia returns $\mathcal{G}^*_{s+l+1} = \mathcal{G}^*_{s+l}  \bigcap \mathcal{G}_{s+l+1}$.
%The sketch of the algorithm is shown in Figure~\ref{alg:r-poly-inter}.
It runs the similar routine as the \cpia algorithm, except that (1) it saves polygons in arrays, and (2) the advance strategies are partitioned into two parts, \ie $\vv{A} \bigcap \vv{B} \ne \phi$ and $\vv{A} \bigcap \vv{B} = \phi$, where the former applies Proposition \ref{prop-rule1} and \ref{prop-rule2}, and the later remains the same as the \cpia algorithm.
...
Algorithm \emph{regularPolyInter} runs in the same routine as algorithm \cpia, except that it fastens the advancing speed of directed edges $\vv{A}$ and $\vv{B}$ in some circumstances as shown in Proposition \ref{prop-rule1} and \ref{prop-rule2}. Given $\mathcal{G}^*_{s+l}$ and $\mathcal{G}_{s+l+1}$,  Proposition \ref{prop-rule1} and \ref{prop-rule2} prove that it has the same result as the output of {\cpia}, \ie $\mathcal{G}^*_{s+l+1}$. This ensure the correctness of \rpia.
