...
while the inscribed regular polygons $\mathcal{G}_{s+i}$ ($i\in[1, k]$) the cone projection  circles are constructed in a unified way,
while allows us to develop a fast method to compute their intersection.

Let $\vv{A} = (P_{s_A}, P_{e_A})$ and $\vv{B} = (P_{s_B}, P_{e_B})$  be two directed edges on polygons $\mathcal{G}_{l+1}$ and $\mathcal{G}^*_l$, respectively.
Note that $\vv{A}$ and $\vv{B}$ are advanced step by step each time by the two advancing rules of algorithm \cpia.
%
...
\small
    \hspace{2ex} s =  \left\{
    \begin{aligned}
        & 2(g(\vv{A}) - g(\vv{B}))  \hspace{5ex}~if  ~g(\vv{A}) > g(\vv{B}) \\
        & {1}              \hspace{18ex}~if  ~g(\vv{A}) = g(\vv{B}) \\
        & 2(m+g(\vv{A}) - g(\vv{B})) ~if  ~g(\vv{A}) < g(\vv{B}), \\
    \end{aligned}
    \right.       \hspace{6ex}{}
\end{equation*}
...
\begin{proof}
We first explain how the edges $\vv{A}$ and $\vv{B}$ are moved forward.
Indeed, $\vv{A}$ is moved from its original position to its opposite edge on $\mathcal{G}_{l+1}$ \wrt $\vv{B}$.
For example, in Figure~\ref{fig:r-poly-rule1}.(1), there is $\vv{A} \bigsqcap \vv{B} \ne \emptyset$ \And $\vv{A} \times \vv{B} > 0$ \And $P_{e_B} \in \mathcal{H}(\vv{A})$, hence $\vv{A}$ moves on. As $g(\vv{A})=2 < 8=g(\vv{A})$, $\vv{A}$ moves forward $2(m+g(\vv{A}) - g(\vv{A})) = 2(8+2-8)= 4$ steps. 
Here, the label of edge $\vv{A}$ is changed to $6$, the opposite of $2$ \wrt edge $\vv{B}$ labeled with $8$ on $\mathcal{G}^*_l$.

We then present the proof.
If ($\vv{A} \bigsqcap \vv{B} \ne \emptyset$ \And $\vv{A} \times \vv{B} < 0$ \And $P_{e_A} \not \in \mathcal{H}(\vv{B})$) or ($\vv{A} \bigsqcap \vv{B} \ne \emptyset$ \And $\vv{A} \times \vv{B} > 0$ \And $P_{e_B} \in \mathcal{H}(\vv{A})$), then as all edges in the same edge groups $E^j$ ($1\le j\le m$) are in parallel with each other, it is easy to find that, for each position of $\vv{A}$ between its original to its opposite positions, we have $\vv{A} \bigsqcap \vv{B} = \emptyset$ and $P_{e_A} \not \in \mathcal{H}(\vv{B})$ and $P_{e_B} \in \mathcal{H}(\vv{A})$. Hence, by the advance rule (1) of algorithm \cpia in Section~\ref{subsec-cpi}, edge $\vv{A}$ is always moved forward until it reaches the opposite position of its original one. From this, we have the conclusion.
\end{proof}
...
The presented regular polygon intersection algorithm, \ie \rpia, is the optimization of the convex polygon intersection algorithm, by Theorem \ref{prop-rule1} and \ref{prop-rule2}. We also saves vertexes or edges of a polygon in an fixe size array, which is different with algorithm \cpia  that saves polygons in linked lists.
...
Algorithm \emph{regularPolyInter} runs in the same routine as algorithm \cpia, except that it fastens the advancing speed of directed edges $\vv{A}$ and $\vv{B}$ in some circumstances as shown in Theorem \ref{prop-rule1} and \ref{prop-rule2}. Given $\mathcal{G}^*_k$ and $\mathcal{G}_{k+1}$,  Theorem \ref{prop-rule1} and \ref{prop-rule2} prove that it has the same result as the output of \emph{convexPolyInter}, \ie $\mathcal{G}^*_{k+1}$. This ensure the correctness of \emph{regularPolyInter}.
