...
Algorithm \emph{regularPolyInter} runs in the same routine as algorithm \cpia, except that it fastens the advancing speed of directed edges $\vv{A}$ and $\vv{B}$ in some circumstances as shown in Proposition \ref{prop-rule1} and \ref{prop-rule2}. Given $\mathcal{G}^*_{l}$ and $\mathcal{G}_{s+l+1}$,  Proposition \ref{prop-rule1} and \ref{prop-rule2} prove that it has the same result as the output of {\cpia}, \ie $\mathcal{G}^*_{l+1}$. This ensure the correctness of \rpia.
