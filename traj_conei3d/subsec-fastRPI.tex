...
Note that if the edge labeled with 8 were not actually existing in the intersection polygon $\mathcal{G}^*_{s+l}$, then $\vv{B}$ should repeatedly move on until it reaches the first ``real" edge on $\mathcal{G}^*_{s+l}$.
...
Given intersection polygon $\mathcal{G}^*_{s+l}$ of the preview $l$ polygons and the next approximate polygon $\mathcal{G}_{s+l+1}$, the algorithm \rpia returns $\mathcal{G}^*_{s+l+1} = \mathcal{G}^*_{s+l}  \bigsqcap \mathcal{G}_{s+l+1}$.
%The sketch of the algorithm is shown in Figure~\ref{alg:r-poly-inter}.
It runs the similar routine as the \cpia algorithm, except that (1) it saves polygons in arrays, and (2) the advance strategies are partitioned into two parts, \ie $\vv{A} \bigsqcap \vv{B} \ne \emptyset$ and $\vv{A} \bigsqcap \vv{B} = \emptyset$, where the former applies Proposition \ref{prop-rule1} and \ref{prop-rule2}, and the later remains the same as the \cpia algorithm.
