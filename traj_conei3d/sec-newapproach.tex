...
\stitle{Cone projection}. The projection of a cone \cone{(P_s, \mathcal{O}(P_i, \epsilon))} of $P_i(x_i, y_i, t_i)$ \wrt $P_s(x_s, y_s, t_s)$ on a plane $t = t_c > t_s$ is a circle \circle{(P^c_i, r^c_i)} such that (1)  $P^c_i.x = x_s +  \frac{t_c - t_s}{t_i - t_s}(x_i - x_s)$, (2)  $P^c_i.y = y_s +  \frac{t_c - t_s}{t_i - t_s}(y_i - y_s)$, (3)   $P^c_i.t = t_c$ and (4) $r^c_i =\frac{t_c-t_s}{t_i-t_s}{\epsilon}$.
