...
The distance from point $P_i$ to synchronized point $P'_i$, $|P_iP'_i|$, is the \sed of $P_i$ \wrt $\vv{P_sP_e}$.
...
Note that in this definition, the synchronous circle is only defined by the central point $P$ and a constant $\epsilon$. Indeed, it is nothing to do with the start point $P_s$ and the end point $P_e$.
