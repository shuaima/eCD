...
Consider a sub-trajectory $[P_s, \ldots, P_e]$, an error bound $\epsilon$, and a 3D coordinate system whose origin, $x$-axis, $y$-axis and $t$-axis  are $P_s$, longitude, latitude and time, respectively, as shown in Figure~\ref{fig:sed3d}.



\stitle{Synchronous Circles (\circle{}}. The synchronous circle of a point $P_i(x_i, y_i, t_i)$, denoted as \circle{(P_i, \epsilon)}, is a circle on the plane $t=t_i$ that takes $P_i$ as its center and $\epsilon$ as its radius.


It is easy to know that for any point $P'_i$ in the area of a circle \circle{(P_i, \epsilon)}, its distance to $P_i$ is less than $\epsilon$, 
and Figure~\ref{fig:sed3d} illustrates a synchronous circle \circle{(P_i, \epsilon)} of point $P_i$.
...
\stitle{Spatio-temporal cones (\cone{})}. Given a sub-trajectory $\{P_s,\ldots,$ $P_e\}$ and an error bound $\epsilon$, the spatio-temporal cone of point $P_i$ \wrt point $P_s$, denoted as \cone{(P_s, \mathcal{O}(P_i, \epsilon))}, is an oblique circular cone that takes the start point $P_s$ as its apex and the synchronous circle $\mathcal{O}(P_i, \epsilon)$ as its base.

Two spatio-temporal cones \cone{(P_s, \mathcal{O}(P_i, \epsilon))} and \cone{(P_s, \mathcal{O}(P_{i+1}, \epsilon))} are illustrated in Figure~\ref{fig:cis}. 

\stitle{Cone projection}. The projection of a cone \cone{(P_s, \mathcal{O}(P_i, \epsilon))} of $P_i(x_i, y_i, t_i)$ \wrt $P_s(x_s, y_s, t_s)$ on a plane $t = t_c > t_s$ is a circle \circle{(P^c_i, r^c_i)} such that $P^c_i(x^c_i, y^c_i, t_c)$ and $r^c_i$ satisfy:

\ni(1)  $x^c_i = x_s +  \frac{t_c - t_s}{t_i - t_s}(x_i - x_s)$

\ni(2)  $y^c_i = y_s +  \frac{t_c - t_s}{t_i - t_s}(y_i - y_s)$, and

\ni(3)  $r^c_i =\frac{t_c-t_s}{t_i-t_s}{\epsilon}$ 





The red dashed circle $\mathcal{O}(P^i_{i+1}, r^i_{i+1})$ on plane $t=t_i$ in Figure~\ref{fig:cis} is the projection of cone \cone{(P_s, \mathcal{O}(P_i, \epsilon))}.
...
Given a sub-trajectory $[P_s, \ldots, P_e]$, the intersection point $P'_i$ of the directed line segment $\vv{P_sP_e}$ and the plane $t=t_i$ is the synchronized data point of $P_i(x_i, y_i, t_i)$ ($s<i<e$) \wrt  $\vv{P_sP_e}$,
and the distance $|\vv{P_iP'_i}|$ from $P_i$ to $P'_i$ is the \sed distance of $P_i$ to $\vv{P_sP_e}$.
...
\subsection{Approximating Circles with Polygons}
