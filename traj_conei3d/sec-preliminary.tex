...
\begin{table}
	...
	\begin{tabular}{|c|l|}
		...
		\hline
		$\overline{\mathcal{T}}$&  \scriptsize {a piece-wise line representation of a trajectory}\\
		\hline
		$ped(P, \mathcal{L})$ & \scriptsize {the perpendicular Euclidean distance of $P$ to $\mathcal{L}$}\\
		\hline
		$sed(P, \mathcal{L})$ & \scriptsize {the synchronous Euclidean distance of $P$ to $\mathcal{L}$}\\
		\hline
...
%
Next, the algorithm handles three special cases of the two polygons, \ie $\mathcal{G}_1$ is inside of $\mathcal{G}_2$, $\mathcal{G}_2$ is inside of $\mathcal{G}_1$, and $\mathcal{G}_1 \bigsqcap \mathcal{G}_2 = \emptyset$ (line 13).
%
...
\mat{0ex}{
	...
	\bcc \hspace{1ex}\=  \SET $\vv{A}$ and $\vv{B}$ {arbitrarily} on $\mathcal{G}_2$ and $\mathcal{G}_1$, respectively;\\
