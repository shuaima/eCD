...
Given a trajectory \trajec{T}${[P_0, \ldots, P_n]}$ and an error bound $\epsilon$, the \textcolor{blue}{compression optimal (not necessary error optimal)} trajectory simplification problem, as formulated by Imai and Iri in \cite{Imai:Optimal}, can be solved in two steps: (1) construct a reachability graph $G$ of \trajec{T}, and (2) search a shortest path from $P_0$ to $P_{n}$ in graph $G$.
