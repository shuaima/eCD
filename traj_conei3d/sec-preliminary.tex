...
\textcolor{blue}{Synchronized points are virtual points based on an assumption that an object moved from $P_s$ to $P_e$ with a uniform speed, \ie the average speed $\frac{|\vv{P_sP_e}|}{P_e.t-P_s.t}$ between points $P_s$ and $P_e$. Then the \emph{synchronized data point} $P'$ of a point $P$ at time $P.t$ \wrt the line segment $\vv{P_sP_e}$ is the position of the moving object on $\vv{P_sP_e}$ at time $P.t$. More specifically, a synchronized point $P'_i$ of $P_i$ ($s<i<e$) is a point on $\vv{P_sP_{e}}$ satisfying ${|\vv{P_sP'_i}|} = \frac{P_i.t - P_s.t}{P_e.t - P_s.t}\cdot {|\vv{P_sP_e}|}$, which means that the object moved from $P_s$ to $P_e$ at an average speed $\frac{|\vv{P_sP_e}|}{P_e.t-P_s.t}$ using time of ${(P_i.t - P_s.t)}$, and its position at time $P_i.t$ were point $P'_i$ on $\overrightarrow{P_sP_{e}}$ having a distance of $\frac{P_i.t - P_s.t}{P_e.t - P_s.t}\cdot|\vv{P_sP_e}|$ to point $P_s$~\cite{Meratnia:Spatiotemporal, Chen:Fast, Zhang:Evaluation}.}
