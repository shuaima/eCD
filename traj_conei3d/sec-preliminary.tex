...
we say that algorithm $\mathcal{A}$ is error bounded by $\zeta$ if  for each point $P$ in \trajec{T}, there exists a point $P_j$ in \trajec{T'} such that the  $sed(P, \mathcal{L}(P_j,P_{j+1}))\le \zeta$ (respectively $ped(P, \mathcal{L}(P_j,P_{j+1}))\le \zeta$).
...
We also employ a convex polygon intersection algorithm ~\cite{ORourke:Intersection}, whose basic idea is straightforward.
...
(1) If $\vv{A} \times \vv{B} < 0$ and $P_{e_A} \not \in \mathcal{H}(\vv{B})$, or $\vv{A} \times \vv{B} > 0$ and $P_{e_B} \in \mathcal{H}(\vv{A})$, then $\vv{A}$ is advanced a step. For examples, in Figure~\ref{fig:c-poly-inter}.(1) and ~\ref{fig:c-poly-inter}.(2), $\vv{A} \times \vv{B} > 0$ and $P_{e_B} \in \mathcal{H}(\vv{A})$, hence, $\vv{A}$ moves forward a step.

(2) If $\vv{A} \times \vv{B} > 0$ and $P_{e_B} \not \in \mathcal{H}(\vv{A})$, or $\vv{A} \times \vv{B} < 0$ and $P_{e_A} \in \mathcal{H}(\vv{B})$, then  $\vv{B}$ is advanced a step. For examples, in Figure~\ref{fig:c-poly-inter}.(3) and ~\ref{fig:c-poly-inter}.(4), $\vv{A} \times \vv{B} < 0$ and $P_{e_A} \in \mathcal{H}(\vv{B})$, hence, $\vv{B}$ moves forward a step.


The compplete convex polygon intersection algorithm ~\cite{ORourke:Intersection} is shown in Figure~\ref{alg:c-poly-inter}.
...
Initially, directed edges $\vv{A}$ and $\vv{B}$ are on polygons $\mathcal{G}_2$ and $\mathcal{G}_1$, respectively, such that $\vv{A} \bigsqcap \vv{B} = \{P_1\}$ by coincidence, \ie $\vv{A}$ and $\vv{B}$ intersect on point $P_1$, as shown in Figure~\ref{fig:c-poly-inter}.(1).
%
Then, by the advance rules, edge $\vv{A}$ moves on a step and makes $\vv{A} \bigsqcap \vv{B} = \emptyset$ as shown in Figure~\ref{fig:c-poly-inter}.(2).
...
%
Next, edge $\vv{B}$ moves on a step, and makes $\vv{A} \bigsqcap \vv{B} = \emptyset$, as shown in Figure~\ref{fig:c-poly-inter}.(4).
