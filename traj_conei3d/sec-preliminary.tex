...
We then describe the optimal \lsa algorithm and the sector intersection mechanism, and show how this mechanism can be used to fast the \lsa algorithms using \ped and why it cannot work with \sed.
...
Finding a shortest path takes $O(n)$ time in this case. Hence, the brute-force algorithm takes $O(n^3)$ time in total.

%
The brute-force algorithm is initially developed using \ped, and can be used for \sed as well.
As pointed out in \cite{Chan:Optimal}, the construction of the reachability graph $G$ using \ped can be implemented in $O(n^2)$ time with the help of the \textit{sector intersection} mechanism (see Section \ref{sub-ci-ped}). However, the sector intersection mechanism cannot work with \sed, hence, the construction of the reachability graph $G$ using \sed remains in $O(n^3)$ time.
