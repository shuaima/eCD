...
\textcolor{blue}{When a line segment $\mathcal{L}$ is in a \emph{x-y} 2D space, where $x$ and $y$ are the longitude and latitude, respectively}, we also use $|\mathcal{L}|$ and $\mathcal{L}.\theta\in [0, 2\pi)$ to denote the length of $\mathcal{L}$, and its angle with the $x$-axis of the coordinate system $(x, y)$.
