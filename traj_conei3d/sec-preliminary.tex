...
The brute-force algorithm of constructing the reachability graph $G$ is to check for each pair of points $P_s$ and $P_{s+k}$ whether the distance of  each $P_{s+i}$, $0<i<k$, to $\vv{P_sP_{s+k}})$ is less than $\epsilon$. 
...
%
Though the optimal algorithm is develop using \ped, it is also suitable for \sed. 
