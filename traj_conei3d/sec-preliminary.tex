...
\textcolor{blue}{
In this section, we first introduce basic concepts for trajectory compression.
We then describe the optimal \lsa algorithm and the sector intersection mechanism, and show how this mechanism can be used to fast the \lsa algorithms using \ped and why it can not work with \sed.
...
\stitle{Perpendicular Euclidean Distance (\ped)}. Given a data point $P$ and a directed line segment $\mathcal{L}$ = $\vv{P_{s}P_{e}}$, the perpendicular Euclidean distance (or simply perpendicular distance) $ped(P, \mathcal{L})$ of point $P$ to line segment $\mathcal{L}$ \textcolor{blue}{is $min\{|PQ|\}$ for any $Q\in \vv{P_{s}P_{e}}$.}
...
\subsection{\textcolor{blue}{The Optimal \lsa Algorithm}}
...
The reachability graph of a trajectory \trajec{T}${[P_0, \ldots, P_n]}$ \wrt an error bound $\epsilon$ is $G$ = (\trajec{T}, $E)$, where, for vertexes $P_s$ and $P_{s+k} \in$ \trajec{T} $(k>0)$, edge $(P_s, P_{s+k}) \in E$ if and only if the distance of each point $P_{s+i} (0<i<k)$ to line segment $\vv{P_sP_{s+k}}$ is no great than $\epsilon$.
%
Then in the reachability graph $G$, a path from point $P_0$ to point $P_{n}$ is a representation of trajectory \trajec{T}. The path also reveals the subset of points of \trajec{T} used in the approximate trajectory and the path length will correspond to the number of line segments in the approximation trajectory. 
Meanwhile, a shortest path is an optimal representation of trajectory \trajec{T}.

The brute-force algorithm of constructing the reachability graph $G$ is to check for each pair of points $P_s$ and $P_{s+k}$ whether the distance of each point $P_{s+i}$, $0<i<k$, to the line segment $\vv{P_sP_{s+k}}$ is less than $\epsilon$. 
...
Since finding a shortest path takes no more than $O(n^2)$ time, the brute-force algorithm takes $O(n^3)$ time.

%
The brute-force algorithm is initially developed using \ped, but it is also suitable for \sed. 
More specifically, the authors of \cite{Chan:Optimal} proved that the construction of the reachability graph $G$ using \ped could further be implemented in $O(n^2)$ time with the help of the \textit{sector intersection} mechanism (see Section \ref{sub-ci-ped}), however, the \textit{sector intersection} mechanism can not work with \sed, hence, currently the construction of the graph $G$ using \sed still needs $O(n^3)$ time.
...
%
\textcolor{blue}{It is also worth pointing out that if we use \sed instead of \ped then  ``the $k$ sectors \sector{(P_s, P_{s+i}, \epsilon)} ($i\in[1,k]$) sharing common data points other than $P_s$" would not anymore ensure ``there exists a data point $Q$ such that for any data point $P_{s+i}$ ($i \in [1, ... k]$), its synchronized Euclidean distance to directed line $\overline{P_sQ}$ is no greater than the error bound $\epsilon$". 
...
\begin{figure}[tb!]
	\centering
	\includegraphics[scale=0.86]{figures/Fig-convex-poly-inter.png}
%	\vspace{-1ex}
...
\sstab(5) The algorithm finally returns the intersection polygon as shown in Figure~\ref{fig:c-poly-inter}.(9).
\end{example}
