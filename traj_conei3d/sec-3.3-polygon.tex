...
\stitle{Saves polygon in array}.
Originally, polygons are saved in linked lists.
Considering the regular polygons each having a fixed number of edges, marked from $1$ to $M$, we save polygons in arrays. This policy alow us to address an edge or vertex by its index.
%, as well as avoid the creating and removing of link nodes.

%\stitle{Parallel of edges}.

\stitle{Advances multiple steps}.
