...
Observe that the algorithm of ~\cite{ORourke:Intersection} is for general convex polygons, while the approximate polygons are special polygons. We further develop an even fast polygon intersection method based on the properties of approximate polygons.
%
...
Originally, $A$ and $B$ are advanced step by step each time by the advancing rules of algorithm ~\cite{ORourke:Intersection}.
%
However, the new algorithm allows the algorithm to move on $A$ or $B$ multiple steps at one time.

The advancing rules of algorithm ~\cite{ORourke:Intersection} are indeed four categories, \ie $A \bigcap B \ne \phi$ and advances $A$, $A \bigcap B \ne \phi$ and advances $B$, $A \bigcap B = \phi$ and advances $A$, and $A \bigcap B = \phi$ and advances $B$.
In each of these categories, an extension is developped, as follows.
