...
For example, in Figure~\ref{fig:r-poly-inter}-(1), $\vv{A} \bigcap \vv{B} = P_1$ and $\vv{A}$ moves on, hence, rule 1 is applied. Because $g(A)=2 < 8=g(B)$, $\vv{A}$ moves forward $2*(M+g(A) - g(B)) = 2*(8+2-8)= 4$ steps, \ie from $2^{th}$ to $6^{th}$ (Figure~\ref{fig:r-poly-inter}-(2)).
...
For example, in Figure~\ref{fig:r-poly-inter}-(3), $\vv{A} \bigcap \vv{B} = P_2$ and $\vv{B}$ moves on, hence rule 2 is applied. $\vv{B}$ moves forward to $e^*_{k, 3}$, \ie the next edge of the $4^{th}$ edge as $g(A) = 4$ (Figure~\ref{fig:r-poly-inter}-(4)).
