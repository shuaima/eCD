...
By Theorem~\ref{prop-rp-intersection}, the intersection polygon $\mathcal{G}^*$ of all approximate polygon $\mathcal{G}_i$, $i \in [1, k]$, has no more than $M$ edges, thus, the computing of the intersection polygon of polygon $\mathcal{G}^*$ and approximate polygon $\mathcal{G}_{k+1}$ is executed in a constant time, \ie time $O(M)$, by the distinct convex polygon intersection algorithm presented in~ \cite{ORourke:Intersection}.


\subsubsection{Approximate polygon intersection}
The convex polygon intersection algorithm of \cite{ORourke:Intersection} is a algorithm for general convex polygons, while the approximate polygons are special polygons.
