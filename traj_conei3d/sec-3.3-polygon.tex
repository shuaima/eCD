...
By Theorem~\ref{prop-rp-intersection}, the intersection polygon $\mathcal{G}^*_k$ of all proximate polygon $\mathcal{G}_i$, $i \in [1, k]$, has no more than $M$ edges, thus, the computing of the intersection polygon of polygon $\mathcal{G}^*_k$ and proximate polygon $\mathcal{G}_{k+1}$ can be implemented in a constant time, \ie time $O(M)$, by the distinct convex polygon intersection algorithm described in~\cite{ORourke:Intersection}.
%
Moreover, observe that the convex polygon intersection algorithm of ~\cite{ORourke:Intersection} is a algorithm for general convex polygons, while the proximate polygons are special polygons. We further develop an even fast proximate polygon intersection method based on the properties of proximate polygons.
...
Given intersection polygon $\mathcal{G}^*_k$ of the preview $k$ polygons and the next proximate polygon $\mathcal{G}_{k+1}$, the proximate polygons intersection algorithm finds $\mathcal{G}^*_{k+1} = \mathcal{G}^*_k  \bigcap \mathcal{G}_{k+1}$. The basic idea of the algorithm is a two steps processing, \ie \emph{Filtering} and \emph{Joining}:
...
\stitle{Finding the intersection polygon.}
...
Hence, we need not consider these two cases in the intersection algorithm.
%
The presented intersection algorithm is essentially a customized version of the algorithm of ~\cite{ORourke:Intersection}.
\textcolor[rgb]{1.00,0.00,0.00}{summary of customization?}
...
The sketch of the algorithm is shown in Figure~
...
\textcolor[rgb]{0.00,0.07,1.00}{Algorithm: INTERSECTION OF CONVEX POLYGONS}

\hspace{4ex} Choose A and B \textcolor[rgb]{0.00,0.07,1.00}{arbitrarily}.

\hspace{4ex} repeat

\hspace{8ex} if A intersects B then

\hspace{12ex} \textcolor[rgb]{0.00,0.07,1.00}{Check for termination}.

\hspace{12ex} Update an inside flag.

\hspace{8ex} Moves on either A or B \textcolor[rgb]{0.00,0.07,1.00}{on candidate edges}.

\hspace{4ex} until both A and B cycle their polygons

\hspace{4ex} return the intersection polygon
