...
If ($\vv{A} \bigcap \vv{B} \ne \phi$ and $\vv{A} \times \vv{B} < 0$ and $P_{e_A} \not \in \mathcal{H}(\vv{B})$) or ($\vv{A} \bigcap \vv{B} \ne \phi$ and $\vv{A} \times \vv{B} > 0$ and $P_{e_B} \in \mathcal{H}(\vv{A})$), then it is possible to move $\vv{A}$ forward ``$s$" steps, where
%\vspace{-1ex}
\begin{equation*}
\label{equ-rule1}
\small
...
\begin{proof}
By equation (4), edge $A$ is indeed moved from the original position to its opposite position \wrt the edge on $\mathcal{G}_{k+1}$ with the same edge group as edge $B$.
For example, in Figure~\ref{fig:r-poly-rule1}.(1), 
%$\vv{A} \bigcap \vv{B} \ne \phi$ and $\vv{A} \times \vv{B} > 0$ and $P_{e_B} \in \mathcal{H}(\vv{A})$, hence 
$\vv{A}$ moves on. Because $g(A)=2 < 8=g(B)$, $\vv{A}$ moves forward $2*(M+g(A) - g(B)) = 2*(8+2-8)= 4$ steps, \ie from $2^{th}$ to $6^{th}$.
Here, $6^{th}$ is the opposite position of $2^{th}$ \wrt $8^{th}$, the edge group number of edge $B$.
