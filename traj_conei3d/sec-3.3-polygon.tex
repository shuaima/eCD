...
\textcolor[rgb]{1.00,0.00,0.00}{requirements: (1) linear; (2) incremental.} The checking of the $N$ circles intersection has a time complexity of \textcolor[rgb]{0.00,0.07,1.00}{${O(N\log N)}$}~\cite{Shamos:Circle}, which is not the linear time.
...
$\mathcal{G}^*$ of all $\mathcal{G}_i$ includes at most one edge from an edge group, \eg the $j^{th}$ edge group.
\end{theorem}
...
By Theorem~\ref{prop-rp-intersection}, the intersection polygon $\mathcal{G}^*$ of all approximate polygon $\mathcal{G}_i$, $i \in [1, k]$, has no more than $M$ edges, thus, the computing of the intersection polygon of polygon $\mathcal{G}^*$ and approximate polygon $\mathcal{G}_{k+1}$ can be implemented in a constant time, \ie time $O(M)$, by the distinct convex polygon intersection algorithm presented in~ \cite{ORourke:Intersection}.
%
However, the convex polygon intersection algorithm of \cite{ORourke:Intersection} is a algorithm for general convex polygons, while the approximate polygons are special polygons. 
%
In this observation, we further develop an even fast approximate polygon intersection algorithm based on the properties of approximate polygons.
