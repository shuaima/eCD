...
%
The presented approximate polygon intersection algorithm is indeed a customized version of the convex polygon intersection algorithm of ~\cite{ORourke:Intersection}, it optimizes the algorithm by customizing the rules for advancing $A$ and $B$, which are the key of algorithm~\cite{ORourke:Intersection}.
...
\emph{Rule 1: 
If $A \bigcap B \ne \phi$ and advances $A$, then moves on $A$ ``"steps.}

\emph{Rule 2: 
If $A \bigcap B \ne \phi$ and advances $B$, then moves on $B$ ``"steps.}
