...
observing that every segment in the two polygons being intersected has to originate from one of the $m$ edges of the regular polygons and the geometric similarity of the regular polygons, 
...
As you mentioned, the intersection of two \textcolor{blue}{general} $m$-edges polygons may produce a polygon with more than $m$ edges. To avoid this, we approximate a circle by a \textcolor{blue}{specialized} polygon, \ie a fixed rotation and m-edges inscribed regular polygon $\mathcal{R}(V, E)$,
...
We have appended the first paragraph of Section 3.3 to explain the motivation and intuition of this: ``Intuitively, we can approximate a circle with its $m$-edges inscribed polygon {whose intersection can be computed more fast}. However, the intersection of two $m$-edges polygons may produce a polygon with more than $m$ edges, thus, the time complexity of finding the common intersection of $n$ polygons on a plane is not linear. To solve this problem, we approximate a circle with a fixed rotation, $m$-edges, inscribed regular polygon."
...
%Keep in mind that its compression ratio is limited. %, an algorithm that outputs the original trajectory is sure having the smallest error. 
...
We have clarified this in the third paragraph of Section 1. Moreover, an extra explain have been presented after the definition of synchronized points in Section 2.1, and an example of the synchronized point has been illustrated in Example.1 in Section 2.1.
