...
To our knowledge, no one-pass and error bounded line simplification algorithm using SED has been developed in the community yet. Hence, one may wonder whether there exists an effective, one-pass and error bounded line simplification algorithm using SED. Theorem 8 is the positive answer to this question, and we proved the theorem by presenting two algorithms CISED-S and CISED-W. Indeed, Theorem 8 is an important contribution that we do for the community. 

%BTW, a algorithm that outputs all the original is also a one-pass algorithm, but it is hardly an effective error bounded trajectory compression algorithm as it does not remove any data points from the input.
