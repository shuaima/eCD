...
Edge $(P_i, P_k)$ can be included in reachability graph $G$ if and only if the intersection of line segment $\vv{P_iP_k}$ and 
the intersection of preview spatio-temporal cones $\bigsqcap_{j=i+1}^{k-1}$ \cone{(P_i, \mathcal{O}(P_{j}, \epsilon))} is not $\{P_i\}$.
