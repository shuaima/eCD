...
\stitle{Algorithm \cisto}. We now present algorithm \cisto. It takes as input a trajectory \trajec{T}${[P_0, \ldots, P_n]}$, an error bound $\epsilon$ and the number $m$ of edges for inscribed regular polygons, and returns a simplified  trajectory $\overline{\mathcal{T}}$ of $\dddot{\mathcal{T}}$.


The algorithm first initializes the start point $P_s$ to $P_0$, the edge set of
...
Then, it checks for every following points $P_{r}$ in the trajectory whether the edge
($P_{s}$,$P_{r}$) can be included into the reachability graph and updates the
intersection by calling procedure $\rpia()$ introduced in Section~\ref{subsec-fastRPI}.(lines 4--10).
		...
		{\small
			\begin{minipage}{3.36in}
				...
				\mat{0ex}{
					...
					%	\sstab
					\bcc \hspace{1ex}\= $P_s := P_0$;  ~$G := (\dddot{\mathcal{T}},\phi)$;
          ...
          \icc \>\hspace{3ex}\= $\mathcal{G}^* := {getRegularPolygon}$($P_s$, $P_{s+1}$, $\epsilon$, $M$, $t_c$) \\
					...
					\icc \hspace{0ex} $\overline{\mathcal{T}} = {Dijkstra}(G)$\\
