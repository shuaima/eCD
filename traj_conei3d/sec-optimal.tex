...
\begin{prop}
	...
	The reachability graph $G'$ can be constructed in $O(n^2)$ time.
\end{prop}

\begin{proof}
For any $P_s (0\le s \le n)$, the construction of graph $G'$ would consider all pairs of vertices $(P_s,P_{s+k}) (1\le k \le n)$ and compute the intersection of $\vv{P_sP_{s+k}}$ and $\bigsqcap_{i=1}^{k - 1}$\cone{(P_s,  \mathcal{O}(P_{s+i}, \epsilon))}. 
