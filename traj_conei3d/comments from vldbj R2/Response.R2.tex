...
Now, we have further revised the paper for the above three comments, as follows:
...
(3) For {R3C4-2}, \textcolor{blue}{we have evaluated an application named ``track playback" under the raw and simplified trajectories. The results are reported in Appendix: Application.}
...
We have add a new figure (Fig.1 in the new submission) to explain, and also compare, PED and SED. Moreover, the 2nd paragraph of Sec.1 is rewritten to introduce the character of PED and explain why it is not suitable for Spatio-temporal queries, \ie the ``unbounded" error of using PED in Spatio-temporal queries. And in the revised 3rd paragraph of Sec.1, we have also explained why SED is suitable for Spatio-temporal queries.
