...
Yes, as you have pointed out, the novelty of the work rests on an adequate justification for SED over PED. The other referees also pointed out that we did not adequately tell the difference between PED and SED in the work (\eg R3C1), which also hindered readers to catch the the contributions of the paper (\eg R2W1). Though we had addressed this problem in the preview response letter and the paper itself from our point of view, it seems that the revision is still inadequate. For convenience, we first list R3C1 and R2W1 here.
...
Now, we have rewritten the 2nd and 3rd paragraphs of Section 1 to a large extent to better explain SED, and also to show the benefits of using SED compared with PED.
More specifically, in the 2nd paragraph, we have introduced PED and explained why it is not suitable for applications like spatio-temporal query; then in the 3rd paragraph, we have introduced the key idea of SED and explained why it is suitable for spatio-temporal query.
We have also added a new figure (Fig.1 in the revised paper) to help understand them.

In the 5th and 6th paragraphs of Section 1, we have further summarized the key difference between one-pass line simplification algorithms using SED and PED, \ie PED works in a 2D space while SED works in a spatio-temporal 3D space, which make it a brand new work to design one-pass line simplification algorithms using SED.

For R2W1, we have rewritten the contributions of the paper in Section 1, and merged the responses of {R2W1} into the revised contributions (contributions 1 and 2). Note the above new introduction and comparison of SED and PED also reveal the significance and innovativeness of the work.
...
We have checked the comments and responses, and confirmed that most comments had been addressed in both response letter and the paper except three comments, \ie~{R2W1}, {R2C7} and {R3C4-2}, that were majorly addressed in response letter only. For R2W1, please refer to R1Ca. We then list R2C7 and R3C4-2 as below:
\begin{itemize}
...
Now, we have further revised the paper for the above comments, as bellow:

(1) For {R2C7}, we have clarified the importance of Theorem 8 (Theorem 7 in the revised version) at the begin of Section 4, where the response to R2C7 in the preview response letter is absorbed.

(2) For {R3C4-2}, we have clarified why we use compression ratio, error and running time as the major metrics at the end of Section 1 (before \emph{Organization}).
