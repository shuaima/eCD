...
%
Using four real-life trajectory datasets, we experimentally show that our approaches run fast as well as have good compresstion ratios.
%
In terms of running time, algorithms \cist and \cista are on average {$3$ times faster} than \squishe (the most fast existing \lsa algorithm using \sed). In terms of compression ratios, \cist is comparable with and \cista is {$19.6\%$} better than \dps (the existing sub-optimal \lsa algorithm using \sed having the best compression ratios) on average, respectively, and they are {$21.1\%$} and {$42.4\%$} better than \squishe on average, respectively.
\end{abstract}
