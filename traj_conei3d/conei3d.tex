...
In terms of execution time, \cist and \cista are {$3.0$ times faster} than \squishe, the existing \sed enabled \lsa algorithm with improved runtime performance.
In terms of compression ratio, \cist is comparable with \dps, the existing \sed enabled \lsa algorithm with distinct compression ratios, and is {$21.1\%$} better than \squishe on average; and \cista is {$19.6\%$} and {$42.4\%$} better than \dps and \squishe, respectively.
\end{abstract}
