...
\newcommand{\cia}{\kw{CI}}
\newcommand{\cpia}{\kw{convexPolyInterI}} % convex polygon intersection algorithm
...
%
In this study, we develop a one-pass, error bounded and \emph{synchronous Euclidean distances} (\sed) enabled trajectory simplification algorithm (\cist) by a novel spatio-temporal cone intersection method, which scans each data point in a trajectory once and only once.
%
We experimentally verify that our approach is both efficient and effective, using four real-life trajectory datasets.
In terms of execution time, \cist is around one time faster than \squishe, the fastest existing \lsa algorithm.
In terms of compression ratio, \cist is comparable with \dpa, the existing \lsa algorithm with the best compression ratio, and is {$20.5\%$} better than \squishe on average.
\end{abstract}
...
%IEEEbiography
\vspace{-6ex}
\begin{IEEEbiography}{Xuelian Lin} is an XXX at the School of Computer Science and Engineering, Beihang University. He received his PhD degree from Beihang University in 2013. His current research interests include distributed systems, large scale data management and processing systems.
\end{IEEEbiography}
\vspace{-6ex}
...
\end{IEEEbiography}
\vspace{-6ex}
\begin{IEEEbiography}{Yimeng Zuo} is a Master student in the School of Computer Science and Engineering, Beihang University. He received his BS degree in computer science and technology from Beihang University in 2015. His current research interests include databases and social data analysis
\end{IEEEbiography}
\vspace{-6ex}
