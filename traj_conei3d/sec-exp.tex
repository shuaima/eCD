...
\textcolor{blue}{In this set of tests, (a) the actual positions of a moving object are the data points of the original trajectory derived from the moving object, and (b) for each point $P$ in an original trajectory, we perform a query on its compressed trajectories taking $P.t$ as input and calculate its errors. Note when compressing these trajectories, we also fixed {$m$=$16$} and varied $\epsilon$ from $10$ to $200$ meters on the entire four datasets, respectively.}
\textcolor{red}{The max and average errors of the queries are reported in Table~\ref{tab:me-query} and Figure~\ref{fig:ae-query}, respectively.}

\ni \textcolor{blue}{(1) When using \ped, the max errors of spatio-temporal queries on compressed trajectories are significantly larger than error bounds. However, when using \sed, they are always not larger than error bounds. These confirm that \sed is more suitable than \ped for spatio-temporal queries, as illustrated in Section~\ref{sec-intro}.}
