...
\ni \textcolor{blue}{(2) Given the same error bound, the average errors of these queries on compressed trajectories using \ped are obvious larger than those using \sed, and some times they are even larger than error bounds due to some points having extraordinary large errors. Algorithm \cia using \ped has larger spatio-temporal query errors than \dpa using \ped because \cia applies a greedy policy that tries to include more points into a line segment, and some of these extra-points can lead to large errors.}
