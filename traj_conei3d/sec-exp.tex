...
\ni(1) When varying $m$, the compression ratios of algorithms \cist and \cista decrease with the increase of $m$ on all datasets. This feature is also holding on small error bounds, \eg $\epsilon < 60$ meters, and large error bounds, \eg $\epsilon > 60$ meters.
...
\ni(1) The compression ratios of these algorithms from the best to the worst are \cista, \dps, \cist and \squishe, on all datasets and for all sizes. %, which is consistent with Figure~\ref{fig:cr-m16}.
...
\ni(1) When varying $m$, the average errors of algorithms \cist and \cista increase with the increase of $m$ on both small error bound {(\eg $\epsilon = 10$ meters)} and large error bound (\eg $\epsilon = 200$ meters) and all datasets.
...
To evaluate the impacts of $\epsilon$, we fixed $m$=$16$ and varying $\epsilon$  from $10$ meters to $200$ meters on the entire \truck, \sercar, \geolife and \pricar, respectively.
