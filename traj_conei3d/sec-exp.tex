...
(4) the impacts of algorithm \rpia and the edge number $m$ of the regular polygons to the effectiveness and efficiency of algorithms \cist and \cista.
...
To evaluate the impacts of parameter $m$ on compression ratios of algorithms \cist and \cista, we fixed the error bounds {$\epsilon =60$ meters} and varied $m$ from $4$ to $40$.
...
\ni(2) When varying $m$, the compression ratios of algorithms \cist and \cista decrease with the increase of $m$ on all datasets. This feature is also holding on small error bounds, \eg $\epsilon < 60$ meters, and large error bounds, \eg $\epsilon > 60$ meters.
...
To evaluate the impacts of $\epsilon$ on compression ratios of these algorithms, we fixed {$m$=$16$} and varied $\epsilon$ from $10$ meters to $200$ meters on the entire four datasets, respectively.
...
\ni(2) When varying $m$, the average errors of algorithms \cist and \cista increase with the increase of $m$ on both small error bound {(\eg $\epsilon = 10$ meters)} and large error bound (\eg $\epsilon = 200$ meters) and all datasets.
...
The average errors of \cist and \cista are on average ($160.5\%$, $188.2\%$, $215.2\%$, {$180.3\%$}) and  ($326.7\%$, $331.1\%$, $349.7\%$, {$284.2\%$}) of \squishe and ($96.9\%$, $119.3\%$, $127.7\%$, $137.9\%$)and ($197.4\%$, $210.1\%$, $207.5\%$, $217.5\%$)  of \dps on (\truck, \sercar, \geolife, \pricar), respectively.

\ni(3) Average errors of algorithm \cista are on average ($93.8\%$, $86.0\%$, $81.4\%$, {$79.4\%$}) of \cist on (\truck, \sercar, \geolife, \pricar), respectively,when they have equal radius.
