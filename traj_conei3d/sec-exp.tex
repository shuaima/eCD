...
\textcolor{blue}{(5) the impacts of the distance metrics \ped and \sed on the compression ratios, errors and running time of trajectory simplification algorithms.}
...
\ni {(2)} Algorithm \cista~\textcolor{blue}{(weak simplification)} has the closest compression ratios to the optimal algorithm~\textcolor{blue}{(strong simplification)} on all datasets and for all $\epsilon$.
...
%
~\textcolor{blue}{This is because algorithm CISED-W allows data interpolations, thus by Proposition \ref{prop-circle-intersection}, it extends the radius of the base circles of the spatio-temporal cones from $\epsilon/2$ in CISED-S to $\epsilon$ in CISED-W to include more points.}
...
\sstab \emph{(1) Datasets}. \textcolor{blue}{The behaviors of all algorithms across all datasets are quite similar, \eg the compression ratio $cr$ of each algorithm has approximately an exponential relation to $\epsilon$, \ie $cr=a^\epsilon$, where $a$ is in $(0,1)$, and the average error of it has a linear relation to $\epsilon$.}

\sstab \emph{(2) Distance metrics}. \textcolor{blue}{The compression ratios of algorithms using \ped are obviously better than using \sed, and the sizes of compresed data using \ped are nearly half of using \sed.}

\sstab \emph{(3) Polygon intersection Algorithms}. Algorithm \rpia runs faster than  algorithm \cpia, and has the same compression ratios and average errors as \cpia.

\sstab\emph{(4) Parameter $m$}. The compression ratio decreases with the increase of $m$, and the running time increases nearly linearly with the increase of $m$. In practice, the range of $[12, 20]$ is a good candidate region for $m$.

\sstab\emph{(5) Compression ratios}. The optimal \lsa algorithm has the best compression ratios among all strong simplification algorithms. Algorithm \cist is close to \dps and algorithm \cista is better than all the sub-optimal \lsa algorithm.
...
\sstab\emph{(6) Average errors}. The average errors of these algorithms from the smallest to the largest are \squishe, \dps, \cist, the optimal \lsa algorithm and \cista. Algorithm \cista has obvious higher average errors than \cist as the former essentially forms spatio-temporal cones with a radius of $\epsilon$.

\sstab\emph{(7) Running time}. Algorithms \cist and \cista are the fastest. They are on average
...
\stitle{Discuss}. \textcolor{blue}{Based on these findings, we are to provide a brief guideline for selecting an algorithm to compresss trajectories.
...
%
(2) If the compression ratio is the priority, then algorithm \cista and the optimal algorithm are the selections, followed by algorithms \dps and \cist.
%
(3) When considering error, \squishe is a choice because it has a relative small average error. Alternatively, we can also use \dps or \cist setting a smaller error bound $\epsilon$ compared with \squishe.
}
