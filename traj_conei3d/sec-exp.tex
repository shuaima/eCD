...
Based on these findings, we further provide some brief guidelines for selecting an algorithm to compresss trajectories.
...
(1) If the running time is the first-level consideration, then the one pass algorithms, \ie~\cist and \cista, are surely the best choices. They have a linear time and need $O(n)$ space, thus, they are suitable for running in resource-constraint devices. Also note that \cista may output data points that are not selected from the input data points.
%
