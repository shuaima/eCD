...
\ni(3) When varying $m$, the compression ratio of algorithms \cist and \cista decreases (a) fast when $m < 12$, (b) slowly when $m \in [12, 20]$, and (c) very slow when $m > 20$. Hence, \emph{the region range of $[12, 20]$ is the good candidate region for $m$ in terms of compression ratios.}
...
\ni(1) Algorithms \cist and \cista using \rpia have the same average errors as their counterparts using \cpia on all datasets and for all $m$.
...
\ni(3) When varying $m$, it is similar to compression ratios, and
 the average errors increases (a) fast when $m < 12$, (b) slowly when $m \in [12, 20]$, and (c) very slowly when $m > 20$. \emph{The range of $[12, 20]$ is also the good candidate region for $m$ in terms of errors.} Here the average error of $m=12$ is only on average {$98.4\%$} of $m=20$.
...
To evaluate the impacts of algorithm \rpia and parameter $m$ on algorithms \cist and \cista, we
...
\vspace{-2ex}
\caption{\small Evaluation of running time: fixed error bound with $\epsilon=60$ meters, and varying $m$. Here ``F'' denotes our fast regular polygon intersection algorithm \rpia, and ``P'' denotes \cpia, repectively.}
...
\sstab \emph{(1) Polygon intersection Algorithms}. \rpia runs faster than \cpia, and is as effectiveness as \cpia.
