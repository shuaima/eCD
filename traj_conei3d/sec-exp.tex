...
In this section, we present an extensive experimental study of our \textcolor{blue}{near optimal algorithm \cisto and one-pass sub-optimal trajectory simplification algorithms (\cist and \cista) compared with} existing algorithms of \dps and \squishe on trajectory datasets.
...
(4) the impacts of polygon intersection algorithms \rpia and \cpia and the edge number $m$ of inscribed regular polygons to the effectiveness and efficiency of algorithms \cisto, \cist and \cista.
...
We implement five \lsa algorithms, \ie our \cisto, \cist and \cista, \dps~\cite{Meratnia:Spatiotemporal} (the most effective existing \lsa algorithm using \sed), and \squishe~\cite{Muckell:Compression} (the most efficient existing \lsa algorithm using \sed).
...
\begin{figure*}[tb]
	...
	\includegraphics[scale = 0.30]{Figures/Exp-error-epsilon-mopsi.png}
	\includegraphics[scale = 0.30]{Figures/Exp-error-epsilon-private.png}
	%\vspace{-1ex}
...
\textcolor{blue}{The near optimal algorithm \cisto is the slowest and is usually ten times slower than other algorithms including \dpa. It is not necessary to report all the detailed running time of \cisto in this section.}
...
\begin{figure*}[tb!]
	\centering
	\includegraphics[scale = 0.30]{Figures/Exp-M-e-60-time-service.png}
	\includegraphics[scale = 0.30]{Figures/Exp-M-e-60-time-geolife.png}
...
\ni(2) Algorithms \cist and \cista are both the fastest \lsa algorithms using \sed,
and are {($8.00$--$10.00$, $5.83$--$8.11$, $3.75$--$7.08$, $5.00$--$8.09$) times faster than \dps,
	and {($2.53$--$3.00$, $2.62$--$3.12$, $2.25$--$3.05$, $2.89$--$3.40$)}} times faster than \squishe on the selected $1K$ to $10K$ points datasets (\sercar,
\geolife, \mopsi, \pricar), respectively.

\ni(3) Algorithms \cist and \cista scale well with the increase of the size of trajectories on all datasets,
and both have a linear running time, while algorithm \dps does not.
This is consistent with their time complexity analyses.

\ni(4) The efficiency advantage of algorithms \cist and \cista increases with the increase of trajectory sizes compared with \dps and \squishe.
...
\sstab\emph{(5) Running time}. \textcolor{blue}{Algorithm \cisto is the slowest while algorithms \cist and \cista are the fastest.} Algorithms \cist and \cista are on average
