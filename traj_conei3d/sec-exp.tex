...
To evaluate the impacts of polygon intersection algorithm \rpia and parameter $M$ on compression ratios of algorithms \cist and \cista, we fixed the error bounds \textcolor[rgb]{1.00,0.00,0.00}{$\epsilon =10m$} and $\epsilon =200m$, and varied $M$ from $4$ to $40$.
...
%The compression ratios of \rpia equipped simplification algorithms are the same as \cpia equipped simplification algorithms on all datasets. \eg, the compression ratio of \cist-\cpia is the same as \cist-\cpia for each $M$.


\ni(2) When varying $M$, the compression ratios of algorithms \cist and \cista decrease with the increase of $M$ on both small error bound \textcolor[rgb]{1.00,0.00,0.00}{(\eg $\epsilon = 20m$)} and large error bound (\eg $\epsilon = 200m$) and all datasets.
More specially, the compression ratio decreases (a) fast when $M < 10$, and (b) very slow when $M  > 20$.
\emph{The region $[10, 20]$ is the candidate region for $M$ in terms of compression ratio.}


\ni(3) The compression ratio of $M=10$ is average \textcolor[rgb]{1.00,0.00,0.00}{$102\%$} of $M=20$.
...
To evaluate the impacts of algorithms \rpia and parameter $M$ on average errors of algorithms \cist and \cista, we fixed the error bounds \textcolor[rgb]{1.00,0.00,0.00}{$\epsilon =10m$} and $\epsilon =200m$, and varied $M$ from $4$ to $40$.
...
Similar as compression ratios, the average errors increases (a) fast when $M < 10$, and (b) very slow when $M  > 20$.
\emph{The region $[10, 20]$ is also the candidate region for $M$ in terms of average errors.}


\ni(3) The average error of $M=10$ is average \textcolor[rgb]{1.00,0.00,0.00}{$102\%$} of $M=20$.
...
\ni(3) Algorithms \cist and \cista have lager average errors compared with \dpa and \squishe.
...
In the set of tests, we compare the efficiency of (regular) polygon intersection algorithms \rpia and \cpia, the efficiency (execution time) of our approach \cist with algorithms \dpa and \squishe, and test the impacts of the parameter $M$, \ie the number of edges of regular polygons, to the efficiency of algorithms \cist and \cista.
%
...
\vspace{-2ex}
\caption{\small Evaluation of running time of compression algorithms: fixed error bound with $\epsilon=60m$, and varying $M$.}
...
\vspace{-2ex}
\caption{\small Evaluation of running time of compression algorithms: fixed error bound with $\epsilon=200m$, and varying $M$.}
...
\vspace{-3ex}
\caption{\small Evaluation of running time of compression algorithms: fixed with $M=20$ and varying error bounds $\epsilon$.}
...
\vspace{-3ex}
\caption{\small Evaluation of running time of compression algorithms: fixed with $M=20$ and $\epsilon=60m$, and varying the size of trajectories.}
\label{fig:time-size}
\vspace{-1ex}
\end{figure*}
...
\vspace{1ex}
\ni\emph{(1) Polygon intersection Algorithms}. \rpia runs fast than \cpia in all datasets, and it has the same effectiveness as \cpia
