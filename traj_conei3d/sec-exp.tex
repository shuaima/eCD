...
\textcolor{blue}{We first perform a spatio-temporal query, \ie ``\emph{the position $P$ of a moving object at time $t$}", on the compressed trajectories simplified by some \lsa algorithm using \ped or \sed, which returns an approximate point (\ie the synchronized point) $P'$ of the moving object at time $t$.
...
\textcolor{blue}{In this set of tests, (a) the actual positions of a moving object are the data points of the original trajectory derived from the moving object, (b) for each point $P$ in an original trajectory, we perform a query taking $P.t$ as input and calculate its error, and (c) we fixed {$m$=$16$} and varied $\epsilon$ from $10$ to $200$ meters on the entire four datasets, respectively.}
\textcolor{red}{The errors of spatio-temporal queries on compressed trajectories are reported in Figure~\ref{fig:ae-query} and Table~\ref{tab:me-query}.} 

\ni \textcolor{blue}{(1) When using \ped, the max errors of spatio-temporal queries on compressed trajectories are significantly larger than error bounds. However, when using \sed, they are always not more than error bounds. These confirm that \sed is suitable but \ped is not suitable for spatio-temporal queries, as illustrated in Section~\ref{sec-intro}.}
