...
(3) the execution time of algorithms \cist and \cista vs. \dps, \squishe and the optimal algorithm, and
...
\sstab \emph{(1) Datasets}. In almost all the tests, the behaviors of all algorithms across all datasets are quite similar, \eg the compression ratio of each algorithm has approximately an exponential relation to $\epsilon$ with a base in $(0,1)$, and the average error of it has a linear relation to $\epsilon$.

\sstab \emph{(2) Polygon intersection Algorithms}. Algorithm \rpia runs faster than \cpia, and is as effective as \cpia.
...
\sstab\emph{(4) Compression ratios}. The optimal \lsa algorithm has the best compression ratios among all strong simplification algorithms. Algorithm \cist is comparable with \dps and algorithm \cista is comparable with the optimal \lsa algorithm.
...
\textcolor{blue}{Based on these findings, we further provide some brief guidelines for selecting an algorithm to compresss trajectories.
First of all, some applications may require a balance among running time, compression ratio and error, and its hard to give a clear guidline for them. So, we discuss scenarios that have special requirements. 
(1) If the running time is the first-level consideration, then the one pass algorithms, \ie~\cist and \cista, are surely the best choices. They have a linear time and need $O(n)$ space, thus, they are suitable for running in resource-constrained devices. Also note that \cista may output data points that are not selected from the input data points.
%
(2) If the compression ratio is the priority, then algorithm \cista and the optimal algorithm are the selections.
%
(3) When considering error, \squishe is the choice because it has a relative small average error. Of course, an algorithm that outputs the original trajectory is sure having the smallest error. 
}
