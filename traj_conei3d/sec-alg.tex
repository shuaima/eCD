...
%	\sstab
    ...
    \icc \> \hspace{4ex} \ \ \ $i$ := $i - 1$;\ \ $\overline{\mathcal{T}} := \overline{\mathcal{T}}\cup \{\mathcal{L}(P_s,P_{i})\}$; \ \ $P_s := P_{i}$;\\
    \icc \>\hspace{2ex} $i$ := $i +1$;\\
 	\icc \> \hspace{0ex} $\overline{\mathcal{T}}$ := $\overline{\mathcal{T}}\cup \{\mathcal{L}(P_s,P_{n})\}$; \\
