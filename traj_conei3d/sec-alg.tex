...
\begin{theorem}
\label{prop-cist-op}
There exist one-pass error bounded trajectory simplification algorithms using \sed for both strong and weak trajectory simplifications.
\end{theorem}
...
and, hence, it replaces $\epsilon/2$ with $\epsilon$ (line 3 of \cist). It also generates new line segments with data points $Q$ which may be interpolated points, and,
...
\begin{prop}
\label{prop-cist-Q}
Given a sub-trajectory \trajec{T}${[P_s, \ldots, P_{s+k}]}$ and an error bound $\epsilon$,  $t_c=P_{s+k}.t$ and $G^*_k$ be the intersection polygon of all polygons $G_{s+i}$ ($i\in[1,k]$) on the plane $P.t - t_c = 0$. If $G^*_k$ is not empty, then any point in the area of $G^*_k$ is feasible for $Q$.
\end{prop}
...
\sstab (1) If $P_{s+k}$ is in the area of $G^*_k$ \wrt $t_c=P_{s+k}.t$, then $Q$ is simply $P_{s+k}$.
...
\sstab (1) After initialization, the \cista algorithm reads point $P_1$ and builds an \emph{oblique circular cone} \cone{(P_0, \mathcal{O}(P_{1}, \epsilon))}, and projects it on the plane $P.t-P_1.t=0$. The inscribed regular polygon $\mathcal{G}_1$ of the projection circle is returned and the intersection polygon $\mathcal{G}^*$ is set to $\mathcal{G}_1$.
