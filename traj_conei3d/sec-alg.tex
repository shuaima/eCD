...
\end{proof}
 ...
 We now present the  one-pass error bounded {\em strong trajectory simplification} algorithm using the synchronous distance, which is shown in Figure~\ref{alg:CI3d} .

\stitle{Algorithm \cist}. It takes as input a trajectory \trajec{T}${[P_0, \ldots, P_n]}$, an error bound $\epsilon$ and the number $m$ of regular polygons, and returns a simplified  piece-wise line representation $\overline{\mathcal{T}}$ of $\dddot{\mathcal{T}}$.
...
%	\sstab
	\bcc \hspace{1ex}\= $P_s := P_0$;  ~$P_e := P_0$;\\
\icc \hspace{1ex}\= ~$\overline{\mathcal{T}} := \emptyset$;  ~$\mathcal{G}^* := \emptyset$;  ~$i := 1$; ,~$t_c$ := $P_n.t$;\\
	\icc \hspace{1ex}\= \While $i \le n$ \Do \\
	\icc \>\hspace{3ex} $\mathcal{G}$ := \kw{getRegularPolygon}($P_s$, $P_i$, $\epsilon/2$, $m$, $t_c$); \\
	\icc \>\hspace{3ex} \If $\mathcal{G}^* = \emptyset$  \\
	\icc \>\hspace{3ex} \Then \ \ $\mathcal{G}^* :=\mathcal{G};$ \\
	\icc \>\hspace{3ex} \Else\ \ $\mathcal{G}^* := {\rpia}(\mathcal{G}^*, ~\mathcal{G})$; \\
	\icc \>\hspace{3ex} \If $\mathcal{G}^* \ne \emptyset$ \\
	\icc \> \hspace{3ex} \Then \ \  $P_e := P_i$; \ \ $i := i+1$; \\
	\icc \>\hspace{3ex} \Else\ \ $\overline{\mathcal{T}} := \overline{\mathcal{T}}\cup \{\mathcal{L}(P_s,P_e)\}$; \ \ $P_s := P_e$;  ~~$\mathcal{G}^* := \emptyset$; \\
...
%	\bcc \hspace{1ex} \textcolor[rgb]{0.00,0.07,1.00}{Transform $P_s$ and $P_i$ to points in Cartesian coordinates} \\
	...
	\icc \hspace{1ex} $P_i.x := P_s.x + c\cdot(P_i.x-P_s.x)$; \\
	\icc \hspace{1ex} $P_i.y := P_s.y + c\cdot(P_i.y-P_s.y)$; \\
