...
\begin{cor}
Given a sub-trajectory \trajec{T}${[P_s, \ldots, P_{s+k}]}$ and an error bound $\epsilon$, let $t_c=t_{s+k}$ and $G^*_k$ \wrt $t_c=t_{s+k}$ be the intersection polygons of all polygons $G_{s+i}$, $1\le i\le k$, on the plane $t=t_c=t_{s+k}$. If $G^*_k$ is not empty, then any point in $G^*_k$ is feasible for $Q$.
\end{cor}
