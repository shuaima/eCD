...
Conversely, assume that Edge $(v_s,v_r)$ can be included in the $\epsilon - graph$ .
Then the synchronous distance of $P_{s+i}$ to $\vv{P_sP_r}$ is error bouned for
each $i \in [1,r - 1]$ , By Proposition~\ref{prop-3d-syn-point}, we have $\vv{P_sP_r} \in $ $\bigsqcap_{i=1}^{r - 1}$\cone{(P_s, \mathcal{O}(P_{s+i}, \epsilon))}  and
$\bigsqcap_{i=1}^{r - 1}$\cone{(P_s, \mathcal{O}(P_{s+i}, \epsilon))} $\ne \{P_s\}$. 
...
\begin{figure}[tb!]
	...
	\end{center}
	\vspace{-2ex}
	\caption{\small Near Optimal algorithm using \sed (\cisto).}
	\label{alg:cisedo}
	\vspace{-2ex}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure*}[tb!]
	...
	%\vspace{-2ex}
	\caption{\small A running example of the \cisto algorithm. The points and the oblique circular cones are projected on an x-y space. The trajectory $\dddot{\mathcal{T}}[P_0, \ldots, P_{10}]$ is compressed into four three segments.}
	\vspace{-1ex}
	\label{fig:exm-cised-o}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
