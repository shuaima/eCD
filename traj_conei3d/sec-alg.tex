...
There exist one-pass error bounded trajectory simplification algorithms using the synchronous Euclidean Distance for both strong and weak trajectory simplification.
\end{theorem}
...
If we restrict $Q=P_{s+k}$, the end point of the sub-trajectory, then the cone projection circles with a radius of $\epsilon/2$ suffices.
...
we now present our one-pass strong simplification algorithm. 
