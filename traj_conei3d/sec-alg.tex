...
%	\sstab
	...
	\icc \>\hspace{2ex} \If $\mathcal{G}^* = \emptyset$  \Then $\mathcal{G}^* :=\mathcal{G};$ \\
%	\icc \>\hspace{2ex} \Else\\ 
    \icc \>\hspace{2ex} $\mathcal{G}^* := {\rpia}(\mathcal{G}^*, ~\mathcal{G})$; \\

%    \icc \>\hspace{2ex} \If $\mathcal{G}^* \ne \emptyset$ \Then \\
%	\icc \> \hspace{2ex} \ \  $P_e := P_i$; \ \ $i := i+1$; \\
	\icc \>\hspace{2ex} \If $\mathcal{G}^* = \emptyset$ \Or $i = n$ \Then \\ 
    \icc \> \hspace{2ex} \ \  $\overline{\mathcal{T}} := \overline{\mathcal{T}}\cup \{\mathcal{L}(P_s,P_i)\}$; \ \ $P_s := P_i$;\\
