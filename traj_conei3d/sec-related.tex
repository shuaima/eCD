...
Its target is to approximate a given finer piece-wise linear curve by another coarser piece-wise linear curve, which is a sub set of the former, such that the maximum distance of the former from the later is bounded by a user specified constant $\epsilon$.
...
%
The \squishe\cite{Muckell:Compression} algorithm is an combination of {opening} window and Bottom-up. It \eat{optimizes the \pavlidis by using} uses a doubly linked list $Q$ so as to achieve a better efficiency. \squishe supports \sed, however, it is not linear time, and its compression ratio is obviously poorer than our algorithm \cist.
