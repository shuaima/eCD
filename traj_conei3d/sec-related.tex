...
%
\textcolor{blue}{Initially, line simplification (\lsa) algorithms use perpendicular Euclidean distances (\ped) as the distance metric. 
...
\textcolor{blue}{We next introduce the related work on line simplification based trajectory compression from two aspects: optimal methods and sub-optimal methods.}
...
%
The authors of \cite{Chan:Optimal} further proved that the optimal algorithm using \ped as the error metric could be implemented in $O(n^2)$ time with the help of the \textit{sector intersection} mechanism. 
Because the \textit{sector intersection} and the convex hull mechanisms can not work with \sed, hence, the time complexity of the optimal algorithm using \sed {remains $O(n^3)$}. 
