...
%
(1) During each step, it checks the intersection of $A$ and $B$. If $A$ intersects $B$, then the algorithm records the inner segment, which is a boundary segment of the intersection polygon (lines 3-5).
%
(2) The algorithm moves on $A$ or $B$ one step under some carefully designed rules, which are depending on geometric conditions of $A$ and $B$ (line 6).
...
The algorithm has a time complexity of $O(|G_1| + |G_2|)$.
