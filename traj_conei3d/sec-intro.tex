...
However, these methods still have a high time and/or space complexity, which significantly hinders their utility in resource-constrained devices \cite{lin:operb}.
...
\stitle{\textcolor[rgb]{0.00,0.07,1.00}{Contributions}}.
...
\stab (1) We develop a one-pass, error bounded and \sed enabled trajectory simplification algorithm, say, the \underline{C}one \underline{I}ntersection with \underline{S}ynchronous \underline{E}uclidean \underline{D}istance (\cist) algorithm, which runs in $O(n)$ time and $O(1)$ space.
...
\stab (2) Using four real-life trajectory datasets (\truck, \sercar, \geolife, \pricar),
...
\stitle{\textcolor[rgb]{0.00,0.07,1.00}{Organization}}.
