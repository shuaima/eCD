...
We will explain this more concretely.
Here, the approximate point $P'$ is formally called the synchronized data point \cite{Meratnia:Spatiotemporal}.
Intuitively, a synchronized data point $P'_i$, $s<i<e$, is a point on $\vv{P_sP_{e}}$ satisfying $\frac{|P_sP'_i|}{|P_sP_e|} = \frac{t_i - t_s}{t_e-t_s}$. 
For instance, $P'_4$ and $P'_7$ are the \emph{synchronized data points} of points $P_4$ and $P_7$ \wrt line segments $\vv{P_0P_{10}}$ and $\vv{P_4P_{10}}$ satisfying $\frac{|P_0P'_4|}{|P_0P_{10}|} = \frac{t_4 - t_0}{t_{10}-t_0}$ and $\frac{|P_4P'_7|}{|P_4P_{10}|} = \frac{t_7 - t_4}{t_{10}-t_4}$, respectively, in Figure~\ref{fig:notations} (left).
And a query for the position of the moving object at time $t_7$ will return the synchronized data point $P'_7$ whose distance to the actual position at time $t_7$, \ie $P_7$, is great than the bound $\epsilon$.
...
%
Obviously, linear time \lsa algorithms are more efficient for resource-constrained devices, and to our knowledge, no one-pass \lsa algorithm using \sed has been developed in the community. This inspired us to develop a linear time \lsa algorithms using \sed. 
