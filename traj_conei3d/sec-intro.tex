...
\textcolor{blue}{Suppose a sub-trajectory $[P_s, ..., P_e]$ is represented by a line segment $\vv{P_sP_e}$  simplified by an error bounded \lsa algorithm using \ped, then for any point $P \in [P_s, ..., P_e]$, its \emph{perpendicular Euclidean distance} to the line segment $\vv{P_sP_e}$  is the shortest distance from point $P$ to the line segment as shown in Figure~\ref{fig:distances}(a).}
\textcolor{blue}{Indeed, after the redundant points (points other than $P_s$ and $P_e$) are removed, information about them is lost except that they are located in a zone composed by a rectangle and two half-circles, such that each point in the zone has a \ped to $\vv{P_sP_e}$ not more than the pre-defined error bound $\epsilon$.}
\textcolor{blue}{The limitation of using \ped is that a zone can be a large/long geographic range, thus, after a ``redundant" point is removed, we can't infer the exact location of it though we know it must be located in its zone.}
%
\textcolor{blue}{Hence, the answer to a spatio-temporal query ``\emph{the position $P$ of a moving object at time $t$} \cite{Cao:Spatio}" on the compressed trajectories is not bounded. This query, in best practice, returns an approximate point $P'$ selected from the line segment $\vv{P_sP_e}$. However, no matter how to choose an approximate point $P'$, the distance between points $P$ and $P'$ is possible larger than error bound $\epsilon$ as point $P$ may be located in any position of a probable large zone.}
...
The \emph{synchronous Euclidean distance} (\sed) was then introduced for trajectory compression~\cite{Meratnia:Spatiotemporal, Cao:Spatio, Potamias:Sampling, Muckell:Compression, Chen:Fast} to support the above spatio-temporal queries.
\textcolor{blue}{The meaning of \sed highly depends on a notion named \emph{synchronized point} \cite{Meratnia:Spatiotemporal,Cao:Spatio} that could be computed easily. As shown in Figure~\ref{fig:distances}(b), the \emph{synchronized point} $P'$ of a point $P$ at time $t$ \wrt a line segment $\vv{P_sP_e}$  is the expected position of the moving object on $\vv{P_sP_e}$ at time $t$ \emph{if the object moved along a straight line from point $P_s$ to point $P_e$ with a uniform speed} \cite{Cao:Spatio}, \ie the average speed of it between points $P_s$ and $P_e$.}
...
\textcolor{blue}{Based on this notion, the \sed of point $P$ to line segment $\vv{P_sP_e}$ is the Euclidean distance between point $P$ and its \emph{synchronized point} $P'$  \wrt the line segment $\vv{P_sP_e}$.}
\textcolor{blue}{Then, when the sub-trajectory $[P_s, ..., P_e]$ is simplified by an error bounded \lsa algorithm using \sed to line segment $\vv{P_sP_e}$, although those redundant points are removed, we can still infer that each of them is located inside of a circle around its \emph{synchronized point} with a radius of $\epsilon$ (Figure~\ref{fig:distances}(b)).}
\textcolor{blue}{Hence, the above spatio-temporal query over the trajectories compressed by \sed enabled approaches returns the \emph{synchronized point} $P'$ of $P$ \wrt the corresponding line segment, and it ensures the Euclidean distance between the point and its \emph{synchronized point} is limited within the error bound $\epsilon$.}
%
...
%\vspace{-1ex}
\caption{\small A sub-trajectory $[P_s, \ldots, P_e]$ is simplified using \ped and \sed, respectively. A spatio-temporal query ``\emph{the position $P$ of the moving object at time $t$}" on the simplified trajectory will return a point (a) in a probable large zone composed by a rectangle and two half-circles, or (b) inside a small circle.}
\vspace{-3ex}
\label{fig:distances}
\end{figure}
...
\textcolor{blue}{Because \sed introduces the temporal information besides the spatial information, a one-pass local distance checking approach of \sed is far more than a delta of \ped though they may share some common idea.} Indeed, it is even more challenging to design one-pass \lsa algorithms using \sed than using \ped.
...
\sstab {(3)} We then develop two one-pass trajectory simplification algorithms \cist and \cista, achieving $O(n)$ time complexity and $O(1)$ space complexity, based on our local synchronous distance checking technique.
