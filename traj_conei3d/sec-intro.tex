...
\textcolor{blue}{The time complexity of the optimal algorithm using \ped is higher than using \sed.
	The authors of \cite{Imai:Optimal} showed that the optimal \ped equipping algorithm can be found in $O(n^3)$ time, where $n$ is the number of the original points.
	Next, the authors of \cite{Chan:Optimal} proved that the algorithm could further be implemented in $O(n^2)$ time with the help of the sector intersection mechanism. 
	It is worth pointing out that, though \cite{Chan:Optimal} did not discuss the \sed metric, it is clear that the time complexity of that \sed enabled optimal algorithm remains $O(n^3)$ as the sector intersection mechanism does not work for \sed. }
%
...
On the other hand, there are a number of \textcolor{blue}{\ped equipped, linear complexity algorithms} \cite{Williams:Longest, Sklansky:Cone, Dunham:Cone, Zhao:Sleeve, Lin:Operb} that are more appropriate for resource-constrained devices. 
%
\textcolor{blue}{These algorithms apply optimization strategies, \ie the sector intersection mechanism of \cite{Williams:Longest, Sklansky:Cone, Dunham:Cone, Zhao:Sleeve} and the fitting function in our preview work \cite {Lin:Operb}, to fast the process of simplification so as to save resources. 
Yet, these algorithms are applicable for \ped only as the core optimization strategies they applied are \ped specific, which means they would not work correctly if the error metric \ped is roughly replaced by other metrics, such as \sed.} 
%

%
\textcolor{blue}{Thus, to the best of our knowledge, there is no near-optimal, linear complexity and \sed enabled line simplification algorithm.}
\textcolor{blue}{And, it is non-trival to develop such an \sed enabled linear complexity algorithm. }
