...
\lsa methods using \sed have been developed for batch algorithms (\eg Douglas-Peucker based algorithm \dpsed \cite{Meratnia:Spatiotemporal}) and online algorithms (\eg\ \squishe \cite{Muckell:Compression}).
...
%
\textcolor{blue}{It is also worth pointing out that the time complexity of the optimal \lsa algorithm using \sed is higher than using \ped.
In \cite{Imai:Optimal}, the authors showed that the optimal \ped equipping \lsa algorithm can be found in $O(n^3)$ time, where $n$ is the number of the original points. Next, the authors of \cite{Chan:Optimal} proved that the algorithm could further be implemented in $O(n^2)$ time with the help of the \textit{sector intersection} mechanism. 
Though \cite{Chan:Optimal} did not discuss the \sed metric, it is clear that the time complexity of the optimal \sed enabled \lsa algorithm remains $O(n^3)$ as the \textit{sector intersection} mechanism can not work with \sed. }
%
