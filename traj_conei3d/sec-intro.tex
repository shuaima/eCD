...
\stab {(2)} We then develop two one-pass, error bounded and \sed enabled trajectory simplification algorithms based on the local synchronous distance checking approach.
...
Both of them run in $O(n)$ time and $O(1)$ space.
