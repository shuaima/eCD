...
\textcolor{blue}{There exists the optimal \lsa algorithm using \sed whose time complexity is $O(n^3)$.
...
\textcolor{blue}{Secondly, sub-optimal and error bounded \lsa algorithms} using \sed have been developed for \textcolor{blue}{trajectory compression, including} batch algorithms (\eg Douglas-Peucker based algorithm \dpsed \cite{Meratnia:Spatiotemporal}) and online algorithms (\eg\ \squishe \cite{Muckell:Compression}).
...
Yet, these algorithms are only applicable for \ped because the core optimization strategies they applied are still \ped specific, which means they would not work correctly if the \ped metric is replaced by other metrics, such as \sed.} 
...
To this end, we propose two one-pass \textcolor{blue}{sub-optimal and a fast near optimal} error bounded \lsa algorithms using \sed for compressing trajectories in an efficient and effective way. % error bounded
...
\sstab {(2)} We then develop two one-pass \textcolor{blue}{sub-optimal} trajectory simplification algorithms \cist and \cista \textcolor{blue}{and a fast near optimal trajectory simplification algorithm \cisto using \sed,} based on our local synchronous distance checking technique\eat{ (Section~\ref{sec-alg})}. 
