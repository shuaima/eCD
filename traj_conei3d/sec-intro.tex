...
\stab {(1)} We first develop a novel local synchronous distance checking approach, \ie the spatio-temporal \underline{C}one \underline{I}ntersection with \underline{S}ynchronous \underline{E}uclidean \underline{D}istance (CISED), by extending the sector intersection method \cite{Williams:Longest, Sklansky:Cone, Zhao:Sleeve}. We further approximate the spatio-temporal cone intersection by the regular polygon intersection and develop a fast regular polygon intersection algorithm,
