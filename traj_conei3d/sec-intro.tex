...
\textcolor{blue}{The problem targeting at finding out the minimal number of line segments to represent the original polygonal lines \wrt an error bound $\epsilon$ is also known as the ``min-\#" problem\cite{Imai:Optimal,Chan:Optimal}. For this problem, 
	...
	Though \cite{Chan:Optimal} did not discuss the \sed metric, it is obviously that the time complexity of the optimal \lsa algorithm using \sed remains $O(n^3)$ as the \textit{sector intersection} mechanism are based on \ped and can not work correctly with \sed. 
...
\sstab {(2)} \textcolor{blue}{We then show that an optimal trajectory simplification algorithm using \sed can achieve $O(n^2 \log n)$ time by using our local synchronous distance checking technique, and provide a fast near optimal trajectory simplification algorithm \cisto that achieves $O(n^2)$ time and $O(n)$ space.}

\sstab {(3)} We next develop two one-pass \textcolor{blue}{sub-optimal} trajectory simplification algorithms \cist and \cista, \textcolor{blue}{achieving $O(n)$ time and $O(1)$ space}, based on our local synchronous distance checking technique\eat{ (Section~\ref{sec-alg})}. 
