...
\stab {(1)} We first develop a novel local synchronous distance checking approach, \ie the \underline{C}one \underline{I}ntersection with \underline{S}ynchronous \underline{E}uclidean \underline{D}istance (CISED), by extending the sector intersection method \cite{Williams:Longest, Sklansky:Cone, Zhao:Sleeve}. We further approximate the spatio-temporal cone intersection by the regular polygon intersection and develop a fast regular polygon intersection algorithm,
such that each data point in a trajectory is checked in O(1) time during the entire process of trajectory simplification.

\stab {(2)} We then develop two one-pass, error bounded and \sed enabled trajectory simplification algorithms based on the local synchronous distance checking approach, and both of them run in $O(n)$ time and $O(1)$ space.
The first one, called \cist, is a \emph{\underline{S}trong simplification}\cite{Trajcevski:DDR}, which outputs a subset of the original points;
and the second one, called \cista, is a \emph{\underline{W}eak simplification}\cite{Trajcevski:DDR}, which allows interpolating data into trajectories under certain conditions.


\stab (3) Using four real-life trajectory datasets (\truck, \sercar, \geolife, \pricar),
we finally conduct an extensive experimental study, by comparing our algorithms \cist and \cista with algorithms \dps~\cite{Meratnia:Spatiotemporal} (\sed equipped \dpa, the distinct existing \lsa algorithm being famous of compression ratio) and \squishe \cite{Muckell:Compression} (the existing \sed enabled \lsa algorithm with improved time performance).
%
