...
\sstab {(2)} We next develop two one-pass trajectory simplification algorithms \cist and \cista, \textcolor{blue}{achieving $O(n)$ time and $O(1)$ space}, based on our local synchronous distance checking technique. 
...
we finally conduct an extensive experimental study, by comparing our methods \cist and \cista  with algorithms \osed~(the optimal algorithm using \sed) \dps~\cite{Meratnia:Spatiotemporal} (the most effective existing \lsa algorithm using \sed) and \squishe \cite{Muckell:Compression} (the most efficient existing \lsa algorithm using \sed).

%{For execution time,} 
Algorithms \cist and \cista are on average ($14.21$, $18.19$, $17.06$, $9.98$) and ($2.84$, $3.45$, $3.69$, $2.86$) times faster than \dps and \squishe on (\sercar, \geolife, \mopsi, \pricar), respectively.
...
($109.1\%$, $109.7\%$, $113.5\%$, $109.2\%$) and {($134.3\%$, $133.3\%$,
  $148.4\%$, $135.4\%$)} of \squishe, \dps and \osed on ( \sercar, \geolife, \mopsi, \pricar), respectively. 
Moreover, algorithm \cista is comparable with the optimal algorithm and better than \squishe and \dps  that are on average ($57.9\%$, $58.8\%$, $48.4\%$, $54.6\%$),
($79.6\%$, $81.2\%$, $83.3\%$, $82.1\%$) and {($98.1\%$, $98.7\%$, $108.9\%$,
  $101.7\%$)} of \squishe, \dps and the optimal algorithm on (\sercar, \geolife, \mopsi, \pricar), respectively.
