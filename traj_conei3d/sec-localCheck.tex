...
Indeed, the intersection of two $m$-edge convex polygons can be computed in $O(m)$ time~\cite{ORourke:Intersection}.
...
The intersection polygon $\mathcal{G}^*_l \bigsqcap \mathcal{G}_{l+1}$ ($ 1\le l< k$) has at most the same number of edges as  $\mathcal{G}^*_l$.
\end{theorem}
