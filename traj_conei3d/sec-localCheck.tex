...
In this section, we develop a local synchronous distance checking approach such that each point in a trajectory is checked only once in $O(1)$ time during the entire process of trajectory simplification, by \textcolor{blue}{substantially} extending the sector intersection method in Section~\ref{sub-ci-ped} \textcolor{blue}{from a x-y 2D space to a Spatio-Temporal 3D space}, which lays down the key for the one-pass trajectory simplification algorithms \textcolor{blue}{using \sed} in Section~\ref{sec-alg}.
...
We first present the spatio-temporal cone intersection method in a 3D Cartesian coordinate system, which extends the sector intersection method~\cite{Williams:Longest, Sklansky:Cone, Zhao:Sleeve}. %in a 2D Cartesian coordinate system.
...
\stitle{Synchronous Circles (\circle{})}. The synchronous circle of a data point $P_{s+i}$ ($1\le i\le k$) in $\dddot{\mathcal{T}}_s$ \textcolor{blue}{\wrt an error bound $\epsilon$}, denoted as \circle{(P_{s+i}, \epsilon)}, or \circle{_{s+i}} in short, is a circle on the plane $P.t-P_{s+i}.t = 0$ such that $P_{s+i}$ is its center and $\epsilon$ is its radius.
...
\textcolor{blue}{For spatio-temporal cones having the same apex $P_s$, the checking of intersection of these cones can be enforced by a simpler way, \ie the checking of intersection of \textit{cone projection circle}s on a plane, as follows.}
...
\begin{figure}[tb!]
	\centering
	\includegraphics[scale=0.66]{figures/Fig-project-circle.png}
%	\vspace{-1ex}
...
Finding the common intersection of $N$ circles \textcolor{blue}{on a plane} has a time complexity of ${O(N\log N)}$~\cite{Shamos:Circle}, which cannot be used for designing one-pass trajectory simplification algorithms using \sed.
%
...
Given a cone projection circle \pcircle{(P, r)}, its inscribed $m$-edge regular polygon is denoted as $\mathcal{G}(V, E)$,
...
Let $\mathcal{G}_{s+i}$ ($1\le i \le k$) be the inscribed regular polygon of the cone projection  circle \pcircle{(P^c_{s+i}, r^c_{s+i})},
...
both edge $\vv{A_i}$ and edge $\vv{A_{i'}}$, which contradicts the assumption.
\end{proof}


\begin{figure}[tb!]
	\centering
	\includegraphics[scale=0.77]{figures/Fig-polygons.png}
	\vspace{-1ex}
...
%By Theorem~\ref{prop-rp-intersection}, the intersection polygon $\mathcal{G}^*_k$ of all approximate polygon $\mathcal{G}_i$, $i \in [1, k]$, has no more than $M$ edges. We know the convex polygon intersection algorithm, \ie \cpia, of Figure~\ref{alg:c-poly-inter} has a time complexity of $O(|\mathcal{G}^*_k| + |\mathcal{G}_{k+1}|)$, which is $O(2M)$ here, thus, the computing of the intersection of polygons $\mathcal{G}^*_k$ and $\mathcal{G}_{k+1}$ can be implemented in a constant time, \ie $O(M)$, by the \cpia algorithm.
...
we further replace a circle $\mathcal{O}(P, r)$ to a specially designed $M$-edges inscribed regular polygon,
