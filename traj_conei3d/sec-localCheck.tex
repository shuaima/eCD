...
In this section, we develop a local synchronous distance checking approach such that each point in a trajectory is checked only once in $O(1)$ time during the entire process of trajectory simplification, by substantially extending the sector intersection method in Section~\ref{sub-ci-ped} from a 2D space to a Spatio-Temporal 3D space, which lays down the key for the one-pass trajectory simplification algorithms using \sed (Section~\ref{sec-alg}).
