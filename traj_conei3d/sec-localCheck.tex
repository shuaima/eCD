...
\textcolor{blue}{The key idea of the local distance checking method using \sed is largely depending on the \emph{spatio-temporal cones} that convert the \sed distance tolerance into the intersection of spatio-temporal cones for testing the successive data points.}
\textcolor{blue}{More specifically, we first substantially extend the \textit{sectors} in Section~\ref{sub-ci-ped} from a 2D space to a Spatio-Temporal 3D space and get \emph{spatio-temporal cones},}
\textcolor{blue}{then we simplify the spatio-temporal cones intersection into the circles intersection, and finally, we approximate a circle by a specialized polygon (fixed rotation and edge number) so as to get the local synchronous distance checking approach.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textcolor{blue}{We then define the \textit{spatio-temporal cone} in the 3D coordinate system and present the \textit{spatio-temporal cone intersection} method which converts the \sed distance tolerance into the intersection of spatio-temporal cones.}

\stitle{Spatio-temporal cones (\cone{})}. Given the start point $P_s$ of $\dddot{\mathcal{T}_s}$ and an error bound $\epsilon$, the spatio-temporal cone (or simply \textit{cone}) of a data point $P_{s+i}$ ($1\le i\le k$) in $\dddot{\mathcal{T}_s}$ \wrt the point $P_s$ and the error bound $\epsilon$, denoted as \cone{(P_s, \mathcal{O}(P_{s+i}, \epsilon))}, or \cone{_{s+i}} in short, is an oblique circular cone such that point $P_s$ is its apex and the synchronous circle $\mathcal{O}(P_{s+i}, \epsilon)$ of point $P_{s+i}$ is its base.
...
\stitle{Cone projection circles}.
\textcolor{blue}{As the base of a cone \cone{(P_s, \mathcal{O}(P_{s+i}, \epsilon))}, \ie the synchronous circle $\mathcal{O}(P_{s+i}, \epsilon)$, is on the plane $P.t-P_{s+i}.t = 0$, which is sure parallel to any other plane $P.t- t_c = 0$, hence,} the projection of a cone \cone{(P_s, \mathcal{O}(P_{s+i}, \epsilon))} on a plane $P.t- t_c = 0$ ($t_c > P_s.t$) is a circle \pcircle{(P^c_{s+i}, r^c_{s+i})}, or \pcircle{_{s+i}} in short, such that
%
...
%
Intuitively, we can approximate a circle with its $m$-edge inscribed polygon {whose intersection can be computed more fast}. \textcolor{blue}{However, the intersection of two $m$-edge polygons may produce a polygon with more than $m$ edges, thus, the time complexity of finding the common intersection of $n$ polygons on a plane is not linear. To solve this problem, we approximate a circle with a fixed rotation, $m$-edge inscribed regular polygon.}
...
\textcolor{blue}{So far, we have designed a local distance checking method for \sed and proved its correctness. Next, we further improve the efficiency of the method by customizing the regular polygon intersection approach.}
