...
The red dashed circle \pcircle{(P^c_{s+i+1}, r^c_{s+i+1})} on plane $t=P_{s+i}.t$ in Figure~\ref{fig:cis} is the projection circle of cone \cone{(P_s, \mathcal{O}(P_{s+i+1}, \epsilon))}.
...
\begin{figure}[tb!]
\centering
\includegraphics[scale=0.68]{figures/Fig-cis.png}
\vspace{-2ex}
...
Figure~\ref{fig:polygons}-(1) illustrates an inscribed $m$-edge regular octagon of the cone projection  circle  \pcircle{(P, r)}.
