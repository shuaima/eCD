...
\textcolor{blue}{We first substantially extending the \textit{sector intersection} method in Section~\ref{sub-ci-ped} from a 2D space to a Spatio-Temporal 3D space, which converts the \sed distance tolerance into the intersection of spatio-temporal cones for testing the successive data points. }
\textcolor{blue}{Then we simplify the spatio-temporal cones intersection into the circles intersection and approximate a circle by a specialized polygon so as to get the local synchronous distance checking approach.}
