...
Two example spatio-temporal cones \cone{(P_s, \mathcal{O}(P_{s+i}, \epsilon))} {(purple)} and \cone{(P_s, \mathcal{O}(P_{s+k}, \epsilon))} \textcolor{blue}{(red) with the same apex $P_s$ and error bound $\epsilon$ are also} illustrated in Figure~\ref{fig:cis}.
...
The red dashed circle \pcircle{(P^c_{s+i+1}, r^c_{s+i+1})} on plane ``$P.t-P_{s+i}.t=0$" in Figure~\ref{fig:cis} is the projection circle of cone \cone{(P_s, \mathcal{O}(P_{s+i+1}, \epsilon))} on the plane.
...
\begin{figure}[tb!]
	\centering
	\includegraphics[scale=0.8]{figures/Fig-project-circle.png}
	\vspace{-1ex}
...
Finding the common intersection of $N$ circles \textcolor{blue}{on a plane} has a time complexity of ${O(N\log N)}$~\cite{Shamos:Circle}, which cannot be used
