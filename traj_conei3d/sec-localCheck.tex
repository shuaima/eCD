...
\textcolor{blue}{Finally, wesimplify the spatio-temporal cones intersection into the circles intersection, and approximate a circle by a special class of (fixed rotation and edge number) polygons.}
...
It is easy to see that for any point in the area of a circle \circle{(P_{s+i}, \epsilon)}, its distance to $P_{s+i}$ is no greater than $\epsilon$.

\sstab(2) Figure~\ref{fig:cis} also illustrates two example spatio-temporal cones: \cone{(P_s, \mathcal{O}(P_{s+i}, \epsilon))} {(purple)} and \cone{(P_s, \mathcal{O}(P_{s+k}, \epsilon))} (red), with the same apex $P_s$ and error bound $\epsilon$.
\end{example}
...
\textcolor{blue}{Indee, the \sed distance tolerance can be checked by finding the common intersection of spatio-temporal cones, as shown below.}
...
\textcolor{blue}{Recall that the base of a cone \cone{(P_s, \mathcal{O}(P_{s+i}, \epsilon))} is a cirle on plane $P.t-P_{s+i}.t = 0$, and plane $P.t- t_c = 0$ is parallel to plane $P.t-P_{s+i}.t = 0$. These  ensure that the projection of a cone is a circle.}
