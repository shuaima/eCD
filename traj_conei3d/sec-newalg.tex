...
Given a sub trajectory $[P_s,...,P_e]$, a constant $\epsilon$ and time $t_c > t_s$, algorithm \cista outputs point $Q$ of Corollary~\ref{prop-circle-intersection}, which is got by the following strategies.
...
\ni (4) At last, the algorithm outputs 3 continuous line segments, \ie $\vv{P_0P'_4}$, $\vv{P'_4P_8}$ and $\vv{P_8P_{10}}$. Note $P_8$ and $P_{10}$ are the original points.
\end{example}
...
\begin{proof}
For each data point, the procedures $getRegularPolygon()$ and $getIntersectPolygon()$ are both called one time.

\hspace{-2ex}(1) Procedure $getRegularPolygon()$ completes in $O(1)$ time, and,

\hspace{-2ex}(2) Procedure $getIntersectPolygon(\mathcal{G}^*, \mathcal{G})$ completes in $O(|\mathcal{G}^*| + |\mathcal{G}|)$ time. By Theorem~\ref{prop-rp-intersection}, we know that $|\mathcal{G}^*| \le M$ and $|\mathcal{G}|\le M$, where $M$ is the number of vertices in a regular polygon. Hence, $getIntersectPolygon()$ completes in $O(M)$ time.
