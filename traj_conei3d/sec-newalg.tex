...
\\
	{\bf Procedure} ~$getRegularPolygon(P_s,P_i,radius,M,t_c)$\\
%	\sstab
	\bcc \hspace{1ex} transform $P_i$ to Cartesian coordinate \\
	\icc \hspace{1ex}\= $k = t_c/(P_i.t - P_s.t)$ \\
	\icc \hspace{1ex} $radius = radius / k$ \\
	\icc \hspace{1ex} $P_i.x = P_i.x / k$; $P_i.y = P_i.y / k$ \\
	\icc \hspace{1ex} \For $(j = 0;j < M;j++)$ \\
	\icc \> \hspace{3ex} $angle = PI / M * (2 * i + 1)$ \\
	\icc \> \hspace{3ex} $x = P_i.x + cos(angle) * radius$\\
	\icc \> \hspace{3ex} $y = P_i.y + sin(angle) * radius$\\
	\icc \> \hspace{3ex} $Poly[j].x = x$; $Poly[j].y = y$\\
	\icc \hspace{1ex}\Return $Poly$
}
\vspace{-2ex}
\myhrule
\end{minipage}
