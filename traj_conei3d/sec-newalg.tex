...
This is performed by calling procedure $\rpia()$, as shown in Figure~\ref{alg:r-poly-inter}, which returns the intersection polygon of the two input polygons.
...
\vspace{-1ex}
\mat{0ex}{
	{\bf Procedure} ~$getRegularPolygon(P,start,radius,M)$\\
%	\sstab
	\bcc \hspace{1ex}\= $t_0 := 1000$; \\
%         \hspace{2ex}     $intersection = \emptyset$;   \\
	\icc \hspace{1ex}\= $k = t_0/P.time$ \\
	\icc \hspace{1ex} $P = transform(P,start)$ \\
	\icc \hspace{1ex} $radius = radius / k$ \\
	\icc \hspace{1ex} $P.x = P.x / k$ \\
	\icc \hspace{1ex} $P.y = P.y / k$ \\
	\icc \hspace{1ex} \For $(i = 0;i < M;i++)$ \\
	\icc \> \hspace{3ex} $angle = PI / M * (2 * i + 1)$ \\
	\icc \> \hspace{3ex} $x = P.x + cos(angle) * radius$\\
	\icc \> \hspace{3ex} $y = P.y + sin(angle) * radius$\\
	\icc \> \hspace{3ex} $res.vertices[i] = setVertex(x,y,i)$\\
	\icc \hspace{1ex}\Return $res$
%
%	{\bf procedure} $getPolygon$($P_s$, $P_i$, $\epsilon/2$) \\
%
%	{\bf procedure} $getIntersection(ipolygon, \mathcal{G})$ \\
