...
\> \> \> $R_i.X\asymp N$ \bd{and} \bd{not} ($R_i.Y\asymp R$ \bd{and} $R_i.Y\asymp N$)}
\end{footnotesize}

\begin{footnotesize}\mat{0ex}{
$Q_{j}^v$ \= \bd{select} \= \bd{distinct } \= $X_{L}$ \bd{from} ( \\
\> \> \bd{select} $L.\at{cid}$ \bd{as} \at{cid}, $X_{L}$, $Y_{R}$ \bd{from} $R_i$, \Enc{L} $L$, \Enc{R} $R$, \Enc{\ne} $N$ \\
\>\> \> \bd{where} $L.\at{cid}=R.\at{cid}$ \bd{and} $R_i.X\asymp L$ \bd{and} \\
...
\noindent for each $A_{j} \in X$; (3) $R_i.Y\asymp R$ is defined similarly for attributes in $Y$; (4) $R_i.X\asymp N$ is the conjunction of
...
\noindent for $A_{j} \in X$; (5) $R_i.Y\asymp N$ is defined similarly, but with $N.\at{pos}='\RHS'$; (6) $X_{L}$ is the set of following attributes

\begin{footnotesize}\mat{3ex}{
(\bd{case when} $L.A_j$ \bd{is null then null} \bd{else} $R_i.A_j$ \bd{end}) \bd{as} $A_{jL}$
}
\end{footnotesize}
\noindent for each $A_j \in X$; (7) Similarly, $Y_{R}$ is the set of
\begin{footnotesize}\mat{3ex}{
(\bd{case when} $R.B_k$ \bd{is null then null} \bd{else} $R_i.B_k$ \bd{end}) \bd{as} $B_{kR}$
}
\end{footnotesize}
\noindent for each $B_k \in Y$; (8) $R.Y='\_'$ is the disjunction of $R.B_k='\_'$ for each $B_{k} \in Y$;

Intuitively, detecting violations of \pCFDs in $\SCFD^i$ is a two-step process. Firstly, query $Q_{i}^{c}$ detects single-tuple violations, that is the tuples $t$ in $I_{i}$ that match the \LHS patterns of some \pCFDs in $\SCFD^i$, but $t$ does not match the corresponding \RHS patterns of these \pCFDs. Secondly, query $Q_{i}^{v}$ finds multi-tuple violations, that is, tuples $t_{1}$ in $I_{i}$ for which there exists a tuple $t_{2}$ in $I_{i}$ such that $t_{1}[X]=t_{2}[X] \asymp L$, but $t_{1}[Y] \neq t_{2}[Y]$. For each \CFD, query $Q_{i}^{v}$ uses the \bd{group by} clause to group tuples with the same value on $X$ and it counts the number of distinct $Y$. If there is more than one instantiation, then there is a violation.
...
$Q_{1}^{v}$\= \bd{select} \= \bd{distinct} \= \at{sale_L}, \=\at{price_L} \bd{from} ( \\
 ...
 \> (\bd{case when} $L.\at{sale}$ \bd{is null then null else} $R_1.\at{sale}$ \bd{end}) \bd{as} \at{sale_L},\\
 \> (\bd{case when} $L.\at{price}$ \bd{is null then null else} $R_1.\at{price}$ \bd{end}) \bd{as} \at{price_L},\\
 \> (\bd{case when} $R.\at{shipping}$ \bd{is null then null else} $R_1.\at{shipping}$ \bd{end}) \bd{as} \at{shipping_R},  \\
 \> (\bd{case when} $R.\at{price}$ \bd{is null then null else} $R_1.\at{price}$ \bd{end}) \bd{as} \at{price_R}  \\
...
\hspace{-3ex} \=  \bd{select} $R_1.*$ \bd{from} \at{item} $R_1$, \Enc{L} L, \Enc{\ne} N \\
