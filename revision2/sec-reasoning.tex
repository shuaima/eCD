...
\vspace{-1ex}
\stitle{Remarks}. Inference systems play an important role for the implication analyses \cite{AbHuVi1995}.  For the inference system of \pCFDs and \pCINDs alone, we can readily extend the one for \CFDs \cite{CFDs} and \CINDs \cite{tcs-CINDs}, respectively, by deliberately handling the entailment of ordered pattern values involved with built-in predicates and their interaction with the wildcard `\_'. The details are left to interested readers.  Note that it is easy to know that the implication analysis of \pCFDs and \pCINDs together is not finitely axiomatizable by Corollary~\ref{thm-IM-pcfd-pcind}.
