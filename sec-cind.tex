%\vspace{-1ex}
\section{Incorporating Built-in Predicates into CINDs}
\label{sec-cind}
%\vspace{-1ex}

Along the same lines as \pCFDs, we next define \pCINDs, also
referred to as {\em conditional inclusion dependencies}.
...
constant in $\dom(A)$.
 ...
 We denote $X \cup X_p$ as $\LHS(\psi)$ and $Y \cup Y_p$ as $\RHS(\psi)$, and
separate the $X_p$ and $Y_p$ attributes in a pattern tuple with
`$\pa$'. We use \kw{nil} to denote an {\em empty} list. \eat{Relation
...
or (b) there exist $C \in Y, A \in X$ such that $A$ corresponds to $B$ in the
\IND and $t_p[C]$ and $t_p'[A]$ are not satisfiable
when taken together;
...
\stitle{Special cases.}~Observe that (1) a standard
\CIND $(R_1[X] \subseteq R_2[Y])$ can be expressed as a \pCIND
$(R_1[X;\ \NIL] \subseteq R_2[Y;\ \NIL],\ T_p)$ such that $T_p$ is
simply a {\em empty} set; and (2) a \CIND $(R_1[X;\ X_p]
\subseteq R_2[Y;\ Y_p],\ T_p)$ with $T_p = \{t_{p1},\ldots,
t_{pk}\}$ can be expressed as a set
$\{\psi_1,\ldots,\psi_k\}$ of \pCINDs, where for $i\in [1, k]$,
$\psi_i$ = $(R_1[X;\ X_p] \subseteq R_2[Y;\ Y_p],\ T_{pi})$ such
that $T_{pi}$ consists of a single pattern tuple $t_{pi}$ of $T_p$
defined in terms of equality ($=$) only.
