...
Web graph  & 3,563 M  & 1,828 M & $>$25\%     \\  \hline
\end{tabular}
\vspace{-.5ex}
\end{small}
\end{center}
\caption{\small Statistics of citation graphs and Web graphs~\cite{MeuselVLB14,LeskovecLDM09}.}
\label{tab-batch}
\vspace{-8ex}
\end{table}
%%%%%%%%%%%%%%%%%%%
...
Due to the existence of the ``bow tie'' structure and the giant \scc in Web graphs~\cite{BroderKMRRSTW00}, the edge ratio $|E_i|/|E|$ could be greater than 25\% of $|E|$ even if edges were randomly generated. In contrast, $|E_i|/|E|$ is less than 1\% for all tested scholarly articles.
%
Taking $t=100$ for example, algorithm \twprscc needs to scan only about $2|E|$ edges on citation graphs, while $26|E|$ edges on Web graphs.
...
By the analyses above and Lemma~\ref{prop-venue-time-complexity}, it is easy to verify that algorithm \batensemble runs in $O(|V^c|+|E^c_a|+t|E^c_i|+|V^v|+|E^v_a|+t|E^v_i|+|PA|)$, where $|PA|$ is the total number of author-article relationships.


Compared with the traditional power method~\cite{Brin98:PageRank} , algorithm \batensemble uses $(2|V^c{'}|+|E^c{'}|)$ and $(2|V^v{'}|+|E^v{'}|)$ extra space to store the converted graph $G^c{'}(V^c{'},E^c{'})$ of $G^c$ and $G^v{'}(V^v{'},E^v{'})$ of $G^v$, and their topological orders. To conclude, algorithm \batensemble only uses limited extra space to speed-up computation.
