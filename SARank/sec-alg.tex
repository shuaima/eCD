...
%
In the Web graph with the ``bow tie" strucutre~\cite{BroderKMRRSTW00}, the nodes in the largest SCC amount to 51\% of the entire graph in 2014~\cite{MeuselVLB14}, and $|E_i|$ is already 25\% of $|E|$ when edges are randomly generated.
%
...
Based on the above analysis, algorithm \twprscc performs much better on scholarly data than the Web graph. Taking $t=100$ for example, the complexity is $O(|V|+2|E|)$ and $O(|V|+25|E|)$ on scholarly data and the Web graph, respectively.
%
By Lemmas \ref{prop-prscc} \& \ref{prop-venue-time-complexity}, we have shown Fact~\ref{thm-prestige}.
