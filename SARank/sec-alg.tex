...
The popularity can be computed by scanning through all citations once. While the prestige of entities is based on TWPageRank, which is usually computed in an iterative manner\cite{Brin98:PageRank}. %with the time complexity being $O(t\cdot (|E|+|V|))$ on graph $G=(V,E)$ for $t$ iterations.
...
We first present the details of prestige computation.
%
The prestige of articles and venues is computed with TWPageRank on citation graphs and venue graphs, which occupies most computation cost of importance computation.
%
While the prestige of authors is defined as the average prestige of their published articles, it suffices to scan through all author-article  relationships for computing the prestige of authors.
%
...
%by a block-based PageRank computation method


\stitle{Temporal order}. Scholarly data follows temporal order in the sense that an article can only cite the articles published earlier, and, moreover, it is pretty rare that two articles published in the same time mutually cite each other.
...
%according to the statistics of three scholarly datasets,

We next prove Theorem~\ref{thm-prestige} by showing that a block-wise PageRank computation method achieves the desired property by using characteristics of scholarly data obeying the temporal order, such that each block equals to an SCC of the original graph and the directed and acyclic structure of the converted graph can improve the efficiency of PageRank computation~\cite{Berkhin05}.
...
\stitle{Time \& space complexity analyses}. By the analyses above and Lemma~\ref{prop-venue-time-complexity}, it is easy to verify that algorithm \batensemble runs in $O(|V^c|+|E^c_b|+t|E^c_w|+|V^v|+|E^v_b|+t|E^v_w|+|PA|)$, where $|PA|$ is the total number of author-article relationships.
