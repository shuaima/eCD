...
Indeed, \ensemblerank improves the \PairAcc over (\pagerank, \futurerank, \hhgrank) by (15.9\%, 3.8\%, 4.8\%) on \aan, (11.7\%, 3.7\%, 3.2\%) on \aminer, and  (11.7\%, 4.4\%, 1.5\%) on \magdata, respectively.
...
When varying $dif$, the \PairAcc of all algorithms increases with the increment of $dif$, since scholarly article pairs with larger $dif$ are easier to rank. Moreover, \ensemblerank consistently ranks better than all competitors, regardless of easy or difficult article pairs. Indeed, \ensemblerank improves the \PairAcc over (\pagerank, \futurerank, \hhgrank) by (11.2\%, 2.1\%, 3.0\%) on \aan, (9.6\%, 4.1\%, 3.0\%) on \aminer, and (6.8\%, 4.0\%, 1.2\%) on \magdata, on average, respectively.
...
When varying $\sigma$, the \PairAcc of \ensemblerank is very stable on all datasets using both \recom and \fcita. Indeed, with \recom and \fcita, the \PairAcc only varies (0.42\%, 1.45\%, 0.81\%) and (0.70\%, 0.53\%, 0.66\%) on (\aan, \aminer, \magdata), respectively.
%
...
Indeed, with \recom and \fcita, the \PairAcc of combining prestige and popularity is (10.2\%, 12.1\%, 5.5\%) and (3.7\%, 6.4\%, 7.6\%) higher than using prestige alone, and is (1.2\%, -0.73\%, 1.0\%) and (0.19\%, 0.03\%, 0.06\%) higher than using popularity alone on (\aan, \aminer, \magdata), respectively.
...
\aminer & 0.684 \ 0.565 \ 0.679 & 0.689 \ 0.746 \ 0.742 & {\bf 0.750} \\
...
Moreover, algorithm \ensemblerank is consistently better than \drank when $\lambda \ge 0.6$, which, with \recom and \fcita, improves the \PairAcc by (2.92\%, 6.93\%, 4.51\%) and (0.51\%, 0.97\%, 0.27\%) on (\aan, \aminer, \magdata) on average, respectively.
...
Overall, with \recom and \fcita, \ensemblerank improves the \PairAcc over \drank by (1.78\%, 3.33\%, 3.20\%) and (0.25\%, 0.51\%, 0.16\%) on (\aan, \aminer, \magdata) on average, respectively.
}
