...
\includegraphics[scale=0.37]{./exp/AAN_TWPageRank_recom.eps}}
\hfill
\subfigure[{\scriptsize \aminer with \recom}]{\label{exp-aminer-recom-drank}
\includegraphics[scale=0.37]{./exp/AMiner_TWPageRank_recom.eps}}
\hfill
\subfigure[{\scriptsize \magdata with \recom}]{\label{exp-mag-recom-drank}
\includegraphics[scale=0.37]{./exp/MAG_TWPageRank_recom.eps}}
\\%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{-1.5ex}
\subfigure[{\scriptsize \aan with \fcita}]{\label{exp-aan-fcita-drank}
\includegraphics[scale=0.37]{./exp/AAN_TWPageRank_fcita.eps}}
\hfill
\subfigure[{\scriptsize \aminer with \fcita}]{\label{exp-aminer-fcita-drank}
\includegraphics[scale=0.37]{./exp/AMiner_TWPageRank_fcita.eps}}
\hfill
\subfigure[{\scriptsize \magdata with \fcita}]{\label{exp-mag-fcita-drank}
\includegraphics[scale=0.37]{./exp/MAG_TWPageRank_fcita.eps}}
\end{center}
\vspace{-2.5ex}
...
\etitle{Exp-4.4}.
\marked{To evaluate the impacts of the proposed TWPageRank, we compared our approach \ensemblerank with an algorithm alternative (referred to as \drank) the same to \ensemblerank except exploiting exponentially decayed impact weights~\cite{Wang13AAAI,sayyadi09,WalkerXKM07}, \ie $w(u,v)=e^{\sigma(T_u-T_v)}$ in Eq.~(\ref{eq-infl-weights}).
%The two algorithms produce the same popularity while different prestige. 
To better understand the impacts, we varied the importance weighting factor $\lambda$ from 0.1 to 1 and tested the effectiveness and efficiency of algorithms \ensemblerank and \drank. Note that \ensemblerank and \drank produce the same results when $\lambda=0$ since their popularity computations are the same. The results are reported in Fig.~\ref{exp-drank}, where the numbers represent the improvement of \PairAcc by \ensemblerank over the one by \drank.}

\marked{
When varying $\lambda$, the \PairAcc of \ensemblerank is better than the one of \drank in most cases, except on \aminer with \fcita, which shows the superiority of the TWPageRank than exploiting exponentially decayed weights as most previous work does. 
The difference of \PairAcc by the two algorithms is higher with \recom than with \fcita, possibly because the article pairs in \fcita are of the same years.
Moreover, algorithm \ensemblerank is consistently better than \drank when $\lambda \ge 0.9$, which, with \recom and \fcita, improves the \PairAcc by (2.18\%, 6.94\%, 5.02\%) and (0.70\%, 0.13\%, 0.27\%) on (\aan, \aminer, \magdata) on average, respectively. The region of $\lambda$ when \ensemblerank outperforms \drank is even larger if considering \aan and \magdata only.
...
Overall, with \recom and \fcita, \ensemblerank improves the \PairAcc over \drank by (1.73\%, 2.97\%, 2.92\%) and (0.22\%, -0.08\%, 0.17\%) on (\aan, \aminer, \magdata) on average, respectively.
}
