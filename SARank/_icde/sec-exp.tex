...
Indeed, \ensemblerank improves the \PairAcc over (\pagerank, \futurerank, \hhgrank) by (15.9\%, 3.8\%, 4.8\%) on \aan, (19.4\%, 7.7\%, 6.0\%) on \aminer, and  (11.7\%, 4.4\%, 1.5\%) on \magdata, respectively.
...
\marked{
When varying $\lambda$, the \PairAcc of \ensemblerank is better than the one of \drank in most cases, except on \aminer with \fcita, which shows the superiority of the TWPageRank than exploiting exponentially decayed weights.
