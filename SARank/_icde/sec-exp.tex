...
\begin{center}
%\hspace{10ex}
\subfigure[{\scriptsize \aan}]{\label{fig-aminer-time1}
\includegraphics[scale=\graphscale]{./exp/AAN_TWPageRank_recom.eps}}
\hfill
\subfigure[{\scriptsize \aminer}]{\label{fig-aminer-time2}
\includegraphics[scale=\graphscale]{./exp/AMiner_TWPageRank_recom.eps}}
\hfill
\subfigure[{\scriptsize \magdata}]{\label{fig-mag-time1}
\includegraphics[scale=\graphscale]{./exp/MAG_TWPageRank_recom.eps}}
\\%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{-1.5ex}
\subfigure[{\scriptsize \aan}]{\label{fig-aminer-time1}
\includegraphics[scale=\graphscale]{./exp/AAN_TWPageRank_fcita.eps}}
\hfill
\subfigure[{\scriptsize \aminer}]{\label{fig-aminer-time2}
\includegraphics[scale=\graphscale]{./exp/AMiner_TWPageRank_fcita.eps}}
\hfill
\subfigure[{\scriptsize \magdata}]{\label{fig-mag-time1}
\includegraphics[scale=\graphscale]{./exp/MAG_TWPageRank_fcita.eps}}
\end{center}
\vspace{-2.5ex}
\caption{\small Accuracy tests with \recom ((a)--(c)) and \fcita ((d)--(f)): varying importance weighting
factor $\lambda$}
\label{fig-time}
\vspace{-2ex}
\end{figure*}
...
\marked{
\etitle{Exp-4.4}.
To evaluate the impacts of the proposed TWPageRank, we compared our approach \ensemblerank with alternatives the same to \ensemblerank except exploiting exponentially decayed impact weights, \ie $w(u,v)=e^{\sigma(T_u-T_v)}$ in Eq.~(\ref{eq-infl-weights}). We varied $\lambda$ from 0.1 to 1 to test both the effectiveness of using prestige alone and of combining prestige and popularity, since there has no effect on popularity.}

\marked{
With \recom and \fcita, the \PairAcc of \ensemblerank is (2.20\%, 6.63\%, 5.68\%) and (0.25\%, 0.05\%, 0.05\%) higher than its counterparts exploiting exponentially decayed impact weights on (\aan, \aminer, \magdata), respectively, when using prestige alone, and is (1.73\%, 2.97\%, 2.92\%) and (0.22\%, -0.08\%, 0.17\%) higher when combining prestige and popularity, on average.
%
With the increment of $\lambda$, \ensemblerank has more promotion than TWPageRank, possibly due to the addition of popularity will correct the mistaken pairs better on TWPageRank, although \ensemblerank rank more pairs correctly.
...
Moreover, the \PairAcc of \ensemblerank is better than its counterparts in most cases, except when combining prestige and popularity on \aminer with \fcita. Recall that in Fig.~\ref{fig-aminer-lambda} using popularity alone gives the best results on \aminer, indicating that the prestige computed by TWPageRank is less accurate on \aminer than on the other two datasets.
%
...
%
Our incremental algorithm \incensemble is on average (1.8, 3.0, 3.1, 79) and (2.0, 3.0, 4.4, 245) times faster than (\batensemble, \powensemble, \futurerank, \hhgrank)  on the large \aminer and \magdata, respectively.
