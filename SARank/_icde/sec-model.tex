...
\noindent(2) \marked{The semantics of citation relationships are time-dependent, which means that citations at different periods of time potentially reveal different information. This has already been  exploited for scholarly article ranking~\cite{Li08TSRanking,Wang13AAAI,WangSB13}, while PageRank does not consider temporal information at all.}
...
\marked{Recall that~\cite{Chakraborty15} categorizes all articles into six citation patterns featured by the time when the articles reach their citation peaks. Typical patterns are {\em PeakInit} with the citation-count peak in the first five years (but not the first year) after publication, {\em PeakMul} with multiple peaks at different time points and {\em PeakLate} with a single peak in at least five years after publication. That is, the impact of an article is time-dependent, but not simply in the form of exponential decay if taking the number of citation as an indicator of the impact of an article~\cite{WangSB13,Garfield471}.}  In general, the impact of an article tends to decay with time after the peak time only.
