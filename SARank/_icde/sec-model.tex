...
\noindent(2) \marked{The semantics of citation relationships} are time-dependent, which has been successfully exploited for scholarly article ranking~\cite{Li08TSRanking,Wang13AAAI,WangSB13},
...
Based on the above statistics, we propose TWPageRank that evaluates the prestige of nodes in a directed graph, such that each node is attached with time information. It differs from PageRank by weighting the influence propagation using the {\em impact weights on edges}, which represent the relative amounts of time-dependent prestige that should be propagated from the edge sources to targets.
%
