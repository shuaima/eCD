...
\marked{
For scholarly article ranking, $Peak_v$ should be ideally set to the time when article $v$ has the highest impacts on others. Basically, it can be the year when the article obtains the largest number of citations. However, this will let $Peak_v$ bias to recent years since scholarly data is dynamic and continuously growing and the number of citations  by year grows rapidly. This is due to the increased numbers of published articles and the increased average number of references of each article over time. To evaluate to what extent such bias is, we plot the (log) number of yearly-references of three real-life scholarly datasets (\aan, \aminer and \magdata) in Fig.~XX, where an exponential distribution of yearly-references can be observed. To handle this, we compute $\Psi_v^{(t)}=\Phi_v^{(t)} / \log Z^{(t)}$ where $\Phi_v^{(t)}$ and $Z^{(t)}$ are the numbers of citations of article $v$ at year $t$ and the total number of references at year $t$, respectively, and set $Peak_v$ to the year that maximizes $\Psi_v^{(t)}$.
}

\textbf{gc: add a figure of the (log) numbers of yearly-references.}
