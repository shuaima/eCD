...
\marked{Recall that~\cite{Chakraborty15} categorizes all articles into six citation patterns featured by the time when the articles reach their citation peaks. These patterns are {\em PeakInit} with the citation-count peak in the first five years (but not the first year) after publication, {\em PeakMul} with distinct multiple peaks, {\em PeakLate} with a single peak in at least five years after publication, {\em MonDec} with monotonically decreasing citation counts, {\em MonIncr} with monotonically increasing citation counts, and, finally, {\em Other} for articles whose average numbers of citations per year are less than 1. That is, the impacts of article are time-dependent, but not simply in the form of exponential decay if taking the number of citation as an indicator of the impacts of articles~\cite{WangSB13,Garfield471}.}  In general, the impact of an article tends to decay with time after the peak time only.
...
Note that {\em each individual article has its own peak time} as articles may reach their citation peaks in different pattern.
