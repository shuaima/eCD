...
\noindent(2) \marked{The semantics of citation relationships are time-dependent, which means that citations at different periods of time potentially reveals different information, and has already been  exploited for scholarly article ranking~\cite{Li08TSRanking,Wang13AAAI,WangSB13}, while PageRank does not consider temporal information at all.}
...
\marked{Recall that~\cite{Chakraborty15} categorizes articles into six citation patterns featured by the time when the articles reach their citation peaks. Typical patterns are {\em PeakInit} with the citation-count peak in the first five years following publication (but not the first year), {\em PeakMul} with multiple peaks at different time points and {\em PeakLate} with a single peak after at least five years after publication. That is, the impact of an article is time-dependent, but not simply in the form of exponential decay if taking the number of citation as an indicator of the impact of an article~\cite{WangSB13,Garfield471}.}  In general, the impact of an article tends to decay with time after the citation peak only.
...
Note that {\em each individual article has its own peak time} as articles may reach their citation peaks in different time.
...
\marked{
For scholarly article ranking, $Peak_v$ should be ideally set to the time when article $v$ has the highest impacts on others. Basically, it can be the year when the article obtains the largest number of citations. However, this will let $Peak_v$ bias to recent years since scholarly data is dynamic and continuously growing and the number of references by year grows rapidly. This is due to both the increased numbers of published articles and the increased number of references of each article over time. To evaluate to what extent such bias is, we plot the fraction of references of each year on three real-life scholarly datasets (\aan, \aminer and \magdata) in Fig.~\ref{fig-reference}, where the y-axis is log-scaled and an exponential distribution of the number of yearly references can be observed. To handle this, we compute scaled number of citations $\Psi_v^{(t)}=\Phi_v^{(t)} / \log Z^{(t)}$ where $\Phi_v^{(t)}$ and $Z^{(t)}$ are the numbers of citations of article $v$ at year $t$ and the total number of references at year $t$, respectively, and set $Peak_v$ to the year that maximizes $\Psi_v^{(t)}$.
}
...
\vspace{-2ex}
\caption{\small Reference statistics of scholarly articles}
\label{fig-reference}
\vspace{-3ex}
\end{figure}
...
are two TWPageRank vectors after $t$ and $t-1$ iterations, respectively, $M$ is the transition matrix such that $M_{u,v}=w(u,v)/W(u)$ and $e$ is an $n$-dimensional all-one vector $[1]_{n\times1}$.
...
%
\marked{Different from $Peak_v$ of Eq.~(\ref{eq-infl-weights}), the popularity of an articles is a weighted sum over all its citations, and the numbers of references by year have little impacts on that. Moreover, the motivation of popularity is to assign higher ranks to articles of recent attention, which is somehow biased to recent articles.}
%
...
\end{equation}
\end{small}
\noindent Here aggregating parameters $\alpha$ and $\beta$ are used to regularize the contributions of the citation, venue and author information. Intuitively, these parameters indicate the intensity of the correlation between the importance of scholarly articles and the specific information.
