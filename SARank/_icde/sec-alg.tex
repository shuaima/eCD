...
\marked{We first introduce the framework of our batch algorithm, and then go deep into the details.}
...
The vector $PR$ returned by~\twprscc converges such that $||PR-PR^*||_1 < \epsilon$ given the convergent TWPageRank vector $PR^*$~\cite{Berkhin05}.
\end{cor}
...
%
Taking $t=100$ for example, algorithm \twprscc needs to scan through $2|E|$ and $4|E|$ edges on citation and venue graphs, respectively, while over $50|E|$ edges on Web graphs.
