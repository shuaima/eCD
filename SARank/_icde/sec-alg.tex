...
\aminer  & 3.14M/56K & 6.3M/2.8M   & 23/530 &  0.1\%/1.8\%     \\ %\hline
...
Due to the existence of the ``bow tie'' structure and the giant \scc in Web graphs~\cite{BroderKMRRSTW00}, the edge ratios $|E_i|/|E|$ are greater than 59\% for all Web graphs~\cite{LeskovecLDM09}. \marked{In contrast, the \sccs in citation and venue graphs are quite small as a result of the temporal order, and $|E_i|/|E|$ is less than 3\% for all tested citation graphs and venue graphs of scholarly data.
%
This specific structure in scholarly data has long been ignored in the literature, which has a large impact on the running time.} Taking $t=100$ for example, algorithm \twprscc needs to scan through $2|E|$ and $4|E|$ edges on citation and venue graphs, respectively, while over $59|E|$ edges on Web graphs.
