...
$\Delta V\cap V = \emptyset$ is a set of nodes, and $\Delta E$ is a set of directed edges on $\Delta V$ and from $\Delta V$ to $V$ only, as article citations obey a natural temporal order, \ie an article only cites those published earlier, and it is rare for the mutual citations between two articles published in the same time.
...
\sstab(2) Let $V_A = V\setminus V_B$. Since nodes in $V_A$ are not reachable from newly added or affected nodes, $V_A$ is essentially not affected by the update $\Delta$. And TWPageRank scores on $V_A$ can be simply adjusted by scaling existing results with constant ${n}/{n^+}$.
...
\begin{figure}[tb!]
\centering
\includegraphics[scale=0.8]{fig/General_framework_peak.eps}
\vspace{-2ex}
...
\hline
{\bf Statis.} & \aan & \aminer & \magdata & \aan & \aminer & \magdata \\
\hline \hline
$|V_A|$ & 46.9\% & 81.4\% & 69.5\% & 2.1\% & 45.8\% & 12.4\% \\
$|V_B|$ & 47.3\% & 10.7\% & 25.9\% & 92.0\% & 47.7\% & 84.6\% \\
$|V_C|$ & \ 5.8\% & \ 7.8\% & \ 4.5\% & \ 5.8\% & \ 6.4\% & \ 3.0\% \\ \hline
% venue graphs
$|E_A|$ & \ 2.9\% & 18.0\% & \ 1.0\% & \ 1.0\% & \ 0.0\% & \ 0.0\% \\
$|E_{AB}|$ & 25.9\% & 53.2\% & 27.4\% & \ 1.0\% & \ 3.2\% & \ 0.1\% \\
$|E_B|$ & 60.5\% & 25.1\% & 64.5\% & 88.8\% & 95.4\% & 92.6\% \\
$|E_{CB}|$ & 10.4\% & \ 2.3\% & \ 7.0\% & 10.0\% & \ 1.0\% & \ 7.1\% \\
$|E_C|$ & \ 0.3\% & \ 1.4\% & \ 0.1\% & \ 0.2\% & \ 0.1\% & \ 0.1\% \\ \hline
\end{tabular}
\end{small}
\end{center}
\caption{\small Statistics of affected/unaffected areas on citation graphs (columns 2--4) and venue graphs (columns 5--7) given a yearly update, \ie articles of 2011 on \aan and 2015 on \aminer and \magdata, respectively.}
\label{tab-inc}
\vspace{-6ex}
\end{table}
%%%%%%%%%%%%%%%%%%%
