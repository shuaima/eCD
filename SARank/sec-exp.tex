...
The results of \PairAcc are reported in Figs.~\ref{fig-aan-futureyear}, \ref{fig-aminer-futureyear} and \ref{fig-mag-futureyear}, in which red markers $\Box$ in dashed lines mean that \hhgrank ran out of memory.
...
Again, parameters $\alpha$ and $\beta$ have few impacts on efficiency. Here we only present the main results and more details are available at~\cite{SARank-full}.
...
\noindent(1) Our model \ensemblerank is effective for ranking scholarly articles, which is consistently better than competitive methods in all tests. With \recom and \fcita, \ensemblerank improves \PairAcc over (\pagerank, \futurerank, \hhgrank) by
...
\noindent(3) Our ranking model \ensemblerank introduces the time decaying factor $\sigma$, importance weighting factor $\lambda$ and aggregating parameters $\alpha$ and $\beta$ for the sake of practicability and flexibility in real-life applications, and \ensemblerank is very robust to these parameters.
