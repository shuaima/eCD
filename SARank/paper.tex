...
%\newcommand{\twprscc}{{\sc VTWPR}\xspace}
\newcommand{\twprscc}{{\sc TWPR}\xspace}
\newcommand{\inctwprscc}{\kw{inc}{\sc TWPR}\xspace}
\newcommand{\powtwprscc}{\kw{pow}{\sc VTWPR}\xspace}
...
%\titlenote{corresponding author}
%\vspace{-.5ex} \\
%{\affaddr  SKLSDE Lab, Beihang University, China}\\
%{\affaddr  Beijing Advanced Innovation Center for Big Data and Brain Computing, China}
%\vspace{0ex} \\
%{\affaddr \{mashuai, gongchen, hurenjun, lds1995, hucm, huaijp\}@buaa.edu.cn }
%}
...
%an ensemble enabled
an importance assembling approach by Time-Weighted PageRank, which extends traditional PageRank with a time decaying factor,
and assembling the importance of heterogeneous entities involved in scholarly articles.
\marked{After showing that a block-wise PageRank computation method is a good choice for computing Time-Weighted PageRank on scholarly data, we then propose both batch and incremental algorithms for efficient ranking computation.}
%We also propose to use external data sources to further improve the ranking accuracy.
We finally experimentally show that our approach is both effective and efficient for ranking scholarly articles. %, using real-life datasets.
\end{abstract}
