...
As the popularity of articles is defined as the freshness sum of all their citations, it is convenient to maintain. Consider an updated citation graph $G^{c,+}(V^c\cup\Delta V^c, E^c\cup\Delta E^c)$ of $G^c(V^c, E^c)$,  the updated popularity $pop_{c}^+(v)$ can be easily computed as:
\begin{small}
...
\hspace{6ex} topological order $O$ of the block-wise graph $G'$.\\
...
\icc\> label all SCCs of $G_C$ as $C$ and all SCCs of $G$ with outgoing edges having \\ \hspace{2.5ex} weight changes as $B$,  all remailing SCCs of $G$ as $A$\\
...
When processing on $V_B$ with Eq.~(\ref{eq-inc-prscc}), {\em edges in $E_{AB}$ can be skipped}  since $PR^+(u)={n}/{n^+}\cdot PR(u)$ for nodes $u\in V_A$ and $M_{u,v}=M^+_{u,v}$ given $(u,v)\in E_{AB}$. Besides, we use ${n}/{n^+}\cdot PR$ as the initial iteration vector. Both of them can speed up the computation.
...
%
The original graph $G$ is then partitioned into affected and unaffected areas, in which subgraphs $G_A$, $G_B$ and $G_C$ are associated with node sets $V_A$, $V_B$ and $\Delta V$, respectively. Here the citation peak time of node $x$ is changed, and, hence, node $y$ as well as all nodes reachable from $y$ are included in $G_B$.
%
...
\begin{proofSketch}
Assume a topological order $v_1'/\dots/v_{l}'$ of block-wise graph $G^+{'}$ where $l=|O^+|$. It suffices to prove the sum of change of $PR^+(v)$ where $v\in scc_k$ is no more than $\epsilon\cdot |scc_k|/|V^+|$ for $scc_k$ corresponding to $v_k'$ ($k\in [1,l]$) by induction.
...
(b) It saves $O(|E^c_A \cup E^c_{AB}|)$ time when updating scores on $V^c$, where edges in $E^c_A\cup E^c_{AB}$ accounts for more than 28\% of total edges;
...
\eat{By the analyses above, the time complexity of \incensemble is the same as \batensemble, except that \incensemble saves $O(|E^c_A\cup E^c_{AB}|)$ and $O(|E^v_A\cup E^v_{AB}|)$ time on the updated citation graphs and venue graphs when updating scores. And its space complexity is also the same as \batensemble, except that it uses $(|V^c\cup\Delta V^c|+|V^v\cup\Delta V^v|)$ extra space to store the affected/unaffected areas and $(|E^c\cup \Delta E^c|+|E^v\cup \Delta E^v|)$ extra space to store the original edge weights.


Indeed, algorithm \incensemble is typically faster than \batensemble as follows.
(a) It saves $O(|V^c|+|E^c|)$ and $O(|V^v|+|E^v|)$ time when maintaining \sccs and the topological order based on $\Delta^c$ and $\Delta^v$ only;
...
%
According to our statistics on citation and venue graphs in Table 2, $|V_A\cup V_B|$ (\ie $|V|$) and $|E_A\cup E_{AB}\cup E_B|$ (\ie $|E|$) are more than 89\% of $|V^+|$ and $|E^+|$ on all graphs, respectively, and edges in $E_A\cup E_{AB}$ accounts for more than 28\% of total edges on all citation graphs.}
