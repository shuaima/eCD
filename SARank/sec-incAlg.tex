...
%
As a result, TWPageRank scores on $V_B$ are recomputed by iterations as follows.
...
When processing $V_B$ with Eq.~(\ref{eq-inc-prscc}), the {\em edges in $E_{AB}$ can be skipped}  since $PR^+(u)={n}/{n^+}\cdot PR(u)$ for nodes $u\in V_A$ and $M_{u,v}=M^+_{u,v}$ such that $(u,v)\in E_{AB}$. Besides, we use ${n}/{n^+}\cdot PR$ as the initial iteration vector. Both of them can speed up the computation.
