...
As the popularity of articles is defined as the freshness sum of all their citations, it is convenient to maintain. Again, we denote the popularity of node $v$ on citation graphs $G^c$ and $G^{c,+}$ as $pop_c(v)$ and $pop_c^+(v)$, respectively.
...
Moreover, the popularity computation for venues and authors is exactly the same as \batensemble, similar to the incremental prestige computation for authors.
%due to the way that the  popularity of venues in a specific year and authors is defined, and the popularity of every article is potentially changed.


\stitle{Incremental prestige computation}.  We first present the incremental  prestige computation. Again, it differs for the prestige of articles and venues in a specific year and the one of authors.

The incremental prestige computation for authors is exactly the same as \batensemble, due to the way that the prestige of authors is defined and the prestige of each article is potentially changed. Indeed, it is meaningless to develop incremental algorithm for that due to the large number of affected authors.
...
%
Algorithm~\twprscc maintains two auxiliary data structures for computing TWPageRank: a) a mapping $Map$ that, given a node of the citation graph or venue graph $G$, returns the index of the SCC it belongs to,
and b) a sequence $Seq$ of a valid topological order of the block-wise graph $G'$.
Observing that both of $Map$ and $Seq$ can be easily maintained given the update $\Delta$, the idea is to divide $G$ into affected and unaffected areas such that TWPageRank scores of nodes in affected and unaffected areas are updated accordingly.


%% DS maintain
\stitle{Auxiliary data structure maintain}. The two auxiliary data structures are dynamically maintained as follows.

\sstab(1) We extend $Map$ with nodes of $\Delta V$ as well as their corresponding SCC indexes computed on the induced subgraph $G^+[\Delta V]$, based on the fact that there only exist edges from $\Delta V$  to $V$.
%The set of SCCs on the updated graph $G^+$ consists of existing SCCs on $G$ and SCCs on the induced subgraph $G^+[\Delta V]$, based on the fact that there only exist edges from $\Delta V$  to $V$.

\sstab(2) We extend $Seq$ such that $O^+=\Delta O/O$, where $\Delta O$ is a topological order of the block-wise graph $G'_\Delta(V'_\Delta,E'_\Delta)$ of $G^+[\Delta V]$. And we have:
%Let $\Delta O$ be a topological order of the block-wise graph $G'_\Delta(V'_\Delta,E'_\Delta)$ of $G^+[\Delta V]$; further let $O^+$ be $\Delta O/O$. We then have the follows.
