...
The incremental prestige computation for authors is exactly the same as \batensemble, due to the way that the prestige of authors is defined and the prestige of each article is potentially changed.
...
%
Algorithm~\twprscc computes SCCs of citation and venue graphs $G$ as well as the an topological order of the block-wise graph $G'$ as auxiliary data structures.
...
\sstab(2) Let $\Delta O$ be a topological order of the block-wise graph $G'_\Delta(V'_\Delta,E'_\Delta)$ of $G^+[\Delta V]$; further let $O^+$ be $\Delta O/O$. We then have the follows.
