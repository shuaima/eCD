...
\icc\>\>\> update $PR^+(v)$ where $v\in scc$ with Eq.~(\ref{eq-inc-prscc}) until convergence; \\
%the sum of \\ \hspace{8ex} TWPageRank score changes is less than $\epsilon\cdot |scc|/|V^+|$; \\
\icc\>\>\> label SCC $w'$ as $B$ such that $(v',w')\in E'$;\\
...
%
\marked{According to our statistics of affected/unaffected areas in Table 2, $E_A\cup E_{AB}$ accounts for 28\% of total edges on all citation graphs, while $|V_C|$ and $|E_{CB}\cup E_C|$ are less than 11\% of $|V^+|$ and $|E^+|$ on all graphs.
%
These make \inctwprscc faster than \twprscc even though they have very similar time complexity. }
%, as will also been shown by the experimental study,

\marked{
\stitle{Time \& space complexity analyses of the complete algorithm}.
By the analyses above, the time complexity of \incensemble is the same as \batensemble, except that \incensemble saves $O(|E^c_A\cup E^c_{AB}|)$ time on the updated citation graph and $O(|E^v_A\cup E^v_{AB}|)$ time on the updated venue graph when updating scores. And its space complexity is also the same as \batensemble, except that \incensemble uses $(|V^c\cup\Delta V^c|+|V^v\cup\Delta V^v|)$ extra space to store the affected and unaffected areas and $|E^c\cup \Delta E^c|+|E^v\cup \Delta E^v|$ extra space to store edge weights before updates.
}
