...
We first present the incremental computation of prestige.
...
The incremental prestige computation for article and venues is to compute the TWPageRank vector on the updated graph $G^+$,
given an update $\Delta$ = $\Delta V\cup\Delta E$ and the TWPageRank vector on the original citation graph or venue graph $G$.
%
...
We now present the details of incremental TWPageRank.

%%% affected/unaffected division
\stitle{Affected and unaffected areas}.
The TWPageRank vector of the original graph $G$ is mainly affected in two ways.
...
\sstab(2) Let $V_A = V\setminus V_B$. As the TWPageRank scores on $V_A$ can be simply adjusted by scaling in terms of  Lemma~\ref{lemma-subgraphA} (to be seen shortly), $V_A$ is essentially not affected by the update $\Delta$.
...
%
Then we have $V_C$ = $\Delta V$, $E_C\cup E_{CB}$ = $\Delta E$, $V$ = $V_A\cup V_B$, and $E$ = $E_A\cup E_B\cup E_{AB}$.
...
\bcc \hspace{1.5ex}\=  $G_C'$ := the converted graph of $G_C$; \ \  $E_{CB}'$ := edges from $G_C'$ to $G'$\\
...
\icc\>\>\hspace{3ex}\= update $PR^+(v)$ where $v\in scc$ following lines (5--6), Fig.~\ref{alg-TWPageRank-venue}; \\
\icc\>\>\> label SCC $w'$ as $B$ such that $w'\in V'$ and $(v',w')\in E_{CB}'$;\\
\icc\>\> \Else \If $scc$ is labeled as $B$ \Then \\
\icc\>\>\> update $PR^+(v)$ where $v\in scc$ following lines (5--6), Fig.~\ref{alg-TWPageRank-venue}; \\
\icc\>\>\> label SCC $w'$ as $B$ such that $(v',w')\in E'$;\\
...
\stitle{Algorithm \inctwprscc}. We now present our incremental algorithm for computing TWPageRank, shown in Fig.~\ref{alg-inctwprscc}.
Note that notations without/with superscript `$+$' are defined on $G$ and $G^+$, respectively.
%We expand $M$ and $PR$ to the same sizes as $M^+$ and $PR^+$, respectively, by filling zeros, and let $\Delta M=M^+ - M$ and $\Delta PR = PR^+ - PR$.


It takes as input an update $\Delta$ and the previous results on the original graph $G(V,E)$, and returns the TWPageRank vector of the updated graph $G^+$. It first incrementally computes the topological order $O^+$ by concatenating the topological orders of $G_C'$ and $G'$ (lines 1--2).
%
...
%
More specifically, the scores of nodes in SCCs labeled as $C$ and $B$ are updated the same to Algorithm~\twprscc (lines 5--6, Fig.~\ref{alg-TWPageRank-venue}), while the scores of nodes in SCCs labeled as $A$ are simply scaled. Algorithm \inctwprscc further uses the existing TWPageRank vector scaled with constant ${n}/{n^+}$ as the initial vector for nodes in SCCs labeled as $B$.
%
...
\begin{proof}
It suffices to show that for each $(u,v)\in E'\cup E_C'\cup E_{CB}'$, $u$ comes before $v$ in $O^+$,
which obviously holds (1) for  $E'\cup E_C'$ as $O$ and $\Delta O$ are topological orders of $G'$ and $G_C'$, respectively, and (2) for $E_{CB}'$ as nodes in $G_C'$ come before nodes in $G'$.
\end{proof}
...
\begin{proofSketch}
Assume a topological order $v_1'/\dots/v_{l}'$ of graph $G^+{'}$ where $l=|O^+|$. It suffices to prove the sum of change of $PR^+(v)$ for $v\in scc_k$ is no more than $|scc_k|\cdot\epsilon/n^+$ for each $scc_k$ corresponding to $v_k'$ ($k\in [1,l]$) by induction.
...
Given an update $\Delta$ = $\Delta V\cup\Delta E$, and the original graph $G(V,E)$, together with its TWPageRank vector $PR$ and the topological order $O$, algorithm \inctwprscc takes $O(|V\cup \Delta V|+|E\cup\Delta E\setminus E_A|+t^+|E_{Bw}\cup E_{Cw}|)$ time.
\end{theorem}

\begin{proof}
Observe the following. (1) A topological order of $G_C$ can be computed in $O(|V_C|+|E_C|+|E_{CB}|)$ time.
(2) Updating the TWPageRank scores of nodes in subgraphs $G_B$ and $G_C$ costs $O(|V_B\cup V_C|+|E_{B,b}\cup E_{C,b}\cup E_{AB}\cup E_{CB}|)+t^+|E_{B,w}\cup E_{C,w}|)$ time. Finally, (3) Updating the scores of nodes in $G_A$ costs $O(|V_A|)$ time.
\end{proof}
...
By Proposition~\ref{prop-converg} and Lemmas \ref{prop-prscc},\ref{lemma-inc-topo} \& \ref{lemma-subgraphA}, one can easily verify the correctness of algorithm \inctwprscc.
%
Note that  a) algorithm \inctwprscc only costs $|V_A|$ time to update the scores of nodes in $G_A$, b) it derives the topological order based  on $\Delta G$ only, instead of $G^{+}$, and c) the number $t^+$ is very likely smaller than the number $t$ of \twprscc when updating scores of nodes in $V_B$. As shown by the experimental study, \inctwprscc is faster than \twprscc, even though they have very similar time complexity.
...
(2) It processes $G^c_A$ and $G^v_A$ in $O(|V^c_A|)$ (comparatively, $O(|V^c_A|+|E^c_{A,b}|+t^+|E^c_{A,w}|)$) and $O(|V^v_A|)$ (comparatively, $O(|V^v_A|+|E^v_{A,b}|+t^+|E^v_{A,w}|)$) time;
