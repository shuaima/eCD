...
\icc\>\>\> label SCC $w'$ as $B$ such that $w'\in G'$ and $(v', w')\in E^+{'}$;\\
...
When processing on $V_B$ with Eq.~(\ref{eq-inc-prscc}), the {\em edges in $E_{AB}$ can be skipped}  since $PR^+(u)={n}/{n^+}\cdot PR(u)$ for nodes $u\in V_A$ and $M_{u,v}=M^+_{u,v}$ given $(u,v)\in E_{AB}$. Besides, we use ${n}/{n^+}\cdot PR$ as the initial iteration vector. Both of them can speed up the computation.
...
%
When updating the TWPageRank scores, following $O^+$, scores of nodes in $G_C$, $G_B$ and $G_A$ are computed by iterations from scratch, by iterations with Eq.~(\ref{eq-inc-prscc}) using existing TWPageRank vector and by scaling, respectively.
\end{example}
