...
(1) It saves $O(|V^c|+|E^c|)$ and $O(|V^v|+|E^v|)$ time when maintaining \sccs and the topological order on $G^c_C$ and $G^v_C$;
(2) It saves $O(|E^c_A \cup E^c_{AB}|)$ and $O(|E^v_A \cup E^v_{AB}|)$ time when updating scores on $V^c$ and $V^v$;
(3) It saves $O(|E^c|)$ time when computing popularity of articles;
