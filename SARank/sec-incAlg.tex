...
%
\marked{According to our statistics of affected/unaffected areas in Table 2, $E_A\cup E_{AB}$ accounts for 28\% of total edges on all citation graphs, while $|V_C|$ and $|E_{CB}\cup E_C|$ are less than 11\% of $|V^+|$ and $|E^+|$ on all graphs, respectively.
%
...
\marked{
Indeed, by using a reasonable extra space, algorithm \incensemble is typically faster than \batensemble as follows.
...
And, finally, (d) It is likely to update TWPageRank scores on $V^c_B$ and $V^v_B$ with less iterations.
}

%That is, \incensemble speeds up computation with a reasonable extra space.
