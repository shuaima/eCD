...
$\dots,R_n)$, where $R_i$ is a relation schema for $i \in [1, n]$.
...
relations and their arities in $\R$, regardless of $\Sigma$.\eop
\end{theorem}
%\vspace{-0.5ex}

We next present the main techniques for the query generation
method. Let $\SCFD^i$ be the set of all \pCFDs in $\Sigma$
defined on the same relation schema $R_i$, and $\SCIND^{(i,j)}$ the
...
\bi
\item Add a tuple $t_1$ to \Enc{L} such that
...
\item Similarly add a tuple $t_2$ to \Enc{R} for attributes in $Y$.
\vspace{0.5ex}
...
\vspace{-1ex}
\begin{center}
\hspace{6ex}(1)~\Enc{L} \hspace{22ex}  (2)~\Enc{R} \hspace{18ex} (3)~\Enc{\ne}\\
\vspace{0.5ex}
...
\vspace{-1ex}
\begin{center}
\hspace{-1ex} (1)~\Enc{} \hspace{16ex} (2)~\Enc{L} \hspace{10ex} (3)~\Enc{R} \hspace{13ex}(4)~\Enc{\ne} \\
\vspace{0.5ex}
\begin{tabular}{|c|c|c|}
...
omitted, for the same reason as for \pCFDs mentioned above. \eop
\end{example}\vspace{-0.5ex}
...
\vspace{-1ex} \begin{footnotesize}\mat{4ex}{
\= \=\bd{select} \=  $R_i.*$ \\
...
$R$.\at{cid}  \bd{and} $L$.\at{cid} = $H$.\at{cid} \bd{and}\\
\>\>\>\>$R_j.Y\asymp R$ \bd{and} $R_j.Y\asymp N$) }\vspace{-1ex}
\end{footnotesize}
...
\vspace{-1.5ex} \begin{footnotesize}\mat{1ex}{
...
\> \bd{and} ($L.A_{k_{<}}$ \bd{is null or} $R_i.A_k$ $<$ $L.A_{k_{<}}$)
 \bd{and}
($L.A_{i_{\le}}$ \bd{is null or} $R_i.A_k$ $\le$ $L.A_{i_{\le}}$))
}\vspace{-1.5ex}
\end{footnotesize}
...
\vspace{-1.5ex} \begin{footnotesize}\mat{2ex}{
\bd{not exists}
(\bd{select} $*$ \bd{from} $N$ \bd{where} \= $L.\at{cid}$ =
...
$L.$\at{type} \bd{or} $L.$\at{type} = `\_') \bd{and not exist} (\\
\>\hspace{2ex}\=\bd{select * }\bd{from} $N$ \\
\>\>\bd{where} $N.\at{cid}$ =
$L.\at{cid}$ \bd{and} $N.\at{pos} = $ `\LHS' \bd{and} $N.\at{att} = $ `type')\\
\>\> \bd{and} ($L.$\at{state} \bd{is null or}  $R_1.$\at{state} =
$L.$\at{state} \bd{or} $L.$\at{state} = `\_') \bd{and not exist} (\\
\>\>\bd{select * }\bd{from} $N$ \\
\>\>\bd{where} $N.\at{cid}$ = $L.\at{cid}$ \bd{and} $N.\at{pos} = $
`\LHS' \bd{and} $N.\at{att} = $ `state' \bd{and} $R_1.\at{state} =
$N.\at{val})\\
\stab

\> \>  \bd{and not exists} (\\
\>\>\bd{select} $R_2.*$ \bd{from} \at{tax} $R_2$, \Enc{} H, \Enc{R} R\\
\>\>  \bd{where}  ($H.\at{state_L}$ \bd{is null or}
$H.\at{state_R}$ \bd{is null or} $H.\at{state_L} != H.\at{state_R}$ \bd{or}\\
\>\>\hspace{2ex}\=$R_2.\at{state}$ = $R_1.\at{state}$) \bd{and}
$L.\at{cid}$ =
$H.\at{cid}$  \bd{and} $L.\at{cid}$ = $R.\at{cid}$ \bd{and}\\
\>\>\>($R.$\at{rate} \bd{is null or}  $R_2.$\at{rate} =
$R.$\at{rate} \bd{or} $R.$\at{rate} = `\_') \bd{and not exist} (\\
\>\>\>\bd{select * }\bd{from} $N$ \\
\>\>\>\bd{where} $N.\at{cid}$ = $R.\at{cid}$ \bd{and} $N.\at{pos} =
$ `\RHS' \bd{and} $N.\at{att} = $ `rate' \bd{and} $R_2.\at{rate} =
$N.\at{val})) }\end{footnotesize}
...
or \Enc{R} when encoding \pCINDs or \pCFDs. \eop
\end{example}\vspace{-1.5ex}
