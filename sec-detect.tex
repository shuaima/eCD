...
\noindent for $j\in[1, m_1]$; (3) $R_i.Y\asymp R$ is defined similarly for attributes in $Y$; (4) $R_i.X\asymp N$ is the conjunction of
...
\noindent for $j\in[1, m_1]$; (5) $R_i.Y\asymp N$ is defined similarly, but with $N.\at{pos}='\RHS'$; (6) $X_{L}$ is the set of following attributes

\begin{footnotesize}\mat{3ex}{
(\bd{case when} $L.A_j$ \bd{is not null then} $R_i.A_j$ \bd{end}) \bd{as} ${A_L}_j$
}
\end{footnotesize}
\noindent for $j \in [1, m_1]$; (7) Similarly, $Y_{R}$ is the set of
\begin{footnotesize}\mat{3ex}{
(\bd{case when} $R.B_k$ \bd{is not null then } $R_i.B_k$ \bd{end}) \bd{as} ${B_R}_k$
}
\end{footnotesize}
\noindent for $k \in [1, m_2]$; (8) $R.Y$ = `\_' is the disjunction of $R.B_k$ = `\_' for  $k \in [1, m_2]$.
...
\> ($L.A_{i_{>}}$ \bd{is null or} $R_i.A_k$ $>$ $L.A_{i_{>}}$) bd{and} \\
\>  $L.A_{i_{\ge}}$ \bd{is null or} $R_i.A_k$ $\ge$ $L.A_{k_{\ge}}$) bd{and} \\
