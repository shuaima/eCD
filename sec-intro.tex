...
Extensions of functional dependencies (\FDs) and inclusion dependencies
(\INDs),
known as {\em conditional functional dependencies} (\CFDs~\cite{CFDs})
and {\em conditional inclusion dependencies} (\CINDs~\cite{CINDs}),
...
Conditional dependencies are able to capture more inconsistencies
than \FDs and \INDs~\cite{CFDs,CINDs}.

Conditional dependencies specify constant patterns in terms of
equality $(=)$. In practice, however, the semantics of data often
needs to be specified in terms of other predicates such as
$\ne, <, \le, >$ and $\ge$, as illustrated by the example below.
...
and conditional dependencies (\CFDs, \CINDs~\cite{CFDs,CINDs})
...
as \CFDs of \cite{CFDs} or \CINDs of \cite{CINDs},
...
range $[2.99, 9.99)$. Dependency $\kw{pind}_1$ extends
\CINDs~\cite{CINDs} by specifying patterns with $\ne$: for any
...
Using $\kw{pfd}_1$--$\kw{pfd}_4$ and $\kw{pind}_1$ as data quality
...
or as \CFDs~\cite{CFDs} or \CINDs~\cite{CINDs}, which specify
patterns with  equality $(=)$ only. While there have been
extensions of \CFDs~\cite{icde08,divesh08}, none of these allows
...
\stitle{Contributions}. This paper proposes
...
$\ne, <, \le, >, \ge$ predicates. For example, all
...
taken separately or together. The
...
These are {\em the same as} their \CFDs and \CINDs counterparts.
While data with linearly ordered domains often makes our
lives harder (see, \eg~\cite{Mayden97}), \pCFDs and \pCINDs
do not complicate their static analyses.

\sstab
\noindent(3) We provide \SQL-based techniques to detect errors
based on \pCFDs and \pCINDs. Given a set $\Sigma$ of {\pCFDs}
...
of $\Sigma$.
No previous work has been studied error detection based on \CINDs,
